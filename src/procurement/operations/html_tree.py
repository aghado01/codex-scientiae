...
_ARXIV_LATEXML_MARKER = "generated by latexml"
...
def is_arxiv_html_paper(head: str) -> bool:
    """Return whether decoded HTML is a LaTeXML arXiv paper rather than site chrome."""

    if not is_html_document(head):
        return False
    folded = head.casefold()
    return _ARXIV_PAPER_MARKER in folded or _ARXIV_LATEXML_MARKER in folded
...
def _read_head(root: PinnedPublicationRoot, leaf: str, *, limit: int = HTML_HEAD_BYTES) -> str:
...
async def retrieve_html_tree(
    ...
    *,
    ...
    try:
        ...
        raise ArtifactUnavailableError("HTML payload is not valid UTF-8") from exc
    if not html_probe_accepts(document, provider=provider):
