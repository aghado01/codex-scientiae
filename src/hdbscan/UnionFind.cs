...
/// <summary>
/// Path-compressed union-find with union-by-size — a reusable connectivity primitive.
/// HDBSCAN's Kruskal single-linkage pass drives it (Find / Union / Reroot / Reset), but
/// the general operations (Size / GetLabels / WriteRootSizesTo) are kept so the type
/// stands on its own as a component-labelling utility.
/// </summary>
public sealed class UnionFind
{
    ...
    /// <summary>
    /// Resets all nodes to singleton components without reallocation, so the structure
    /// can be reused across runs (the dendrogram build calls this before each pass).
    /// </summary>
    public void Reset()
    {
