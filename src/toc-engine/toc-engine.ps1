...
# The heading-slug engine comes from the SHARED tier, not from the legacy module this one supersedes.
# Carrying a private copy (which is what independence from md-toc first bought) leaves two live
# definitions of a repo-wide invariant — anchors written by one lane must match a TOC regenerated by
# any other, so drift between copies breaks links silently.
. "$PSScriptRoot/../shared/md-anchor.ps1"
