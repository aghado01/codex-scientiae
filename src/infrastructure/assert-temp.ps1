...
<#
  src/infrastructure/assert-temp.ps1 — child-process CDXSCI_TEMP projection.

  tests/run.ps1 calls this so Pester TestDrive and GetTempPath cannot follow the ambient OS temp
  tree. It is not part of containment.ps1: Set-TempEnvironment owns the parent-run value;
  this assert is a child-entry check that can be retired independently.
#>

. "$PSScriptRoot/containment.ps1"

function Assert-TempEnvironment {
    <# Require CDXSCI_TEMP under artifacts/. Project it onto TEMP/TMP/TMPDIR for this process so
    ...
    if ([string]::IsNullOrWhiteSpace($value) -or
        ...
        throw 'CDXSCI_TEMP must be an absolute path under RepositoryRoot/artifacts'
    }
    $resolved = Resolve-ArtifactDescendantPath -Value $value `
        -RepositoryRoot $RepositoryRoot -Role 'CDXSCI_TEMP'
