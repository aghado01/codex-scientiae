...
. "$PSScriptRoot/../audits/math-render/math-render.ps1" # reusable math-render audit; retained here while the membrane shell retires
. "$PSScriptRoot/../audits/md-lint.ps1"             # markdown structure lint (markdown_lint): the non-math half of the standard
# EXPERIMENTAL: Export-BenchmarkTrial (harvest well-posed repair problems). The reorg RENAMED
# src/benchmark.ps1 to src/pdf-converter/harvest.ps1; this reference was never updated, and because the
# dot-source was unconditional a missing optional module took the whole server down at startup.
# Guarded now — the harvest tool is opt-in (-Experimental) and its module is about to relocate again
# with the pdf-converter tree, so its absence must degrade to one unavailable tool, not a dead server.
$script:HarvestLib = Join-Path $PSScriptRoot '../pdf-converter/harvest.ps1'
if (Test-Path -LiteralPath $script:HarvestLib -PathType Leaf) { . $script:HarvestLib }
else { [Console]::Error.WriteLine('codex-membrane: harvest.ps1 not found — the EXPERIMENTAL harvest tool is unavailable this session') }
...
$Tools = @(
    ...
    @{ name = 'latex_convert'
       description = 'The LaTeX ORACLE lane: convert a staged arXiv LaTeX SOURCE into a codex-standard markdown FAITHFUL transcription — the near-lossless, algorithmic ground truth / benchmark answer key against which lossy PDF->IR conversions are measured. In-house (do NOT shell out to pandoc/latexml). Expands \newcommand macros to KaTeX primitives, wraps alignment envs in \begin{aligned}, resolves \cite/\ref/\eqref + figure/table refs to numbers, reflows source hard-wraps into flowing prose (STANDARDS §4), recovers biblatex + inline bibliographies, and emits an inline References section. FAITHFUL, not editorial: acknowledgements are KEPT and references stay INLINE (never split to a sidecar) — that filtering is the PROMOTION phase''s job, not the converter''s. GROUPING-AWARE: address with `paper` — a slug OR an ingestion-root-relative paper-dir PATH (e.g. "gauntlet/ph-zigzag/2403.08110v4") — and it locates the source wherever it lives and writes {slug}-latex.md + figures BESIDE the source; it does NOT force-stage into _inbox or flip-flop locations. Unpacks the tarball ONCE into {slug}-latex/ beside the source — deterministic, so re-runs read it instead of re-expanding a private copy (pass reuse_source: true to skip the unpack entirely). Per-run regenerable output (oracle counts plus the strict shared math-render audit) lands under artifacts/latex-ingest/runs/{stamp}/{slug}/, not beside the source. Requires the "source" artifact staged via codex-arxiv fetch (artifacts: ["source"]) and the shared Node payload restored under packages/node — a PDF-only paper has no LaTeX source. Returns path + stats + audits.'
       inputSchema = @{ type = 'object'; properties = @{ paper = @{ type = 'string'; description = 'a slug OR an ingestion-root-relative paper-dir path — respects curated groups, e.g. "gauntlet/ph-zigzag/2403.08110v4"; {slug}-latex.md is written BESIDE the source' }; id = @{ type = 'string'; description = '(legacy alias for `paper`) a bare arXiv id; its *.tar.gz is searched for anywhere under the root, e.g. 1611.03935' }; enable_embedded_toc = @{ type = 'boolean'; description = 'enable embedded in-doc ## Contents block (default false; disabled for pristine manuscript transfer)' }; disable_tree_toc = @{ type = 'boolean'; description = 'disable standalone {slug}-tree.md sidecar (default false; sidecar enabled by default)' }; reuse_source = @{ type = 'boolean'; description = 're-run against the already-unpacked {slug}-latex/ source instead of re-expanding the tarball; also preserves hand-edits made to the staged source while iterating' }; disable_jsonl_toc = @{ type = 'boolean'; description = 'disable standalone {slug}.toc.jsonl sidecar (default false; sidecar enabled by default)' } } } }
    @{ name = 'render_check'
       description = 'Validate that every math span in a markdown deliverable RENDERS under the shared math-render audit (currently KaTeX). Reports total/ok/failed plus each failure with its exact engine error. Address by `paper` (its latest run''s finalized <slug>.md) OR a repo-relative `path`. strict=true is the KaTeX-strict bar. Requires the shared Node payload restored under packages/node.'
...
function Invoke-Tool([string]$name, $arguments) {
    ...
    switch ($name) {
        ...
        'latex_convert' {
            ...
            $out = Invoke-ArxivLatexToMarkdown -TarGz $tarItem.FullName -Slug $slug -OutDir $dir `
                -ReuseSource:([bool]$arguments.reuse_source) `
                -EnableEmbeddedToc:([bool]$arguments.enable_embedded_toc) `
                -DisableTreeToc:([bool]$arguments.disable_tree_toc) `
                -DisableJsonlToc:([bool]$arguments.disable_jsonl_toc)
        }
        'render_check'  { $out = Invoke-MathRenderAudit -Path (Resolve-Deliverable ([string]$arguments.paper) ([string]$arguments.path)) -Strict:([bool]$arguments.strict) }
