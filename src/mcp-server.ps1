...
# relative or absolute) resolves IN PLACE — so a curated group like compendia/ph-zigzag/{slug} is honored and
...
function Resolve-SourceDir([string]$paper) {
    if ([string]::IsNullOrWhiteSpace($paper)) { throw "provide 'paper' (a slug, or an ingestion-relative paper-dir path like compendia/ph-zigzag/2403.08110v4)" }
    $p = $paper -replace '\\', '/'
    $rootFull = [System.IO.Path]::GetFullPath($Root)
    $sep = [System.IO.Path]::DirectorySeparatorChar
