...
#Results: Here, we focus on recent work using TDA to study different regimes of parameter space for a well-studied model of angiogenesis. We propose a method for combining TDA with ABC to infer parameters in the Anderson? Chaplain model of angiogenesis. We demonstrate that this topological approach outperforms ABC approaches that use simpler statistics based on spatial features of the data. This is a ?rst step toward a general framework of spatial parameter inference for biological systems, for which there may be a variety of ?ltrations, vectorizations and summary statistics to be considered.

Availability and implementation: All code used to produce our results is available as a Snakemake work?ow from github.com/tt104/tabc_angio.
...
Fig. 2. Visualizations of simulation output from the Anderson?Chaplain model for ?ve parameter sets sampled from a uniform prior on the model parameters. The ?rst column shows the observed data, while the second shows a contour plot of the posterior density inferred by applying the TABC methodology, with the red cross indicating the known parameter values used to generate the observed data. The remaining three columns show simulations of parameter values drawn from the ABC posterior predictive distribution
...
H.A.H. thanks PG Kevrekidis and N Whitaker for ?rst presenting the challenge of inferring parameters in models of angiogenesis. H.A.H. also thanks members of the Centre for TDA, speci?cally A. Barbensi, H. Byrne, L. Marsh and U. Tillmann for many stimulating discussions and helpful comments. P.D.W.K. is grateful to L. Reali for useful conversations.
...
All code used to produce our results is available as a Snakemake ( Ko �ster and Rahmann, 2012 ) work?ow from github.com/tt104/tabc_angio. It is also stored as an archive on Zenodo with doi: 10.5281/zenodo.5562670.
$$
