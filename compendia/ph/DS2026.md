...
```
Input:  ZZ — quasi zigzag bi-filtration; k ≥ 1; p Output: λ(p, k) — ZZ-GRIL value at point p for fixed k

Initialize: δ_min ← 1, δ_max ← len(F), λ ← 1 while δ_min ≤ δ_max do δ ← (δ_min + δ_max) / 2 I ← p²_δ;  r ← ComputeRank(ZZ, I)
    if r ≥ k then λ ← δ;  δ_min ← δ + 1 else δ_max ← δ − 1 end if end while return λ
```
