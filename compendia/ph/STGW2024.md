...
$$
\Omega_{n}^{k} ( M ) = \{\omega \in \Omega^{k} ( M ) \, | \quad \omega |_{\partial M} = 0 \} ;
$$

\Omega _ { t } ^ { k } ( M ) = \{ \omega \in \Omega ^ { k } ( M ) | \quad ^ { \* } \omega | _ { \partial M } = 0 \}.

$$
Following their definitions, the spaces Ω k n ( M ) and Ω m - k t ( M ) are isomorphic under the Hodge star operator \star , also known as the Hodge duality. Moreover, the di ff erential d preserves the normal boundary conditions, while the codi ff erential \delta preserves the tangential boundary conditions.
...
$$
The orthogonality of the decomposition directly comes from the adjointness of \delta and d when enforcing the normal or tangential boundary conditions. For \omega \in Ω k ( M ), there is a unique decomposition of \omega given as follows:
$$

\omega = d \alpha _ { n } + \delta \beta _ { t } + \eta,

$$
where \alpha n \in Ω k + 1 n ( M ), \beta t \in Ω k + 1 t ( M ), and \eta \in H k ( M ). Note that the potentials \alpha n and \beta t are not uniquely determined as all \alpha n + d \eta and \beta t + \delta \gamma with any \eta \in Ω k - 2 n ( M ) and \gamma \in Ω k + 2 t ( M ) serve as potentials for the same components. However, the issue can be addressed by enforcing gauge conditions, such as
$$
...
$$
\begin{cases} \Delta \alpha_{n} = \delta \omega, \\ \Delta \beta_{t} = d \omega, \end{cases} \quad ( 2. 1 6 )
$$
...
$$
= d \Omega_{n}^{k - 1} ( M ) \oplus \delta \Omega_{t}^{k + 1} ( M ) \oplus \mathcal{H}_{e x}^{k} ( M ) \oplus \mathcal{H}_{t}^{k} ( M ).
$$
...
$$
The null spaces of these discrete Hodge Laplacians, as in the continuous case, are fully determined by the topology of the underlying manifold M, since they only depend on the di ff erential and projection matrices. The dimension of the kernel of L k, n is given by the Betti number \beta m - k, while the dimension of the kernel of L k, t is given by \beta k. Here, the Betti number \beta k presents directly the number of k -dimensional holes on the manifold M. For instance, \beta 0 gives the number of connected components, \beta 1 gives the number of tunnels, and \beta 2 provides the number of closed cavities, respectively. The spectra of these Laplacians, in addition, could be used to study the geometric information of the manifold. It is known that the nonzero eigenvalues of the Laplacians provide rich insights into the shape of a manifold. For instance, the Fiedler value, defined as the smallest nonzero eigenvalue of a graph Laplacian, describes connectivity. As another example, the multiplicity of eigenvalues can reveal certain symmetries of the shape.
...
Denote by I l, 1 the map from Ω k n ( M l ) to Ω k n ( M l + 1 ) sending \omega to \omega . Note that ( d ◦ I l, 1 )( \omega ) is 0 on M l, l + 1 and thus the same as the extension of the di ff erential of a normal form d \omega on M l, l + 1, i.e., d ◦ I l, 1 = I l, 1 ◦ d. It follows that there is a commutative diagram
$$
...
$$
Here d l,\delta l denotes the di ff erential and codi ff erential on Ω k ( M l ), d l + p,\delta l + p denotes the di ff erential and codi ff erential on Ω k ( M l + p ), respectively, and R l, p is the projection of di ff erential forms in Ω k n ( M l + p ) to the space spanned by the harmonic extensions followed by the restriction to M l. Let ˜ \delta l, p = \delta l + p ◦I l, p and ˜ d l, p = R l, p ◦ d l + p. By the construction of the extension, we have ( ˜ \delta l, p \omega ,\eta ) = ( \omega , ˜ d l, p \eta ), i.e., ˜ \delta l, p are adjoint to ˜ d l, p. We then define the p -persistent Hodge Laplacian operator \Delta p n, l : Ω k n ( M l ) \to Ω k n ( M l ) as follows:
$$
...
$$
See Figure 4 for an example of such a nested sequence of sub-cell complexes in a 2D Cartesian grid. Denote by C k ( K l ) the space of discrete k -forms on sub-complex K l with 0 \leq l \leq s. Note that K l \subset K l + 1. A discrete k -form on K l can be easily extended to K l + 1 by solving the discrete Laplace equation with the above boundary conditions for values on every k -cells in K l, l + 1 = Cl( K l + 1 \ K l ), the closure of the di ff erence complex. We denote this extension map as I l, 1 : C k ( K l ) \to C k ( K l + 1 ) and by I l, p = I l + p - 1, 1 ◦ I l + p - 2, 1 ◦ \cdot \cdot \cdot ◦ I l, 1 : C k ( K l ) \to C k ( K l + p ) the extension mapping from the space of discrete k -forms on K l to the space of discrete k -forms on K l + p, which may also be constructed directly by solving the Laplace equation on K l, l + p = Cl( K l + p \ K l ). With this extension mapping, the space of discrete k -forms on K l can be seen as a subspace of discrete k -forms on K l + p.
...
$$
where D k l : C k + 1 ( K l ) \to C k ( K l ) denotes the discrete di ff erential operator, and \delta k l : C k ( K l ) \to C k - 1 ( K l ) denotes the discrete codi ff erential operator on K l.
...
$$
where Id K l is the identity matrix in K l up to a rescaling in the boundary, provides the combination of \omega in K l and d ˜ \zeta in K l, l + p \ \partial K l, when applied to \omega . The matrix corresponding to I l, p is dense for rows corresponding to cells in K l, l + p but diagonal for rows corresponding to cells in K l. Note that \delta \partial K l is not necessarily 0 for co-closed \omega , but is 0 for co-exact \omega . k 1 n
The adjoint operator of \delta +, l, p defines D k l, p. In the following, we drop most of the subscripts for clarity. Recall that ( \omega , ˜ d \eta ) = ( ˜ \delta \omega ,\eta ) can be discretized as
$$

[ W ] ^ { T } S [ \tilde { D } E ] = [ S ^ { - 1 } D ^ { T } S I _ { l, p } W ] ^ { T } S [ E ]

$$
with W and E as discrete versions of \omega and \eta . Thus, ˜ D = S - 1 I T l, p S D, from which we may recognize the restriction operator as R = S - 1 I T l, p S. This restriction operator can be seen as the L 2 -projection onto the space formed by all harmonic extensions from Ω K n ( M l ). k k + 1 ˜
Note that in this case, immediately \delta l, p \delta l = 0 since the extension operator will generate \zeta = 0 for any co-exact form \omega = \delta \beta on K l as the righthand side of the associated linear system essentially
$$
