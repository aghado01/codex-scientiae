...
$$
Y_{0} \times ( - \infty, a_{1} ] \cup_{f_{0}} Z_{1} \cup_{g_{1}} \dots, \cup_{f_{n - 1}} Z_{n} \cup_{g_{n}} Y_{n} \times [ a_{n}, \infty ) \quad \stackrel{\text{which}}{\text{is}} \text{last}
$$
...
$$
\begin{array}{r l r l}{X_{0}^{0}} &{X_{1}^{1}} &{\cdots} &{X_{n - 1}^{n - 1}} &{X_{n}^{n}} \\{( - \infty, a_{1} )} &{( a_{1}, a_{2} )} &{\cdots} &{( a_{n - 1}, a_{n} )} &{( a_{n}, \infty )} \end{array}
$$
...
$$
\begin{aligned}{[ X_{i-1}^i, X_{j-1}^j ]} &\leftrightarrow [a_i, a_j] \quad &\text{for} 1 \leq i \leq j \leq n, \\{[ X_{i-1}^i, X_j^j )} &\leftrightarrow [a_i, a_j) \quad &\text{for} 1 \leq i < j \leq n+1, \\{( X_i^i, X_{j-1}^j ]} &\leftrightarrow (a_i, a_j] \quad &\text{for} 0 \leq i < j \leq n, \\{( X_i^i, X_j^j )} &\leftrightarrow (a_i, a_j) \quad &\text{for} 0 \leq i < j \leq n+1.
\end{aligned}
$$
...
$$
H_{p} ( X_{0}^{0} ) \rightarrow \cdots \rightarrow H_{p} ( X_{0}^{n} ) \leftarrow \cdots \leftarrow H_{p} ( X_{n}^{n} ) \quad \\tag{1} \quad \ t i l a{x}
$$
...
$$
\begin{array}{r l r l}{H_{p} ( X_{0}^{0} )} &{\to} &{\cdots} &{\to} &{H_{p} ( X_{0}^{n} )} \\ &{\downarrow} & &{\downarrow} &{\\tag{2}} \\{H_{p} ( X_{0}^{n}, X_{0}^{n} )} &{\leftarrow} &{\cdots} &{\leftarrow} &{H_{p} ( X_{0}^{n}, X_{n}^{n} )} \end{array}
$$
...
The following two theorems relate the diagrams of f and − f if the domain X is a manifold [7]. Denoting with superscript T the reflection across the principal diagonal, ( x, y ) $\to$ ( y, x ), we get the following duality.
...
$$
\begin{array}{r l r}{O r d_{r} ( f )} &{=} &{O r d_{d - r - 1}^{R} ( - f ),} \\{E x t_{r} ( f )} &{=} &{E x t_{d - r}^{0} ( - f ),} \\{R e l_{r} ( f )} &{=} &{R e l_{d - r + 1}^{R} ( - f ).} \end{array}
$$
...
$$
\begin{array}{r l r}{O r d_{r} ( f )} &{=} &{R e l_{r + 1}^{0} ( - f ),} \\{E x t_{r} ( f )} &{=} &{E x t_{r}^{R} ( - f ),} \\{R e l_{r} ( f )} &{=} &{O r d_{r - 1}^{0} ( - f ).} \end{array}
$$
...
$$
\begin{array}{r l r}{d_{B} ( O r d_{p} ( f ), O r d_{p} ( g ) )} &{\leq} &{\delta,} \\{d_{B} ( R e l_{p} ( f ), R e l_{p} ( g ) )} &{\leq} &{\delta,} \\{d_{B} ( E x t_{p} ( f ), E x t_{p} ( g ) )} &{\leq} &{\delta.} \end{array}
$$
...
$$
\begin{aligned}
\text{If} \quad H ( K_{i} ) \stackrel{f_{i}}{\rightarrow} H ( K_{i + 1} ), \\ \text{then} \quad & \mathcal{R}_{i + 1} = \left ( f_{i} ( R_{i}^{0} ), f_{i} ( R_{i}^{1} ), \dots, f_{i} ( R_{i}^{i} ), H ( K_{i + 1} ) \right ) \\ & b_{i + 1} = ( b_{i}^{1}, \dots, b_{i}^{i}, i + 1 ).
\end{aligned}
$$
...
## 2. Let c = C i [ j ][ \sigma ] be the coefficient of \sigma in the chain C i [ j ]
...
- 6: j = k
- 2. Subtract cycle ( C i [ k ][ \sigma ] /Z i [ j ][ \sigma ]) \cdot Z i [ j ] from every chain C i [ k ].
$$
...
$$
if and only if there is a k \leq k such that low( T [ k ]) = j.
