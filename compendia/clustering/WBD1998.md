...
The goal is to find the "best" classification for a given distribution of data points. A classification $\{s\}$ is defined by assigning to each point $\mathbf{x}_i$ a label $s_i$ that may take integer values $s_i = 1, \ldots, q$. We define a cost function $\mathcal{H}[\{s\}]$,

$$
\mathcal{H}[\{s\}] = -\sum_{\langle i,j\rangle} J_{ij}\,\delta_{s_i,s_j}, \quad s_i = 1,\ldots,q, \tag{1}
$$
...
$$
\mathcal{H} = -\frac{M}{N_c}J_1\sum_{a=1}^{M}\sum_{i<j}\delta_{\sigma_i^a,\sigma_j^a} - \frac{1}{N_b}J_2\sum_{i<j}\delta_{\sigma_i^b,\sigma_j^b} - \frac{1}{N_c}\sqrt{\frac{M}{N_b}}\,J_3\sum_{a=1}^{M}\sum_{i,j}\delta_{\sigma_i^b,\sigma_j^a}, \tag{2}
$$
...
$$
\frac{\mathcal{H}}{N_c} = E = -\frac{J_1}{2M}\sum_{a,\alpha}m_{a\alpha}^2 - \frac{J_2 N_b}{2N_c}\sum_\alpha m_{b\alpha}^2 - \sqrt{\frac{N_b}{M}}\frac{J_3}{N_c}\sum_{a,\alpha}m_{b\alpha}\,m_{a\alpha}, \tag{3}
$$

while the entropy per spin, to leading order in $1/N_c$, $1/N_b$, is given by

$$
\mathcal{S} = -\frac{1}{M}\sum_{a,\alpha}m_{a\alpha}\ln m_{a\alpha} - \frac{N_b}{N_c}\sum_\alpha m_{b\alpha}\ln m_{b\alpha}. \tag{4}
$$
...
$$
\beta f = -\beta\frac{J_1}{2}\!\left(\frac{q-1}{q}s^2 + \frac{1}{q}\right) - \beta\frac{J_2 N_b}{2N_c}\!\left(\frac{q-1}{q}t^2 + \frac{1}{q}\right) - \beta\frac{\sqrt{MN_b}}{N_c}J_3\!\left(\frac{q-1}{q}st\hat{s} + \frac{1}{q}\right) - \mathcal{S}(s) - \frac{N_b}{N_c}\mathcal{S}(t) - \frac{M}{N_c}\mathcal{S}(\hat{s}), \tag{9}
$$

where the entropy function $\mathcal{S}$ is given by

$$
\mathcal{S}(s) = -\frac{1+(q-1)s}{q}\ln[1+(q-1)s] - \frac{(q-1)(1-s)}{q}\ln[1-s]. \tag{10}
$$
...
Expanding $\mathcal{S}(t)$ to second order in $t$ and $\mathcal{S}(\hat{s})$ to zeroth order in $\epsilon$, and using Eq. (22), we obtain from Eq. (21)

$$
-\frac{\beta}{2}\!\left(J_1 + \frac{M}{N_c}\frac{(\beta J_3)^2}{q-\beta J_2}\right)\frac{q-1}{q}s^2 - \mathcal{S}(s) - \frac{M}{N_c}\mathcal{S}(\hat{s}) = 0. \tag{24}
$$
...
$$
\widetilde{\mathcal{H}} = \mathcal{H} - \frac{M}{N_c^2}J_4\sum_{a<a'}^{M}\sum_{i,j}\delta_{\sigma_i^a,\sigma_j^{a'}}. \tag{33}
$$
...
(g) Apply a grouping procedure. If $\langle\delta_{s_i,s_j}\rangle \geq \theta$, points $\mathbf{x}_i$, $\mathbf{x}_j$ are defined as "friends." Then all mutual friends (including friends of friends, etc.) are assigned to the same cluster. We chose $\theta = 0.5$.
...
Possibilities for the dependence of the interaction strength $J_{ij}$ on the distance $d_{ij} = |\mathbf{x}_i - \mathbf{x}_j|$, other than Eq. (44), were also considered. For instance, a power law interaction (for neighbors only) such as
...
$$
\frac{(q-1)}{2q}\beta\frac{\sqrt{MN_b}}{N_c}J_3\hat{t}\hat{s}\,\epsilon + \frac{N_b}{N_c}\!\left[-\frac{\beta J_2}{2}\frac{(q-1)}{q}t^2 - \mathcal{S}(t)\right] - \beta\frac{\sqrt{MN_b}}{N_c}J_3\frac{(q-1)}{q}t\hat{s}(s^-+\epsilon) - \frac{M}{N_c}\mathcal{S}(\hat{s}) = 0. \tag{B2}
$$
...
Expanding $\mathcal{S}(t)$ to second order in $t$, neglecting terms proportional to $(\sqrt{MN_b}/N_c)\,J_3\,t\hat{s}\epsilon$, and using Eq. (17) we obtain from Eq. (B2)

$$
-\frac{q-1}{2q}\frac{M}{N_c}\frac{(\beta J_3 s^-)^2}{q-\beta J_2}\hat{s}^2 - \frac{M}{N_c}\mathcal{S}(\hat{s}) = 0. \tag{B4}
$$
