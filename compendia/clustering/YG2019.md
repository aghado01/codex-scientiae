...
We map stock market interactions to spin models to recover their hierarchical structure using a simulated annealing based Super-Paramagnetic Clustering (SPC) algorithm. This is directly compared to a modified implementation of a maximum likelihood approach we call fast Super-Paramagnetic Clustering (f-SPC). The methods are first applied to standard toy test-case problems, and then to a data-set of 447 stocks traded on the New York Stock Exchange (NYSE) over 1249 days. The signal to noise ratio of stock market correlation matrices is briefly considered. Our results recover clusters approximately representative of standard economic sectors and mixed ones whose dynamics shed light on the adaptive nature of financial markets and raise concerns relating to the effectiveness of industry based static financial market classification in the world of real-time data analytics. A key result is that we show that f-SPC maximum likelihood solutions converge to ones found within the Super-Paramagnetic Phase where the entropy is maximum, and those solutions are qualitatively better for high dimensionality data-sets.
...
- **B4** Relabel nodes. `nodel` $\gets$ `nodelp`.
...
for k = 0 to k = M do
 ...
 Compute, and store thermodynamic quantities i.e. m, c_ij end for
...
```
INPUT : Matrix of links set label counter to 0, Initialize nodel Nx1 array
Create nodelp, an empty array for i = 0 to i = N do if node i isn't linked at all then
 Set label counter to i's label
 Store i's label to nodelp
 Increase label counter by 1 else
 Find i's linked neighbors, and store their nodelp labels if None are labeled then
 Set label counter to i's label
 Store i's label to nodelp
 Increase label counter by 1 else store the labels of the linked neighbors
 Store root of the labels of the linked neighbors
 Set min the smallest root label
 Set the nodel of i to min
 In nodelp change linked neighbors root labels to min end if end if
 { Make nodelp sequential } for y = 0 to y = len(nodelp) do n = y while The root of n is less than n do
 Set n to the root of n end while
 Set the root of y to n end for
 { Relabel the labels with their roots } for i = 0 to i = len(nodelp) do
 Find labels in nodel == i
 Update them with their root in nodelp end for end for
...
Create Initial Population of N individuals for (In parallel) All individuals do
 Evaluate Fitness end for for Number of Generations G do
 Create Offspring (from the entire population)
 Mutate offspring for (In parallel) All offspring do
