...
The goal is to find a partition of a given data set into several compact groups. Each group indicates the presence of a distinct category in the measurements. The problem of partitional clustering can be formally stated as follows. Determine the partition of $N$ given patterns $\{\mathbf{v}_i\}_{i=1}^N$ into groups, called clusters, such that the patterns of a cluster are more similar to each other than to patterns in different clusters. It is assumed that either $d_{ij}$, the measure of dissimilarity between patterns $\mathbf{v}_i$ and $\mathbf{v}_j$, is provided or that each pattern $\mathbf{v}_i$ is represented by a point $\mathbf{x}_i$ in a $D$-dimensional metric space, in which case $d_{ij} = |\mathbf{x}_i - \mathbf{x}_j|$.
...
$$
P(S) = \frac{1}{Z} \exp\!\left(-\frac{\mathcal{H}(S)}{T}\right), \tag{2.3}
$$

plays the role of the probability density, which gives the statistical weight of each spin configuration $S = \{s_i\}_{i=1}^N$ in thermal equilibrium and $Z$ is a normalization constant, $Z = \sum_S \exp(-\mathcal{H}(S)/T)$.
...
When the spins are on a lattice and all nearest-neighbor couplings are equal, $J_{ij} = J$, the Potts system is homogeneous. Such a model exhibits two phases. At high temperatures the system is paramagnetic or disordered, $\langle m \rangle = 0$, indicating that $N_{\max}(S) \approx N/q$ for all statistically significant configurations. In this phase the correlation function $G_{ij}$ decays to $1/q$ when the distance between points $\mathbf{v}_i$ and $\mathbf{v}_j$ is large; this is the probability of finding two completely independent Potts spins in the same state. At very high temperatures even neighboring sites have $G_{ij} \approx 1/q$.

As the temperature is lowered, the system undergoes a sharp transition to an ordered, ferromagnetic phase; the magnetization jumps to $\langle m \rangle \neq 0$. This means that in the physically relevant configurations (at low temperatures), one Potts state dominates and $N_{\max}(S)$ exceeds $N/q$ by a macroscopic number of sites. At very low temperatures $\langle m \rangle \approx 1$ and $G_{ij} \approx 1$ for all pairs $\{\mathbf{v}_i, \mathbf{v}_j\}$.
...
$$
\frac{q-1}{q}\left(\frac{q-2}{q-1}\right)^2 + \frac{1}{q} \simeq 1 - \frac{2}{q} + \mathcal{O}\!\left(\frac{1}{q^2}\right),
$$
...
1. **Construct the physical analog Potts spin problem:**
   - (a) Associate a Potts spin variable $s_i = 1, 2, \ldots, q$ to each point $\mathbf{v}_i$.
   - (b) Identify the neighbors of each point $\mathbf{v}_i$ according to a selected criterion.
   - (c) Calculate the interaction $J_{ij}$ between neighboring points $\mathbf{v}_i$ and $\mathbf{v}_j$.
...
3. **In the superparamagnetic regime:**
   - (a) Measure the spin-spin correlation $G_{ij}$ for all neighboring points $\mathbf{v}_i$, $\mathbf{v}_j$.
...
3. Assign a frozen bond between nearest neighbor points $\mathbf{v}_i$ and $\mathbf{v}_j$ with probability $p_{i,j}^f$ (see equation 3.2).
...
We use the spin-spin correlation function $G_{ij}$, between neighboring sites $\mathbf{v}_i$ and $\mathbf{v}_j$, to build the data clusters. In principle we have to calculate the thermal average (see equation 3.1) of $\delta_{s_i, s_j}$ in order to obtain $G_{ij}$. However, the SW method provides an improved estimator (Niedermayer, 1990) of the spin-spin correlation function. One calculates the two-point connectedness $C_{ij}$, the probability that sites $\mathbf{v}_i$ and $\mathbf{v}_j$ belong to the same SW cluster, which is estimated by the average (see equation 3.1) of the following indicator function:

$$
c_{ij} = \begin{cases} 1 & \text{if } \mathbf{v}_i \text{ and } \mathbf{v}_j \text{ belong to the same SW cluster} \\ 0 & \text{otherwise} \end{cases}
$$

$C_{ij} = \langle c_{ij} \rangle$ is the probability of finding sites $\mathbf{v}_i$ and $\mathbf{v}_j$ in the same SW cluster. Then the relation (Fortuin & Kasteleyn, 1972)
...
1. Build the clusters' core using a thresholding procedure; if $G_{ij} > 0.5$, a link is set between the neighbor data points $\mathbf{v}_i$ and $\mathbf{v}_j$. The resulting connected graph depends weakly on the value (0.5) used in this thresholding, as long as it is bigger than $1/q$ and less than $1 - 2/q$. The reason is that the distribution of the correlations between two neighboring spins peaks strongly at these two values and is very small between them (see Figure 3b).
2. Capture points lying on the periphery of the clusters by linking each point $\mathbf{v}_i$ to its neighbor $\mathbf{v}_j$ of maximal correlation $G_{ij}$. It may happen, of course, that points $\mathbf{v}_i$ and $\mathbf{v}_j$ were already linked in the previous step.
...
$$
\mathcal{P}(\mathbf{x}) = \frac{\left(\sqrt{2\pi}\,\sigma\right)^{-D}}{2} \left[\exp\!\left(-\frac{\|\mathbf{x} - \mathbf{y}_1\|^2}{2\sigma^2}\right) + \exp\!\left(-\frac{\|\mathbf{x} - \mathbf{y}_2\|^2}{2\sigma^2}\right)\right], \tag{5.1}
$$

where $\mathbf{y}_1$ and $\mathbf{y}_2$ are the centers of the Gaussians and $\sigma$ determines their width. Since the two characteristic lengths involved are $\|\mathbf{y}_1 - \mathbf{y}_2\|$ and $\sigma$, the relevant parameter of this example is the normalized distance,

$$
L = \frac{\|\mathbf{y}_1 - \mathbf{y}_2\|}{\sigma}. \tag{5.2}
$$
...
Consider the following graph-based model whose basic entities are bond variables $n_{ij} = 0, 1$ residing on the edges $\langle i, j \rangle$ connecting neighboring sites $\mathbf{v}_i$ and $\mathbf{v}_j$. When $n_{ij} = 1$, the bond between sites $\mathbf{v}_i$ and $\mathbf{v}_j$ is occupied, and when $n_{ij} = 0$, the bond is vacant. Given a configuration $\mathcal{N} = \{n_{ij}\}$, random clusters are defined as the vertices of the connected components of the occupied bonds (where a vertex connected to vacant bonds only is considered a cluster containing a single point). The random cluster model is defined by the probability distribution

$$
W(\mathcal{N}) = \frac{q^{C(\mathcal{N})}}{Z} \prod_{\langle i,j \rangle} p_{ij}^{n_{ij}} (1 - p_{ij})^{(1 - n_{ij})}, \tag{A.1}
$$

where $C(\mathcal{N})$ is the number of clusters of the given bond configuration, the partition sum $Z$ is a normalization constant, and the parameters $p_{ij}$ fulfill $0 \leq p_{ij} \leq 1$.

The case $q = 1$ is the percolation model where the joint probability (see equation A.1) factorizes into a product of independent factors for each $n_{ij}$. Thus, the state of each bond is independent of the state of any other bond. By choosing $q > 1$ the weight of any bond configuration $\mathcal{N}$ is no longer the product of local independent factors. Instead, the weight of a configuration is also influenced by the spatial distribution of the occupied bonds, since configurations with more random clusters are given a higher weight.
