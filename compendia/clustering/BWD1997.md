...
$$
J_{ij} = \begin{cases} \frac{1}{\hat{K}} \exp\!\left(-\frac{d_{ij}^2}{2a^2}\right) & \text{if} v_i \text{and} v_j \text{are neighbors} \\ 0 & \text{otherwise} \end{cases} \tag{4.1}
$$
...
$$
c_{ij} = \begin{cases} 1 & \text{if} v_i \text{and} v_j \text{belong to the same SW cluster} \\ 0 & \text{otherwise} \end{cases}
$$
