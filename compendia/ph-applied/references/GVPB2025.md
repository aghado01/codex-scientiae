...
$$
\chi \colon X_{1} \longleftrightarrow X_{2} \longleftrightarrow \cdots \longleftrightarrow X_{n} ,
$$
...
$$
H_{p} ( \chi ) \colon H_{p} ( X_{1} ) \longleftrightarrow H_{p} ( X_{2} ) \longleftrightarrow \cdots \longleftrightarrow H_{p} ( X_{n} ) .
$$
...
$$
\mathcal{I}_{[ b , d ]} \colon I_{1} \longleftrightarrow I_{2} \longleftrightarrow \cdots \longleftrightarrow I_{n} ,
$$
...
$$
P e r_{p} ( \chi ) = \{[ b_{j} , d_{j} ] \colon j \in J \} \Longleftrightarrow H_{p} ( \chi ) \cong \bigoplus_{j \in J} \mathcal{I}_{[ b_{j} , d_{j} ]}
$$
...
For an interval $[b, d]$ in the zigzag persistence diagram of dimension $p$ of filtration 2, the mapping that enables a bijective transformation to a new interval $[\hat{b}, \hat{d}]$ only across model layers is defined as follows:
$$
\begin{aligned}
\hat{b} &= \begin{cases} b + 1 & \text{if $b$ is an intersection layer} \\ b & \text{otherwise} \end{cases}, \\
\hat{d} &= \begin{cases} d + 1 & \text{if $d$ is an intersection layer} \\ d & \text {otherwise} \end{cases}
\end{aligned}
$$
...
$$
\begin{cases}
\widehat{PI}_p(0, 0) &= PI_p(0, 0) \\
\widehat{PI}_p(b/2, d/2) &= PI_p(b, d) + PI_p(b-1, d) \\
&\quad + PI_p(b, d-1) + PI_p(b-1, d-1) \\
\widehat{PI}_p(b/2, \infty) &= PI_p(b, \infty) + PI_p(b-1, \infty).
\end{cases}
$$

$^{13}$ In our setting we say a $p$-dimensional hole “dies” when the corresponding homology class no longer persists in subsequent layers. In the zigzag filtration, this happens when the hole is no longer represented by an independent equivalence class in the homology group. $^{14}$ An alternative method for constructing these maps and obtaining the zigzag persistence diagram is to use a filtration where, instead of intersections, the union of the complexes from two consecutive layers is considered. However, the Diamond Lemma, as discussed in [32], guarantees that both the intersection and union-based filtrations encode the same homological information. $^{15}$ By construction, all resulting intervals contain even numbers, as the model layers are indexed with these numbers.
