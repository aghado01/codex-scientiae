...
# Bayesian curve-ﬁtting with free-knot splines

# B  ILARIA D  MATTEO , CHRISTOPHER R . GENOVESE  ROBERT E . KASS
...
![The image consists of a graph with three different types of data represented by different colors. The graph has a title at the top that reads Example 1. The x-axis is labeled as x and the y-axis is labeled as y. The graph shows a pattern of data points that are scattered around the x-axis. The data points are scattered in a random pattern, with some points being closer to each other and others being farther apart. The data points are scattered in a random pattern, with some points being closer to each other and others being farther apart.](<images/imageFile1.png>)
...
![In this image we can see a graph.](<images/imageFile2.png>)
...
![The image is a graph titled Experiment 1. The graph is a line graph with two axes: one labeled Experiment 1 and the other labeled Experiment 2. The x-axis is labeled Time (sec) and the y-axis is labeled Time (sec). The graph shows the time-varying behavior of two different sets of data points. The first set of data points is labeled Experiment 1 and the second set of data points is labeled Experiment 2. The x-axis is labeled Time (sec) and the y-axis is labeled Time (sec). The graph shows the time-varying behavior of two different sets of data points. The first set of data points is labeled Experiment 1 and the second set of data points is labeled Experiment 2. The x-axis is labeled Time (sec) and the y-axis is labeled Time (sec). The graph](<images/imageFile3.png>)
...
![The image is a graph depicting the time series of data over three different time intervals. The graph is titled Time Series and is labeled as a and b. The x-axis represents the time, ranging from 0 to 800 seconds, while the y-axis represents the time series, ranging from 0 to 600 seconds. The data points in the graph are represented by a series of horizontal lines. The lines are labeled with the following values: - Time 0: 0 - Time 1: 400 - Time 2: 600 - Time 3: 800 The data points are scattered across the graph, with some points closer to the bottom and others closer to the top. The data points are not perfectly aligned, indicating that there may be some outliers or anomalies in the data. The graph is labeled as follows: - Time 0](<images/imageFile4.png>)
...
![The image is a line graph that shows the time (sec) at which the number of people in a certain area increases. The x-axis represents the time in seconds, ranging from 0 to 600 seconds. The y-axis represents the number of people in the area, ranging from 0 to 600. The graph shows a general trend of increasing numbers of people in the area over time. The graph has a scale of range 0 to 600 on the x-axis, labeled time (sec). The graph also has a scale of range 0 to 600 on the y-axis, labeled people (people). The graph shows a general trend of increasing numbers of people in the area over time. The graph has a few key features: 1. **Title**: The title at the top of the graph is Time (sec) at which the number of people in a certain area increases.](<images/imageFile5.png>)
...
![The image is a line graph titled Bayesian curve-fitting. The graph is titled Condition 1 and is labeled as such. The graph shows a line graph with a linear scale of range 0 to 250 on the y-axis, labeled Condition 1. The x-axis is labeled Time (msec). The graph shows a trend of decreasing values of the Condition 1 as the time increases. The line graph shows a sharp decrease in the Condition 1 as the time increases. The graph also shows a sharp increase in the Condition 1 as the time increases. The graph also shows a linear trend in the Condition 1 as the time increases. The graph shows a sharp increase in the Condition 1 as the time increases. The graph also shows a linear trend in the Condition 1 as the time increases. The graph shows a sharp increase in the](<images/imageFile6.png>)
...
|Di ﬀ erence|131·8 (4·4)|181·8 (20·4)|
...
- D , D . G . T ., M , B . K . & S , A . F . M . (1998). Automatic Bayesian curve ﬁtting. J. R. Statist. Soc. B 60 , 330–50.
- E , P . H . C . & M , B . D . (1996). Flexible smoothing with B-splines and penalties (with Discussion). Statist. Sci. 11 , 89–121.
...
- G , P . J . (1995). Reversible jump Markov chain Monte Carlo computation and Bayesian model determination. Biometrika 82 , 711–32.
- H , E . F . (1973). Bayesian spline regression when the number of knots is unknown. J. R. Statist. Soc. B 35 , 347–60.
...
- S , M . & K , R . (1996). Nonparametric regression using Bayesian variable selection. J. Economet. 75 , 317–43.
- T , L . (1994). Markov chains for exploring posterior distributions (with Discussion). Ann. Statist. 22 , 1701–62.


- V , V ., C , R ., K , R . E ., G , S . & O , C . R . (2001). Statistical analysis of temporal evolution in single-neuron ﬁring rates. Biostatistics 2 . To appear.
- W , G . (1990). Spline Models for Observational Data. Philadelphia: Society for Industrial and Applied Mathematics.
