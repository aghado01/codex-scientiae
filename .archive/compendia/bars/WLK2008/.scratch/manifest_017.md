...
## REPAIR_PROSE
- RAW: `error , a boolean indicator of failure`
  FIX: `\( \text{error} \), a boolean indicator of failure`
- RAW: `β , the random variate. Only defined if not error .`
  FIX: `\( \beta \), the random variate. Only defined if not \( \text{error} \).`
- RAW: `- β curr ← β ̂`
  FIX: `- \( \beta_{curr} \leftarrow \hat{\beta} \)`
- RAW: `- for i ← 0 to MHI 1`
  FIX: `- for \( i \leftarrow 0 \) to \( MHI - 1 \)`
- RAW: `comment: Accept the candidate β`
  FIX: `comment: Accept the candidate \( \beta \)`
- RAW: `- beta ← β curr .`
  FIX: `- \( \beta \leftarrow \beta_{curr} \).`
