...
In addition, by Theorem 5.12(b) , H d ( N 2 ,N 0 ) ∼ = im i d ∗ ⊕ H d ( N 2 ,N 0 ) ker j d ∗ . Using that H d ( N 2 ,N 0 ) ker j d ∗ ∼ = im j d ∗ , we get
...
which provides a zigzag module by considering the homology groups H ∗ ( K t ) for each complex K t and linear maps ψ ∗ t : H ∗ ( K t ) ↔ H ∗ ( K t +1 ) between the homology groups of consecutive complexes induced by inclusions:
