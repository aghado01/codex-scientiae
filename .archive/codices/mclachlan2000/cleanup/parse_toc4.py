...
def is_separator(line):
    s=line.strip()
    return not s or s.lower() in skip_words or re.match(r'^\|[-\s\|]+$', s)
...
out=['# Contents']
for item in items:
    if item['text'].lower() in skip_words:
        continue
    if item['text'].isdigit() and item['page'] is None:
        continue
    prefix='  ' * item['level'] + '- '
    ...
    if item['page']:
        line += ' — ' + item['page']
    out.append(line)
Path('formatted_toc5.md').write_text('\n'.join(out) + '\n', encoding='utf-8')
print('wrote formatted_toc5.md')
