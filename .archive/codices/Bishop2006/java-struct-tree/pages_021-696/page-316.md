...
k(x,x�) = ck1(x,x�) (6.13) k(x,x�) = f(x)k1(x,x�)f(x�) (6.14) k(x,x�) = q (k1(x,x�)) (6.15) k(x,x�) = exp(k1(x,x�)) (6.16) k(x,x�) = k1(x,x�) + k2(x,x�) (6.17) k(x,x�) = k1(x,x�)k2(x,x�) (6.18) k(x,x�) = k3 (φ(x),φ(x�)) (6.19) k(x,x�) = xTAx� (6.20) k(x,x�) = ka(xa,x�

b) (6.21) k(x,x�) = ka(xa,x�
...
contains only terms of degree two. If we consider the slightly generalized kernel k(x,x�) =

�

�2

with c > 0, then the corresponding feature mapping φ(x) contains constant and linear terms as well as terms of order two. Similarly, k(x,x�) =

xTx� + c

contains all monomials of order M. For instance, if x and x� are two images, then�M the kernel represents a particular weighted sum of all possible products of M pixels in the ﬁrst image with M pixels in the second image. This can similarly be generalized to include all terms up to degree M by considering k(x,x�) =

�

xTx�

xTx� + c with c > 0. Using the results (6.17) and (6.18) for combining kernels we see that�M these will all be valid kernel functions.
