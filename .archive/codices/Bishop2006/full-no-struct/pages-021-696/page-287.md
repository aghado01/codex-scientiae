...
We can further simplify this regularization term as follows. In Section 1.5.5 we saw that the function that minimizes the sum-of-squares error is given by the conditional average E [ t | x ] of the target values t . From (5.131) we see that the regularized error will equal the unregularized sum-of-squares plus terms which are O ( ξ ) , and so the network function that minimizes the total error will have the form

$$
y ( x ) = \mathbb { E } [ t | x ] + O ( \xi ) .
$$

Thus, to leading order in ξ , the ﬁrst term in the regularizer vanishes and we are left with 1

$$
\text {order in $\zeta$, the inst mem in the regularizers and we are left} \\ \Omega = \frac { 1 } { 2 } \int \left ( \tau ^ { T } \nabla y ( x ) \right ) ^ { 2 } p ( x ) \, d x \\ \text {client to the tangent propagation regularizer} \left ( 5 . 1 8 \right ) . \\ \text {der the singular possaion in which the transformation of the inputs simply}
$$

which is equivalent to the tangent propagation regularizer (5.128).

If we consider the special case in which the transformation of the inputs simply consists of the addition of random noise, so that x → x + ξ , then the regularizer takes the form 1

$$
\Omega = \frac { 1 } { 2 } \int \| \nabla y ( x ) \| ^ { 2 } \, p ( x ) \, d x \\ \text {as Tikhonov regularization} \, ( Tikhonov and Arsenin, 1977; Bishop, 1991)
$$
