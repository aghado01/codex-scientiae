...
To prevent parsing collisions with standard text (e.g., currency symbols) across diverse LLM architectures, you must use standard LaTeX bracket delimiters instead of dollar signs.
- **Inline Math**: Must be wrapped in `\( ... \)` (e.g., `\( x \)`, `\( J_{ij} \)`).
- **Block Math**: Must be fenced with `\[ ... \]` on their own lines.
- **Pure Syntax Only**: Do NOT use web-renderer specific injection macros like KaTeX's `\color`, custom spacing tweaks (`\vspace`), or layout stylings. LLMs natively understand pure LaTeX (as trained on ArXiv/Jupyter data); these pure structures maximize token efficiency.
