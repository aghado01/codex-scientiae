...
| `src/procurement/operations/materialization.py`, `storage/source_deposits.py`, and `source/extraction.py` | Use exact PID/process-serial private tree scratch beneath a pinned document generation, with lease-owned no-follow sweeping and immutable-component recovery. | Treat as the procurement source-publication reference; retain generation-pinned cleanup and article-last recovery semantics. |
...
| `src/procurement/source/extraction.py`, `storage/source_deposits.py`, and `jsonl_engine/publication.py` | Reuse the JSONL adjacent temp allocator, pinned roots, guarded recursive cleanup, stable copy, and exclusive directory publication. | Preserve as reuse exemplars; avoid procurement-local naming or publication utilities. |
