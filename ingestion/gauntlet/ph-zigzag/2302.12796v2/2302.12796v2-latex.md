...
in which each $G_i\leftrightarrow G_{i+1}$ is either a forward inclusion $G_i\hookrightarrow G_{i+1}$ or a backward inclusion $G_i\hookleftarrow G_{i+1}$. For computation, we only consider *simplex-wise* filtrations starting and ending with *empty* graphs in this paper, i.e., $G_0=G_m=\varnothing$ and each inclusion $G_i\leftrightarrow G_{i+1}$ is an addition or deletion of a single vertex or edge (both called a *simplex*). Such an inclusion is sometimes denoted as $G_i\xleftrightarrow{\;\sigma\;} G_{i+1}$ with $\sigma$ indicating the vertex or edge being added or deleted. The $p$-th homology functor ($p=0,1$) applied on $\mathcal{F}$ induces a *zigzag module*:
...
$$
![diagram 1 (tikzpicture)](2302.12796v2/diagram-1.png)
$$
...
![figure: v-v-sw](2302.12796v2/v-v-sw.png)

\captionof{figure}{}

}

- **A. The switch is a vertex-vertex switch:** First let $v_1:=\sigma$ and $v_2:=\tau$. As illustrated in  Figure 3, the only situation where the  pairing $\Pi$ changes in this case is that  $v_1,v_2$ are in the same tree in $\mathbb{T}$ and are both unpaired when $e$ is added in $\mathcal{F}$, where $e$ is the edge corresponding to the *nearest common ancestor* of $v_1,v_2$ in $\mathbb{T}$. In this case, $v_1$,$v_2$ are leaves at the *lowest* levels in the subtrees $T_1,T_2$ respectively (see Figure 3), so that $v_2$ is paired with $e$ and $v_1$ is the *representative* (the only unpaired vertex) in the merged connected component due to the addition of $e$. After the switch, $v_1$ is paired with $e$ due to being younger and $v_2$ becomes the representative of the merged component. Notice that the structure of $\mathbb{T}$  stays the same.
...
![figure: e1-e2-same-conns](2302.12796v2/e1-e2-same-conns.png)
...
![figure: e-e-sw-a](2302.12796v2/e-e-sw-a.png)

} \subfloat[]{

![figure: e-e-sw-b](2302.12796v2/e-e-sw-b.png)

} \subfloat[]{

![figure: e-e-sw-c](2302.12796v2/e-e-sw-c.png)

}

*(a) The relevant parts of $\mathbb{T}$ when switching two negative edges in Algorithm 1 for which the structure of $\mathbb{T}$ changes. (b) The changed structure of $\mathbb{T}$ after the switch corresponding to the  connecting configuration in Figure 3. (c) The changed structure of $\mathbb{T}$ after the switch corresponding to the  connecting configuration in Figure 3.*
...
![figure: C1-C2-C3-1](2302.12796v2/C1-C2-C3-1.png)

}

\subfloat[]{

![figure: C1-C2-C3-2](2302.12796v2/C1-C2-C3-2.png)
...
We have that $C_1,C_2,C_3$ are connected by $e_1,e_2$ in $G_{i+1}$ in the two different ways illustrated in Figure 3. For the two different connecting configurations, the structure of $\mathbb{T}$ after the switch is different, which is  shown in Figure 3b and 3c. Furthermore, if $\mathrm{idx}_\mathcal{F}(w)<\mathrm{idx}_\mathcal{F}(u)$ and $e_2$ directly connects $C_1,C_3$ as in Figure 3, then we swap the paired vertices of $e_1,e_2$ in $\Pi$. (See Section 3.1 for further details and  justifications.)
...
Let $N$ be the number of nodes. All above operations in DFT-Tree take $O(\log N)$ time. Among the operations, $\text{root}$ is used to determine whether two nodes in $\mathbb{T}$ are from the same tree; $\text{cut}$ and $\text{link}$ are used to make the structural changes as in Figure 3; $\text{change-val}$ is used to record (and update during switches) the levels of leaves; $\text{subtree-min}$ is used to return the leaf at the lowest level in a subtree.
...
**Determining different connecting configurations.** We also use the MSF of $G$ (represented by Link-Cut trees) to determine the different connecting configurations in Figure 3 for the relevant case in Algorithm 1. Specifically, if $u,w$  in Figure 3 are already connected in $G'_i$, then the configuration in Figure 3 applies; otherwise, the the configuration in Figure 3 applies.
...
- If $e_2$ directly connects $C_2,C_3$ as in Figure 3, then let the roots of $T_2,T_3$ be the children of $e_2$, and let $e_2$ and the root of $T_1$ be the children of $e_1$ in $\mathbb{T}$ (see Figure 3b).

- If $e_2$ directly connects $C_1,C_3$ as in Figure 3, then let the roots of $T_1,T_3$ be the children of $e_2$, and let $e_2$ and the root of $T_2$ be the children of $e_1$ in $\mathbb{T}$ (see Figure 3c). Moreover, if $\mathrm{idx}_\mathcal{F}(w)<\mathrm{idx}_\mathcal{F}(u)$, then  swap the paired vertices of $e_1,e_2$ in $\Pi$.
...
![figure: v-e-sw](2302.12796v2/v-e-sw.png)
...
$$
![diagram 2 (tikzpicture)](2302.12796v2/diagram-2.png)
$$
...
\mathcal{F}:
\varnothing=
G_0\xleftrightarrow{\;\sigma_{0}\;} G_1\xleftrightarrow{\;\sigma_{1}\;}
\cdots
\xleftrightarrow{\;\sigma_{m-1}\;} G_m
=\varnothing
$$
...
$$
{\mathcal U}:\varnothing=\hat{G}_0\xhookrightarrow{\;\hat{\sigma}_0\;}
...
\cdots
\xhookleftarrow{\;\hat{\sigma}_{m-1}\;} \hat{G}_{m}=\varnothing.
$$
...
| $\mathsf{Pers}_1^{\text{cc}}({\mathcal U})$ | $\leftrightarrow$ | $\mathsf{Pers}_0^{\text{oo}}(\mathcal{F})\cup\mathsf{Pers}_1^{\text{cc}}(\mathcal{F})$ |
...
\begin{gathered}
\begin{aligned}
&{\mathcal U}_u:\varnothing=\hat{G}_0\xhookrightarrow{\;\hat{\sigma}_0\;}
\hat{G}_1\xhookrightarrow{\;\hat{\sigma}_{1}\;}
\cdots\xhookrightarrow{\;\hat{\sigma}_{k-1}\;}
\hat{G}_{k},\\
&{\mathcal U}_d:\varnothing= \hat{G}_{m}\xhookrightarrow{\;\hat{\sigma}_{m-1}\;}
\hat{G}_{m-1}
...
$$
![diagram 3 (tikzpicture)](2302.12796v2/diagram-3.png)
$$

- \textit{{Backward}} switch is the symmetric version of forward switch, requiring $\tau\not\subseteq\sigma$:

$$
![diagram 4 (tikzpicture)](2302.12796v2/diagram-4.png)
$$

- The remaining switches swap two inclusions of opposite directions:

$$
![diagram 5 (tikzpicture)](2302.12796v2/diagram-5.png)
$$
...
$$
![diagram 6 (tikzpicture)](2302.12796v2/diagram-6.png)
$$
...
$$
\mathcal{L}:\varnothing=H_0\xhookrightarrow{\;\varsigma_0\;}
...
\cdots
\xhookleftarrow{\;\varsigma_{2\ell-1}\;} H_{2\ell}=\varnothing,
$$
...
For Case D of Algorithm 4 when the deletion of $\epsilon'$ is after the deletion of $\epsilon$ in ${\mathcal U}$, if there is a cycle containing $e_2$ and $\epsilon'$ in $\hat{G}'_j\cap\hat{G}_h$, then the pairing stays the same before and after the switch; otherwise, the pairing changes.

**Remark.** Since we are focusing on the 1-cycles in graphs, for the graph intersection $\hat{G}'_j\cap\hat{G}_h$, we only need to take the intersection of their edge sets. For convenience, we can assume $\hat{G}'_j\cap\hat{G}_h$ to contain all vertices in $\hat{G}$.

To see the correctness of the observation, first suppose that there is a cycle $z''$ containing $e_2$ and $\epsilon'$ in $\hat{G}'_j\cap\hat{G}_h$. Notice that $z''$ is nothing but a representative for the pair $(e_2,\epsilon')$ in $\mathcal{U}'$. We also have that $z$ is still a representative for  the pair $(e_1,\epsilon)$ in $\mathcal{U}'$. So the paring stays the same before and after the switch based on Proposition 13. If there is not a cycle containing $e_2$ and $\epsilon'$ in $\hat{G}'_j\cap\hat{G}_h$, then there is no representative cycle for the pair $(e_2,\epsilon')$ in $\mathcal{U}'$. So $e_2$ cannot be paired with $\epsilon'$ in $\mathcal{U}'$.

Based on Observation 16, in order to obtain the pairing in Case D, we only need to check whether there is a cycle containing both $e_2$ and $\epsilon'$ in $\hat{G}'_j\cap\hat{G}_h$. This is equivalent to independently checking (i) whether there is a cycle $c_1$ in $\hat{G}'_j\cap\hat{G}_h$ containing $e_2$ and (ii) whether there is  a cycle $c_2$ in $\hat{G}'_j\cap\hat{G}_h$ containing $\epsilon'$. The reason is that, if $\epsilon'\in c_1$ or $e_2\in c_2$, then either $c_1$ or $c_2$ would be a cycle in $\hat{G}'_j\cap\hat{G}_h$ containing both edges; otherwise, $c_1+c_2$ is a cycle in $\hat{G}'_j\cap\hat{G}_h$ containing both edges. We then reduce the checking of (i) and (ii) into a graph connectivity problem, e.g., checking (ii) is equivalent to checking whether the two vertices of $\epsilon'$ is connected in $\hat{G}'_{j}\cap\hat{G}_{h+1}$. We conclude:

**Observation 17.** To determine the pairing in Case D of Algorithm 4, one only needs to perform two graph connectivity queries. Each query asks whether two vertices $u,v$ are connected in $H\cap H'$, where $H$ is a graph in the ascending part of $\mathcal{U}'$ and $H'$ is a graph in the descending part of $\mathcal{U}'$.

We now describe how to address the connectivity query as in Observation 17 for $\mathcal{U}$ (the query for $\mathcal{U}'$ can be done similarly by first changing the data structures we maintain for $\mathcal{U}$ into ones for $\mathcal{U}'$). One way to solve such a connectivity problem for $\mathcal{U}$ is to maintain an MSF for each $\hat{G}_\lambda$ in the descending part $\mathcal{U}_d$ of $\mathcal{U}$ ($\lambda\geq k$), where edges in  $\hat{G}_\lambda$ are weighted by their indices in the ascending part $\mathcal{U}_u$. Then for a $\hat{G}_\gamma$ in $\mathcal{U}_u$ ($\gamma<k$), determining whether two vertices $u,v$ are connected in $\hat{G}_\gamma\cap\hat{G}_\lambda$ can be done by querying the bottleneck weight of the path connecting $u,v$ in the MSF of $\hat{G}_\lambda$. Representing the MSF's as Link-Cut trees [20], such a query can be done in $O(\log m)$ time.
...
1. Nicolas Berkouk and Luca Nyckees. One diamond to rule them all: Old and new topics about zigzag, levelsets and extended persistence, 2022. URL: \url<https://arxiv.org/abs/2210.00916>, \href <http://dx.doi.org/10.48550/ARXIV.2210.00916> {\pathdoi:10.48550/ARXIV.2210.00916}.
...
22. Simon Zhang, Soham Mukherjee, and Tamal K. Dey. GEFL: Extended filtration learning for graph classification. In Bastian Rieck and Razvan Pascanu, editors, *Learning on Graphs Conference, LoG 2022*, volume 198 of *Proceedings of Machine Learning Research*, page 16. PMLR, 2022. URL: \url<https://proceedings.mlr.press/v198/zhang22b.html>.
