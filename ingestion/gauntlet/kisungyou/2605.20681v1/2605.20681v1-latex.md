...
Principal component analysis (PCA) is a basic tool for summarizing high-dimensional data by a low-dimensional mean--subspace representation.  Since the classical formulations of [?] and [?], PCA has been used both as a dimension-reduction method and as an inferential summary of dominant variation. Its fixed-dimensional asymptotics and eigenspace perturbation behavior are well understood [?, ?, ?, ?].  These properties make PCA a natural target for distributed computation: each node can compute a local PCA summary, and only the local summaries need to be transmitted.

A local PCA summary, however, is not a single Euclidean object.  It consists of a mean vector and a principal subspace.  The mean lies in $\mathbb R^p$, whereas the leading $r$-dimensional subspace lies on the Grassmann manifold $\operatorname{Gr}(r,p)$ [?].  We use the Grassmann formulation because PCA identifies a subspace, not a particular signed, ordered, or oriented orthonormal basis.  A Stiefel representation would introduce artificial orientation conventions and becomes unstable when eigenvalues inside the leading block are tied or nearly tied.
...
$$
\widehat\theta_k=(\widehat\mu_k,\widehat\mathcal U_k)
    \in \mathbb R^p\times\operatorname{Gr}(r,p).
$$

Median-of-means aggregation is a natural way to combine robustness and scalability [?, ?, ?, ?, ?].  In the present problem, however, the median must respect the product geometry of a Euclidean mean and a Grassmann subspace.  This leads to the scale-calibrated product MoM estimator studied below.

Once PCA is viewed this way, a scale issue becomes unavoidable.  Let $B_1,\ldots,B_K$ be nodes of size $b$, with $n=Kb$, and let $\widehat\theta_k=(\widehat\mu_k,\widehat\mathcal U_k)$ be the local PCA estimate.  We study the scaled geometric MoM estimator

$$
\widetilde\theta_{n,\alpha}
    =\operatorname*{arg\,min}_{(\mu,\mathcal U)\in\mathbb R^p\times\operatorname{Gr}(r,p)}
    {1\over K}\sum_{k=1}^K
    \left\{
    \alpha\|\mu-\widehat\mu_k\|^2
    +(2-\alpha)d_{\operatorname{Gr}}(\mathcal U,\widehat\mathcal U_k)^2
    \right\}^{1/2},
$$

where $d_{\operatorname{Gr}}$ is the canonical Grassmann distance and $\alpha\in(0,2)$.  The scale $\alpha$ balances Euclidean mean error against subspace error, which determines how much a node must disagree in its mean or eigenspace before it is treated as unreliable by the final aggregation.

This paper studies how $\alpha$ should be chosen and how it changes inference in robust distributed PCA.  The question is related to, but distinct from, ordinary scale selection for product-manifold medians.  For ordinary product medians, changing factor weights can change the population center because the objective couples factors through a common radial distance; fixed-scale existence, robustness, limit theory, bootstrap validity, and algorithms are available from [?].  Here, the aggregated objects are node estimators, so scale acts on node-level errors.  In centered MoM regimes, the target may remain $(\mu_0,\mathcal U_0)$, but $\alpha$ changes covariance, finite-block bias, concentration, confidence regions, and bad-node influence.  Scale calibration is therefore an inferential design problem: small eigengaps call for downweighting noisy Grassmann errors, whereas unstable mean estimation calls for downweighting Euclidean errors. Our contributions are as follows.

- We formulate robust distributed PCA as MoM estimation on $\mathbb R^p\times\operatorname{Gr}(r,p)$ and derive the joint influence expansion of $(\widehat\mu_k,\widehat{\mathcal U}_k)$.  The mean influence is $X-\mu_0$ and the Grassmann influence is an eigengap-weighted covariance perturbation.

- We prove that the product-manifold MoM estimator is asymptotically equivalent to a scaled spatial median of node influence errors.  This yields both fixed-$K$ non-Gaussian and growing-$K$ Gaussian limits with finite-block bias.

- We derive the scale-dependent covariance of the centered aggregate.  If $H_\alpha$ is the block scale matrix and $W$ is the limiting node influence error, then
...
where $A_\alpha$ and $S_\alpha$ are the derivative and score covariance of the spatial median of $H_\alpha^{1/2}W$.  This leads to robust block-scale and inference-optimal calibration rules.

- We establish high-probability MoM bounds, factorwise mean--subspace deviation tradeoffs, bad-node robustness, scale-dependent influence constants $\alpha^{-1/2}$ and $(2-\alpha)^{-1/2}$, and node-bootstrap validity.

The rest of the paper is organized as follows.  Section 2 introduces the mean--subspace PCA parameter space $\mathbb R^p\times\operatorname{Gr}(r,p)$, defines the scaled product metric, and states the standing assumptions.  Section 3 derives the node-level PCA influence expansion and proves the local reduction from product-manifold MoM aggregation to a scaled spatial median of node errors.  Section 4 develops the two asymptotic regimes: a fixed-$K$ non-Gaussian limit and a growing-$K$ Gaussian limit with finite-block bias.  Section 5 studies scale-dependent inference, including the covariance formula, robust block-scale calibration, inference-optimal scale selection, and the effect of estimating $\alpha$.  Section 6 gives finite-sample concentration, bad-node robustness, factorwise influence bounds, and node bootstrap validity.  Section 7 presents simulations and a large-scale single-cell RNA-seq analysis.  Section 8 concludes with implications and open directions.  Additional technical results, all proofs, and supplementary numerical experiments are provided in the \nameref{app:main}.

\section{Robust distributed PCA on \texorpdfstring{$\mathbb R^p\times\operatorname{Gr}(r,p)$}{R^p x Gr(r,p)}}
...
Let $X_1,\ldots,X_n\in\mathbb R^p$ be independent copies of a random vector $X$.  Write

$$
\mu_0=\mathbb E [X],
    \qquad
    \Sigma=\mathbb E[(X-\mu_0)(X-\mu_0)^T].
$$
...
$$
\mathcal U_0=\operatorname{span}(u_1,\ldots,u_r)\in\operatorname{Gr}(r,p).
$$

The parameter of interest is

$$
\theta_0=(\mu_0,\mathcal U_0)\in\mathcal M_p:=\mathbb R^p\times\operatorname{Gr}(r,p).
$$

The Grassmann distance is defined by principal angles.  If $\vartheta_1,\ldots,\vartheta_r\in[0,\pi/2]$ are the principal angles between $\mathcal U$ and $\mathcal V$, then

$$
d_{\operatorname{Gr}}(\mathcal U,\mathcal V)^2=\sum_{j=1}^r \vartheta_j^2.
$$

This distance is invariant under the choice of orthonormal bases.  After choosing an eigenbasis $U_r=(u_1,\ldots,u_r)$ and an orthogonal complement $U_\perp=(u_{r+1},\ldots,u_p)$, the tangent space $T_{\mathcal U_0}\operatorname{Gr}(r,p)$ is identified with matrices in $\mathbb R^{(p-r)\times r}$, equipped with the Frobenius norm.

For $\alpha\in(0,2)$, define the scaled product distance

$$
d_\alpha\{(\mu,\mathcal U),(\mu',\mathcal V)\}^2
    =\alpha\|\mu-\mu'\|^2+(2-\alpha)d_{\operatorname{Gr}}(\mathcal U,\mathcal V)^2.
$$

We will often restrict $\alpha$ to a compact interval

$$
I_\varepsilon=[\varepsilon,2-\varepsilon],\qquad 0<\varepsilon<1.
$$
...
and let $\widehat\mathcal U_k\in\operatorname{Gr}(r,p)$ be the leading $r$-dimensional eigenspace of $\widehat\Sigma_k$.  The local estimator is $\widehat\theta_k=(\widehat\mu_k,\widehat\mathcal U_k)$, and the scale-$\alpha$ geometric MoM estimator is

$$
\widetilde\theta_{n,\alpha}
    =\operatorname*{arg\,min}_{\theta\in\mathcal M_p} Q_{K,\alpha}(\theta),
    \qquad
    Q_{K,\alpha}(\theta)= {1\over K}\sum_{k=1}^K d_\alpha(\theta,\widehat\theta_k).
$$

For fixed $\alpha$, this is a geometric median of points on a product manifold.  The general product-manifold theory therefore supplies the fixed-scale existence, local uniqueness, robustness, central limit, bootstrap, and computational foundations [?].  This paper does not revisit those fixed-scale product-median facts.  Instead, it studies how the scale $\alpha$ interacts with the node-level PCA error distribution.

### Tangent notation and assumptions

Let $d=p+r(p-r)$ be the dimension of the product tangent space.  In tangent coordinates at $\theta_0=(\mu_0,\mathcal U_0)$, define
...
For $w=(w_\mu,w_\mathcal U)\in\mathbb R^p\oplus T_{\mathcal U_0}\operatorname{Gr}(r,p)$, write

$$
\|w\|_{H_\alpha}^2=w^T H_\alpha w
    =\alpha\|w_\mu\|^2+(2-\alpha)\|w_\mathcal U\|^2.
$$
...
[label=(A\arabic*), ref=(A\arabic*), leftmargin=2.7em]

- *Moments and eigengap.* The observations satisfy

$$
\mathbb E\|X-\mu_0\|^{4+\delta}<\infty
$$

for some $\delta>0$, and the population covariance matrix satisfies the eigengap condition (3).  The eigenvalues $\lambda_j$ may have multiplicities within the leading block and within the trailing block. Only the separation between the leading $r$-dimensional eigenspace and its orthogonal complement is required.

- *Node localization.* There is a normal neighborhood $\mathcal N$ of $\mathcal U_0$ in $\operatorname{Gr}(r,p)$ such that $\max_{1\le k\le K}\mathbf 1\{\widehat\mathcal U_k\notin\mathcal N\}\to0$ in probability.  For fixed $K$, this follows from $\widehat\mathcal U_k\to\mathcal U_0$ in probability.  For growing $K$, it is enough to assume

$$
K\,\mathbb P(\widehat\mathcal U_1\notin\mathcal N)\to0.
$$

The same neighborhood is used for all $\alpha\in I_\varepsilon$.

- *Spatial median regularity.* Let

$$
W_{k,b}=\sqrt b\begin{pmatrix}
        \widehat\mu_k-\mu_0\\
        \log_{\mathcal U_0}(\widehat\mathcal U_k)
    \end{pmatrix}
$$

be the node error in product tangent coordinates, and let $W_b$ denote a generic copy.  For each $\alpha\in I_\varepsilon$, define

$$
s_{\alpha,b}=\operatorname*{arg\,min}_{s\in\mathbb R^d}\mathbb E\|s-W_b\|_{H_\alpha}.
$$
...
we assume $\mathbb P(R_{\alpha,b}=0)=0$, $\mathbb E R_{\alpha,b}^{-1}<\infty$, and

$$
A_{\alpha,b}=\mathbb E\left[{I-U_{\alpha,b}U_{\alpha,b}^T\over R_{\alpha,b}}\right]
$$

is nonsingular uniformly over $\alpha\in I_\varepsilon$.

- *Finite-block bias expansion.* For refined growing-$K$  inference, the finite-block median satisfies

$$
s_{\alpha,b}=b^{-1/2}a_\alpha+o(b^{-1/2})
$$

uniformly over $\alpha\in I_\varepsilon$, for a continuous function $\alpha\mapsto a_\alpha$.

The finite-block bias $s_{\alpha,b}$ is the population spatial median of the finite-sample node error distribution.  It vanishes under central symmetry, but otherwise can be of order $b^{-1/2}$.  Hence it may survive root-$n$ scaling unless $K/b\to0$.
...
Choose the eigenbasis $u_1,\ldots,u_p$ used in the decomposition of $\Sigma$.  For $a=1, \ldots,p-r$ and $j=1,\ldots,r$, define
...
**Theorem 1 (Node PCA influence expansion).** Suppose Assumption 1 holds.  For a node $B_k$, define $W_{k,b}$ as in (11).  Then

$$
W_{k,b}
    ={1\over\sqrt b}\sum_{i\in B_k}\zeta(X_i)+r_{k,b},
    \qquad
    r_{k,b}\stackrel{p}{\to}0,
$$
...
\zeta(X)=\begin{pmatrix}
        X-\mu_0\\
        \operatorname{vec}\{B(X)\}
    \end{pmatrix}\in\mathbb R^{p+r(p-r)}.
$$

Consequently,

$$
W_{k,b}\rightsquigarrow W\sim\mathcal N(0,\Gamma),
    \qquad
    \Gamma=\operatorname{Var}\{\zeta(X)\}.
$$

If $K=K_n\to\infty$, the expansion is uniform over nodes whenever

$$
\sqrt K\max_{1\le k\le K}\|r_{k,b}\|\stackrel{p}{\to}0.
$$
...
\widehat s_{K,b,\alpha}
    =\operatorname*{arg\,min}_{s\in\mathbb R^d}{1\over K}\sum_{k=1}^K\|s-W_{k,b}\|_{H_\alpha}.
$$
...
**Theorem 2 (Local reduction).** Suppose Assumptions 1, 2, and 3 hold.  If $K$ is fixed, then uniformly over $\alpha\in I_\varepsilon$,
...
$$
\sqrt K\sup_{\alpha\in I_\varepsilon}
    \left\|
    \sqrt b\log_{\theta_0}\widetilde\theta_{n,\alpha}
    -\widehat s_{K,b,\alpha}
    \right\|
...
holds provided the local Riemannian objective approximation error is $o_p(K^{-1/2})$ uniformly over $\alpha\in I_\varepsilon$.  A sufficient condition is $\sqrt K/b\to0$ together with a uniformly bounded third moment for the localized node errors.

The theorem reduces the nonlinear product-manifold aggregation problem to an ordinary spatial median problem in $\mathbb R^d$.  All scale-dependent covariance and robustness calculations below are consequences of this tangent-space representation.
...
**Theorem 3 (Fixed-$K$ limit).** Suppose $K$ is fixed and the conditions of Theorems 1 and 2 hold.  Let $W_1,\ldots,W_K$ be independent copies of $W\sim\mathcal N(0,\Gamma)$.  If

$$
T_{\alpha,K}=\operatorname*{arg\,min}_{s\in\mathbb R^d}{1\over K}\sum_{k=1}^K\|s-W_k\|_{H_\alpha}
$$
...
\sqrt n\log_{\theta_0}\widetilde\theta_{n,\alpha}
    \rightsquigarrow \sqrt K\,T_{\alpha,K}.
$$
...
$$
S_{\alpha,b}=\operatorname{Var}(U_{\alpha,b}),
$$

where $U_{\alpha,b}$ was defined in Assumption 3.  The covariance associated with the spatial median of finite-block errors is
...
**Theorem 4 (Growing-$K$ CLT with finite-block bias).** Suppose $K\to\infty$, $b\to\infty$, and $n=Kb$.  Under Assumptions 1--3 and the growing-$K$ reduction condition in (22),

$$
\sqrt K\left(\widehat s_{K,b,\alpha}-s_{\alpha,b}\right)
    \rightsquigarrow \mathcal N(0,V_{\alpha,b}).
$$
...
where $Z_{\alpha,b}\rightsquigarrow\mathcal N(0,V_\alpha)$ whenever $V_{\alpha,b}\to V_\alpha$.  If $\sqrt K\,s_{\alpha,b}\to\delta_\alpha$, then

$$
\sqrt n\log_{\theta_0}\widetilde\theta_{n,\alpha}
    \rightsquigarrow \mathcal N(\delta_\alpha,V_\alpha).
$$

In particular, if $\sqrt K\,s_{\alpha,b}\to0$, then the limit is centered.

**Corollary 5 (Symmetry and the node growth condition).** If the finite-block node error distribution is centrally symmetric, $W_b\stackrel d=-W_b$, then $s_{\alpha,b}=0$ for every $\alpha\in I_\varepsilon$, and the centered CLT holds.  More generally, if Assumption 4 holds, then
...
Corollary 5 gives a concrete design lesson.  Increasing $K$ improves robustness to bad nodes and parallel scalability, but it reduces the node size $b$.  If the node PCA error has finite-block skewness, the product median aggregation can have a root-$n$ centering bias unless the nodes remain large enough.  This indicates the nature of $K$, $b$, and $\alpha$ as inferential design choices rather than purely computational parameters.
...
Assume the limiting node error is $W\sim\mathcal N(0,\Gamma)$.  For $\alpha\in I_\varepsilon$, define
...
$$
A_\alpha=\mathbb E\left[{I-U_\alpha U_\alpha^T\over R_\alpha}\right],
    \qquad
    S_\alpha=\operatorname{Var}(U_\alpha).
$$

**Theorem 6 (Scale-dependent covariance).** Under the centered growing-$K$ regime of Theorem 4, if $W_b\rightsquigarrow W\sim\mathcal N(0,\Gamma)$, the spatial median of $Y_\alpha=H_\alpha^{1/2}W$ is zero, and $A_\alpha$ is nonsingular, then
...
$$
\Gamma=\begin{pmatrix}
        \Gamma_{\mu\mu} & \Gamma_{\mu\mathcal U}\\
        \Gamma_{\mathcal U\mu} & \Gamma_{\mathcal U\mathcal U}
    \end{pmatrix}.
$$

The block $\Gamma_{\mu\mu}$ describes mean-estimation uncertainty; $\Gamma_{\mathcal U\mathcal U}$ describes eigenspace-estimation uncertainty; and the cross-blocks describe dependence between the two.  Since $\Gamma_{\mathcal U\mathcal U}$ contains eigengap denominators through (15), weak eigengaps can strongly affect $V_\alpha$.
...
$$
\alpha_{\rm white}=2{\sigma_\mathcal U^2\over \sigma_\mu^2+\sigma_\mathcal U^2}.
$$

More generally, define average block variances

$$
\tau_\mu={1\over p}\operatorname{tr}(\Gamma_{\mu\mu}),
    \qquad
    \tau_\mathcal U={1\over r(p-r)}\operatorname{tr}(\Gamma_{\mathcal U\mathcal U}).
$$

The block-noise calibration rule is

$$
\alpha_{\rm block}=2{\tau_\mathcal U\over \tau_\mu+\tau_\mathcal U}.
$$

This rule calibrates the metric to the uncertainty of the node estimators, not to raw data dispersion.  In PCA terms, if the subspace estimator is noisy, then $\tau_\mathcal U$ is large and $\alpha_{\rm block}$ moves toward two, thereby downweighting the Grassmann term $2-\alpha$.  If the mean estimator is noisy, $\alpha_{\rm block}$ moves toward zero, downweighting the Euclidean term.

**Proposition 7 (Efficiency under perfect whitening).** If $Y_\alpha\sim\mathcal N(0,\sigma^2I_d)$, with $d>1$, then
...
$$
\widehat s_\mathcal U= \operatorname{median}_{1\le k\le K}
    \left\{\sqrt b\,d_{\operatorname{Gr}}(\widehat\mathcal U_k,\widetilde\mathcal U_{\rm prelim})\right\}.
$$
...
\widehat\tau_\mu={\widehat s_\mu^2\over p},
    \qquad
    \widehat\tau_\mathcal U={\widehat s_\mathcal U^2\over r(p-r)},
$$
...
\widehat\alpha_{\rm rPCA}
    =2{\widehat\tau_\mathcal U\over \widehat\tau_\mu+\widehat\tau_\mathcal U}.
$$

**Theorem 8 (Consistency of robust PCA scale calibration).** Suppose $\widehat s_\mu\stackrel{p}{\to} s_\mu$ and $\widehat s_\mathcal U\stackrel{p}{\to} s_\mathcal U$, where $s_\mu,s_\mathcal U\in(0,\infty)$.  Then

$$
\widehat\alpha_{\rm rPCA}\stackrel{p}{\to}
    \alpha_{\rm rPCA}
    =2{ s_\mathcal U^2/[r(p-r)]\over s_\mu^2/p+s_\mathcal U^2/[r(p-r)] }.
$$
...
$$
R(\alpha)=\operatorname{tr}(W_0V_\alpha),
$$
...
$$
\alpha^*=\operatorname*{arg\,min}_{\alpha\in I_\varepsilon}R(\alpha).
$$

Estimate $\alpha^*$ by replacing $V_\alpha$ with a plug-in or bootstrap estimate $\widehat V_\alpha$.

**Theorem 9 (Inference-optimal scale).** Assume $\sup_{\alpha\in I_\varepsilon}\|\widehat V_\alpha-V_\alpha\|\stackrel{p}{\to}0$ and $R$ has a unique minimizer $\alpha^*$ in $I_\varepsilon$.  Then

$$
\widehat\alpha^*=\operatorname*{arg\,min}_{\alpha\in I_\varepsilon}\widehat R(\alpha)
    \stackrel{p}{\to} \alpha^*.
$$

If $R$ is twice continuously differentiable at $\alpha^*$, $R''(\alpha^*)>0$, and $\sqrt K\{\widehat R'(\alpha^*)-R'(\alpha^*)\}\rightsquigarrow N(0,\sigma_R^2)$, then

$$
\sqrt K(\widehat\alpha^*-\alpha^*)\rightsquigarrow N\left(0,{\sigma_R^2\over R''(\alpha^*)^2}\right).
$$
...
**Theorem 10 (First-order effect of estimating scale).** Let $\widehat\alpha\stackrel{p}{\to}\alpha_0\in I_\varepsilon$.  Suppose the median process $\sqrt K(\widehat s_{K,b,\alpha}-s_{\alpha,b})$ is stochastically equicontinuous in $\alpha$.  If $s_{\alpha,b}=0$ for all $\alpha\in I_\varepsilon$, then

$$
\sqrt n\log_{\theta_0}\widetilde\theta_{n,\widehat\alpha}
    \rightsquigarrow \mathcal N(0,V_{\alpha_0}).
$$
...
In particular, if $s_{\alpha,b}=b^{-1/2}a_\alpha+o(b^{-1/2})$ uniformly and $\widehat\alpha-\alpha_0=O_p(K^{-1/2})$, then (49) is $O_p(b^{-1/2})$ and is negligible as $b\to\infty$.
...
We revisit the geometric core of MoM robustness, closely related to the majority-ball arguments used in robust mean estimation, that is deterministic and holds in any metric space. [?, ?].

**Theorem 11 (Deterministic good-node lemma).** Let $z_1, \ldots,z_K\in\mathcal M_p$, and let

$$
\widetilde z_\alpha\in\operatorname*{arg\,min}_{z\in\mathcal M_p}\sum_{k=1}^K d_\alpha(z,z_k).
$$
...
$$
\mathbb E d_\alpha(\widehat\theta_k,\theta_0)^2
    \le {\alpha v_\mu+(2-\alpha)v_\mathcal U\over b}
$$

for constants $v_\mu,v_\mathcal U>0$.  Then there exist universal constants $c_1,c_2>0$ such that

$$
\mathbb P\left(
    d_\alpha(\widetilde\theta_{n,\alpha},\theta_0)
    > c_1\sqrt{\alpha v_\mu+(2-\alpha)v_\mathcal U\over b}
    \right)
    \le \exp(-c_2K).
$$
...
**Corollary 13 (Factorwise deviation tradeoff).** Under the conditions of Theorem 12, with probability at least $1-\exp(-c_2K)$,

$$
\|\widetilde\mu_{n,\alpha}-\mu_0\|
    \le {c_1\over\sqrt\alpha}
    \sqrt{\alpha v_\mu+(2-\alpha)v_\mathcal U\over b},
$$

and

$$
d_{\operatorname{Gr}}(\widetilde\mathcal U_{n,\alpha},\mathcal U_0)
...
\psi_\alpha(w)={w\over \|w\|_{H_\alpha}},
    \qquad w=(w_\mu,w_\mathcal U)\ne0.
$$

**Theorem 14 (Bad-node robustness and factorwise influence).** If fewer than half of the node estimators are arbitrary and the remaining nodes are contained in a bounded $d_\alpha$-ball around $\theta_0$, then the scaled median-of-means estimator remains bounded.  Moreover,
...
$$
\sup_{w\ne0}\|\psi_{\alpha,\mathcal U}(w)\|=(2-\alpha)^{-1/2}.
$$
...
Let $\widehat\theta_1^*,\ldots,\widehat\theta_K^*$ be sampled with replacement from $\widehat\theta_1, \ldots,\widehat\theta_K$, and define $\widetilde\theta_{n,\alpha}^*$ by applying the same scaled median aggregation to the bootstrap node estimates.

**Theorem 15 (Node bootstrap validity).** Under the centered growing-$K$ conditions of Theorem 4, conditionally on the observed nodes,

$$
\sqrt n\log_{\widetilde\theta_{n,\alpha}}
    \widetilde\theta_{n,\alpha}^*
    \rightsquigarrow \mathcal N(0,V_\alpha)
$$

in probability.  If $\alpha$ is estimated and the negligibility condition of Theorem 10 is not invoked, then the bootstrap procedure should recompute $\widehat\alpha^*$ inside each bootstrap replicate.

## Experiments

### Eigengap-driven subspace uncertainty

The first simulation examines whether the proposed scale calibration responds to the main statistical difficulty in PCA: instability of the leading eigenspace when the eigengap is small.  We generated data from a spiked covariance model in $\mathbb R^p$ with $p=200$ and target rank $r=5$.  The sample size was $n=40000$, split into $K=80$ equal-sized nodes, and results were averaged over 60 Monte Carlo replications.  The eigengap $\Delta=\lambda_r-\lambda_{r+1}$ was varied over $\Delta\in\{0.1,0.25,0.5,1,2,4\}.$ For each node, we computed the node mean and leading $r$-dimensional principal subspace.  We compared full-sample PCA, random subset PCA using one node-sized subsample, projector-averaged distributed PCA, fixed-scale MoM PCA with $\alpha=1$, and the proposed robust scale-calibrated MoM PCA.
...
![figure: fig-sim1-eigengap](2605.20681v1/fig-sim1-eigengap.png)

*Spiked PCA simulation with $p=200$, $r=5$, $n=40000$, and $K=80$ nodes. As the eigengap increases, subspace estimation becomes easier and the calibrated scale adjusts to the changing mean--subspace uncertainty. Panels show (a) Grassmann subspace error, (b) Euclidean mean error, (c) selected scale $\widehat\alpha_{\rm rPCA}$, and (d) subspace error relative to scale-calibrated MoM.*
...
![figure: fig-sim1-scale-diagnostics](2605.20681v1/fig-sim1-scale-diagnostics.png)
...
![figure: fig-sim2-fraction](2605.20681v1/fig-sim2-fraction.png)
...
\caption{ {Selected 10x mouse brain results.} Mean and subspace errors are reported relative to the full-data PCA reference for $K=100$.  Projector averaging is the standard distributed baseline, random subset PCA uses one node-sized subsample, and the MoM estimators aggregate node mean--subspace pairs on $\mathbb R^p\times\operatorname{Gr}(r,p)$. }

| \noalign{} Rank | Method | Mean error | Subspace error | $\widehat\alpha$ |
| --- | --- | --- | --- | --- |
| \multirow{4}{*}{$r=10$} | Projector average | $0.00009$ | $0.01667$ | -- |
|  | Random subset | $0.19687$ | $0.30608$ | -- |
|  | MoM, $\alpha=1$ | $0.00253$ | $0.01319$ | $1.000$ |
|  | Scale-calibrated MoM | $0.00320$ | $0.01239$ | $0.269$ |
| \multirow{4}{*}{$r=20$} | Projector average | $0.00009$ | $0.09960$ | -- |
...
![figure: fig-real-mouse-stress-K50-r10](2605.20681v1/fig-real-mouse-stress-K50-r10.png)

\caption{ {10x mouse brain bad-node stress test.} Node-level corruption experiment with $K=50$ and $r=10$.  Panels show (a) mean error under mean-only corruption, (b) subspace error under mean-only corruption, (c) mean error under subspace-only corruption, and (d) subspace error under subspace-only corruption. }
...
Let $P_0=U_rU_r^T$ be the spectral projector onto $\mathcal U_0$, and let $\widehat P_k$ be the projector onto $\widehat\mathcal U_k$.  By the differentiability of spectral projectors under the eigengap condition,

$$
\widehat P_k-P_0
    =\mathcal L_\Sigma(\widehat\Sigma_k-\Sigma)+O_p(\|\widehat\Sigma_k-\Sigma\|^2),
$$
...
The moment assumption implies $\|\widehat\Sigma_k-\Sigma\|=O_p(b^{-1/2})$, so the quadratic remainder is $o_p(b^{-1/2})$.  In the Grassmann normal chart at $\mathcal U_0$, the tangent coordinate matrix for $\widehat\mathcal U_k$ is, to first order, the off-diagonal block

$$
U_\perp^T\mathcal L_\Sigma(\widehat\Sigma_k-\Sigma)U_r.
$$

Substituting the covariance expansion yields

$$
\sqrt b\log_{\mathcal U_0}(\widehat\mathcal U_k)
    =b^{-1/2}\sum_{i\in B_k}B(X_i)+o_p(1),
$$

with entries defined in (15).  Combining the mean and subspace components gives (16).  The multivariate central limit theorem applies because $\mathbb E\|X-\mu_0\|^{4+\delta}<\infty$ implies $\zeta(X)$ has finite second moment.  The uniform remainder condition (19) is stated explicitly for growing $K$.

### Proof of Theorem 2

Work on the high-probability event that all node subspaces lie in a common Grassmann normal neighborhood of $\mathcal U_0$.  For bounded tangent vectors $v,w$, the squared product distance between $\exp_{\theta_0}(v/\sqrt b)$ and $\exp_{\theta_0}(w/\sqrt b)$ has the local expansion
...
where the remainder is uniform over $\alpha\in I_\varepsilon$.  Taking square roots gives
...
where $\sup_{v\in C,\alpha\in I_\varepsilon}|\Delta_{K,b,\alpha}(v)|=o_p(1)$ for fixed $K$ on every compact set $C$.  The objectives are convex in the tangent approximation and locally coercive under Assumption (A2).  Standard argmin transfer therefore gives (21).  For growing $K$, the same argument applies at the $K^{-1/2}$ scale under the stated stronger approximation condition.  The sufficient condition $\sqrt K/b\to0$ follows from the displayed remainder bound and a bounded third moment for $W_{k,b}$.

### Proof of Theorem 3

By Theorem  1, $(W_{1,b},\ldots,W_{K,b})\rightsquigarrow(W_1, \ldots,W_K)$.  The map sending $(w_1, \ldots,w_K)$ to the unique minimizer of $K^{-1}\sum_k\|s-w_k\|_{H_\alpha}$ is continuous at configurations with a unique median.  Hence the continuous mapping theorem gives $\widehat s_{K,b,\alpha}\rightsquigarrow T_{\alpha,K}$.  The local reduction theorem transfers this convergence to $\sqrt b\log_{\theta_0}\widetilde\theta_{n,\alpha}$.  Multiplication by $\sqrt K$ gives the root-$n$ form.
...
If $W_b\stackrel d=-W_b$, then $s=0$ minimizes $\mathbb E\|s-W_b\|_{H_\alpha}$ by symmetry and uniqueness, so $s_{\alpha,b}=0$.  The second statement follows by substituting (14) into $\sqrt K s_{\alpha,b}$.
...
If $Y_\alpha=RU$, where $U$ is uniform on the unit sphere in $\mathbb R^d$ and independent of $R/\sigma\sim\chi_d$, then $S_\alpha=\operatorname{Var}(U)=I_d/d$.  Also

$$
A_\alpha=\mathbb E\left[{I-UU^T\over R}\right]
    =\mathbb E(R^{-1})\left(I_d-{I_d\over d}\right)
    ={d-1\over d}\mathbb E(R^{-1})I_d.
$$
...
A_\alpha^{-1}S_\alpha A_\alpha^{-T}
    ={d\over(d-1)^2}\{\mathbb E(R^{-1})\}^{-2}I_d.
$$

Because $R=\sigma\chi_d$ and $\mathbb E(\chi_d^{-1})=2^{-1/2}\Gamma((d-1)/2)/\Gamma(d/2)$, the constant is (38).  Stirling's formula gives $c_d\to1$.
...
Uniform convergence of $\widehat V_\alpha$ implies uniform convergence of $\widehat R(\alpha)$ to $R(\alpha)$ on the compact set $I_\varepsilon$.  Consistency follows from the standard argmin theorem.  For the second statement, the first-order condition $\widehat R'(\widehat\alpha^*)=0$ and a Taylor expansion around $\alpha^*$ give
...
Let $C_\alpha=\alpha v_\mu+(2-\alpha)v_\mathcal U$.  By Markov's inequality,

$$
\mathbb P\{d_\alpha(\widehat\theta_k,\theta_0)>2\sqrt{C_\alpha/b}\}\le {1\over4}.
$$
...
d_\alpha(\theta,\theta_0)^2
    =\alpha\|\mu-\mu_0\|^2+(2-\alpha)d_{\operatorname{Gr}}(\mathcal U,\mathcal U_0)^2.
$$

Therefore $\|\mu-\mu_0\|\le d_\alpha(\theta,\theta_0)/\sqrt\alpha$ and $d_{\operatorname{Gr}}(\mathcal U,\mathcal U_0)\le d_\alpha(\theta,\theta_0)/\sqrt{2-\alpha}$.  Combine these inequalities with Theorem 12.
...
$$
\|\psi_{\alpha,\mu}(w)\|
    ={\|w_\mu\|\over\{\alpha\|w_\mu\|^2+(2-\alpha)\|w_\mathcal U\|^2\}^{1/2}}
    \le \alpha^{-1/2}.
$$

Equality is approached when $w_\mathcal U=0$ and $w_\mu\ne0$.  The subspace component is identical, giving $(2-\alpha)^{-1/2}$.
...
$$
\sqrt K(\widehat s_{K,b,\alpha}^*-\widehat s_{K,b,\alpha})\rightsquigarrow \mathcal N(0,V_\alpha)
$$
...
![figure: fig-supp-s1-oracle-covariance](2605.20681v1/fig-supp-s1-oracle-covariance.png)

*Oracle covariance check. Gaussian node errors are generated directly in tangent coordinates, so the comparison isolates the spatial-median covariance formula without additional PCA estimation error.  Panels show (a) theoretical covariance trace as a function of scale, (b) empirical trace versus theoretical trace, (c) relative trace error, and (d) oracle scale $\alpha^*$ as a function of the mean-to-subspace variance ratio.*

Figure 8 summarizes the comparison.  The theoretical trace changes with the variance ratio and cross-block correlation, but is nearly flat in $\alpha$ for the tested Gaussian configurations.  The empirical trace closely follows the theoretical trace: the scatter plot in panel (b) lies close to the diagonal, and the relative trace errors in panel (c) are small.  The oracle scale $\alpha^*$, computed by minimizing the theoretical trace over the grid, changes with the mean--subspace variance ratio, as expected.  When mean variability is smaller relative to subspace variability, the optimal scale tends to put less weight on the subspace component; when mean variability is larger, the optimal scale moves in the opposite direction.

*Oracle covariance validation. Mean and maximum absolute relative errors between the theoretical covariance trace and the empirical covariance trace of replicated scaled spatial medians.  The ratio column gives the mean-to-subspace variance ratio, and $\rho$ controls cross-block correlation.  The errors are small across all tested configurations.*
...
![figure: fig-supp-s1-replication-distributions](2605.20681v1/fig-supp-s1-replication-distributions.png)
...
![figure: fig-supp-s2-asymptotic-regimes](2605.20681v1/fig-supp-s2-asymptotic-regimes.png)

\caption{ {Fixed-$K$, growing-$K$, and aggressive-$K$ regimes.} Simulation illustrating the finite-block bias term in the growing-node CLT.  The fixed-$K$ regime moves toward centered behavior as $b$ increases, while the aggressive regime has a visibly shifted first root-$n$ coordinate.  Panels show (a) mean of the first root-$n$ coordinate, (b) standard deviation of the first root-$n$ coordinate, (c) mean root-$n$ norm, and (d) boxplots of the first root-$n$ coordinate. }

\caption{ {Asymptotic-regime summary.} Summary of the first coordinate and norm of the root-$n$-scaled tangent error under fixed-$K$, growing-$K$, and aggressive-$K$ regimes.  The aggressive regime has a large nonzero first-coordinate mean, illustrating the finite-block bias effect when the number of nodes is too large relative to the block size. }
...
*Quantiles of root-$n$-scaled errors. Quantiles of the first coordinate and norm of the root-$n$-scaled tangent error.  The aggressive regime has a shifted median and upper quantile for the first coordinate, while the norm quantiles remain comparable across regimes.*
...
This section reports two additional real-data analyses.  The main paper focuses on the 10x mouse brain data because it is a large single-cell RNA-seq example where PCA is a standard preprocessing and visualization tool.  Here we use two supplementary datasets to examine whether the same mean--subspace aggregation behavior appears in different settings.  The first is a human single-cell dataset from Tabula Sapiens [?], which stays close to the single-cell motivation but introduces a different biological context.  The second is a climate EOF/PCA example based on NOAA OISST sea-surface temperature fields [?, ?], which tests the method outside genomics.
...
![figure: fig-tabula-TS_Blood](2605.20681v1/fig-tabula-TS_Blood.png)

\caption{ {Tabula Sapiens blood-cell PCA.} Distributed PCA comparison on the Tabula Sapiens blood subset using $p=2000$ highly variable genes, $K\in\{20,50\}$ nodes, and ranks $r\in\{10,20\}$.  Full-data PCA is used as the centralized reference.  Random subset PCA uses one node-sized subsample, while projector averaging and the two MoM procedures aggregate all node estimates.  Panels show (a) subspace error for $r=10$, (b) subspace error for $r=20$, (c) selected calibrated scale, and (d) mean error. }

Figure 11 summarizes the results.  Random subset PCA has the largest mean error and is consistently unstable in the subspace coordinate.  This is expected because it discards most nodes and uses only one node-sized subsample.  Projector averaging is highly accurate for the mean, but its subspace error increases with the number of nodes.  The MoM estimators have slightly larger mean errors than projector averaging, but their mean errors remain very small on the scale of the log-normalized data.  In contrast, their subspace errors are competitive and often smaller than projector averaging, especially for $r=20$.

\caption{ {Tabula Sapiens blood-cell analysis.} Representative results for $K=50$ nodes using $p=2000$ highly variable genes.  Errors are measured relative to the full-data PCA reference.  Random subset PCA uses one node-sized subsample, projector averaging is the standard distributed baseline, and the MoM estimators aggregate node mean--subspace pairs on $\mathbb R^p\times\operatorname{Gr}(r,p)$. }

| Rank | Method | Mean error | Subspace error | $\widehat\alpha$ |
| --- | --- | --- | --- | --- |
| \multirow{4}{*}{$r=10$} | Projector average | $0.000018$ | $4.4080$ | -- |
|  | Random subset | $0.028788$ | $4.7250$ | -- |
|  | MoM, $\alpha=1$ | $0.000036$ | $4.2133$ | $1.000$ |
|  | Scale-calibrated MoM | $0.000035$ | $4.2141$ | $1.980$ |
| \multirow{4}{*}{$r=20$} | Projector average | $0.000025$ | $6.9695$ | -- |
...
![figure: fig-oisst-clean](2605.20681v1/fig-oisst-clean.png)

\caption{ {NOAA OISST EOF/PCA analysis.} Distributed EOF/PCA comparison on a regional OISST sea-surface-temperature anomaly matrix.  Full-data EOF/PCA is used as the centralized reference.  Projector averaging, random subset PCA, fixed-scale MoM, and scale-calibrated MoM are compared using $K=5$ temporal nodes and rank $r=3$.  Panels show (a) subspace error, (b) mean error, (c) selected calibrated scale, and (d) combined error. }

\caption{ {NOAA OISST EOF/PCA analysis.} Results for the regional OISST anomaly matrix with $n=120$, $p=80$, $K=5$ temporal nodes, and rank $r=3$.  Errors are measured relative to the full-data EOF/PCA reference.  This example is temporally dependent and is used as a supplementary cross-domain demonstration. }
