...
Let $\mathcal{P}_2(\mathbb{R}^d)$ denote the set of Borel probability measures on $\mathbb{R}^d$ with finite second moment, equipped with the quadratic Wasserstein distance $W_2$ [?, ?]. If $\Pi$ is a Borel probability law on $\mathcal{P}_2(\mathbb{R}^d)$ and $\psi\colon [0,\infty)\to\mathbb{R}$ is measurable, define

$$
D_{\psi}(\Pi)=E\bigl[\psi\{W_2(\mu,\nu)\}\bigr],
$$

where $\mu,\nu\stackrel{\text{i.i.d.}}{\sim}\Pi$. We call $D_{\psi}(\Pi)$ the pairwise Wasserstein heterogeneity of $\Pi$. When $\psi(t)=t^2$, the functional measures mean squared separation under the Wasserstein geometry. When $\psi$ is bounded, it yields a more robust summary that downweights unusually distant pairs.

The appeal of (1) is its simplicity. It depends only on pairwise Wasserstein distances and therefore does not require a Wasserstein barycenter or any other centring object that are expensive to attain [?, ?]. The same framework also accommodates different scientific goals through the choice of $\psi$. One may emphasize overall spread with a distance-like transform or suppress rare extreme pairs with a bounded one.

Given independent draws $\mu_1,\ldots,\mu_n\sim\Pi$, the natural estimator of (1) is the order-two $U$--statistic
...
If $E|h(\mu,\nu)|<\infty$, then $U_n$ is unbiased for $D_{\psi}(\Pi)$ by standard $U$--statistic theory [?, ?]. The practical questions are how to express this integrability in terms of transparent assumptions on $\Pi$, and how to translate the resulting large-sample theory into an inferential procedure that is easy to report and interpret.
...
where $\mu_1\sim\Pi$ is independent of $\mu$. The function $g$ is the first Hoeffding projection of the kernel [?]. It compares the expected transformed distance from $\mu$ to a typical draw with the overall population average. Measures with $g(\mu)>0$ are more isolated than a typical observation on average, whereas measures with $g(\mu)<0$ are more central.

**Theorem 2.2.** Assume the growth condition of Lemma 2.1.

(i) If $E\{W_2(\mu,\mu_0)^p\}<\infty$ for some $\mu_0\in\mathcal{P}_2(\mathbb{R}^d)$, then $U_n\to D_{\psi}(\Pi)$ almost surely.

(ii) If $E\{W_2(\mu,\mu_0)^{2p}\}<\infty$ for some $\mu_0\in\mathcal{P}_2(\mathbb{R}^d)$ and $\operatorname{var}\{g(\mu_1)\}>0$, then

$$
n^{1/2}\{U_n-D_{\psi}(\Pi)\}\to \mathcal{N}\bigl[0,4\operatorname{var}\{g(\mu_1)\}\bigr]
$$

in distribution.

Part (i) requires only integrability of the kernel, whereas part (ii) requires square integrability. This is the natural split since consistency depends on a first moment, while the Gaussian approximation depends on a second moment. If $\operatorname{var}\{g(\mu_1)\}=0$, the kernel is first-order degenerate and the normal approximation does not capture the leading behaviour.
...
**Proposition 2.3 (Variance estimation).** Under the assumptions of Theorem 2.2(ii), $\widehat\sigma_n^2\to 4\operatorname{var}\{g(\mu_1)\}$ in probability. Hence
...
The same construction compares two populations of random measures. Let $\Pi_A$ and $\Pi_B$ be laws on $\mathcal{P}_2(\mathbb{R}^d)$, with independent samples of sizes $n_A$ and $n_B$, and let $U_A$ and $U_B$ be the corresponding within-group statistics.

**Proposition 2.4 (Two-sample comparison).** Assume that the conditions of Theorem 2.2(ii) hold for both groups, with non-zero projection variances. If $\Delta=D_{\psi}(\Pi_A)-D_{\psi}(\Pi_B)$, then $U_A-U_B\to \Delta$ almost surely and
...
in distribution, where $\widehat\sigma_A^2$ and $\widehat\sigma_B^2$ are computed from (5) within each group.
...
**Proposition 2.5 (Plug-in stability).** Assume that $\psi$ is $L$-Lipschitz. Let $\widehat U_n$ be obtained from (2) by replacing $\mu_i$ with approximations $\widehat\mu_i$. Then

$$
|\widehat U_n-U_n|\leq 2Ln^{-1}\sum_{i=1}^n W_2(\widehat\mu_i,\mu_i).
$$

Proposition 2.5 shows that first-stage estimation enters only through an average Wasserstein error term. In particular, if $n^{-1}\sum_{i=1}^n W_2(\widehat\mu_i,\mu_i)=o_p(n^{-1/2})$, then replacing $U_n$ by $\widehat U_n$ does not alter the first-order inference. Combined with available rates for empirical measures in Wasserstein distance [?], this justifies replacing idealized unit-level measures by empirical approximations.
...
*Random translations.* Let $\mu_{\theta}=T_{\theta}\#\mu_0$, where $T_{\theta}(x)=x+\theta$ and $\mu_0\in\mathcal{P}_2(\mathbb{R}^d)$ is fixed. Every measure in the family is the same template shifted by a vector $\theta$, so translation equivariance of $W_2$ gives

$$
W_2(\mu_{\theta},\mu_{\theta'})=\|\theta-\theta'\|.
$$

If $\mu=\mu_{\Theta}$ for an $\mathbb{R}^d$-valued random shift $\Theta$, then heterogeneity of the random measures is exactly heterogeneity of the latent shifts
...
$$
E\|\Theta-\Theta'\|^2=2E\|\Theta-E\Theta\|^2=2\operatorname{tr}\{\operatorname{cov}(\Theta)\}.
$$
...
For Gaussian measures with equal covariance matrices, the squared Wasserstein distance is the squared Euclidean distance between the means [?]. Hence

$$
W_2^2\bigl[\mathcal{N}(\theta,1),\mathcal{N}(\theta',1)\bigr]=(\theta-\theta')^2.
$$
...
$$
D_{t^2}(\Pi\mid Y)=2\tau_m^2=2\operatorname{var}(\theta\mid Y).
$$
...
*A symmetric two-point population.* Let $\Pi=\tfrac{1}{2}\delta_{\mu_1}+\tfrac{1}{2}\delta_{\mu_2}$, with $\mu_1\neq\mu_2$. When two independent draws are taken from this population, they coincide with probability $1/2$ and differ with probability $1/2$. It follows that

$$
D_{\psi}(\Pi)=\tfrac{1}{2}\psi(0)+\tfrac{1}{2}\psi\bigl[W_2(\mu_1,\mu_2)\bigr].
$$
...
$$
E\{h(\mu_k,\mu')\}=\tfrac{1}{2}\psi(0)+\tfrac{1}{2}\psi\bigl[W_2(\mu_1,\mu_2)\bigr]=D_{\psi}(\Pi),
$$
...
These examples also clarify how (4) should be read in practice. The empirical eccentricities are observation-level summaries of how strongly individual measures contribute to the overall heterogeneity. They therefore play an interpretive role as well as an inferential one.

## Numerical illustrations

### Synthetic Gaussian measures
...
where $m_i\in\mathbb{R}^2$ is a mean vector and $(s_{i1},s_{i2})$ is the vector of marginal standard deviations. For this family,
...
*Within-digit Wasserstein heterogeneity in MNIST. SE denotes the standard error. Rank 1 corresponds to the largest estimated within-digit heterogeneity.*
...
![figure: sim_mnist_eccentric_527](2603.14815v1/sim_mnist_eccentric_527.png)

*Most eccentric images for digits 5 (top row), 2 (middle row), and 7 (bottom row).*

Figure 3 displays the most eccentric images from digits 5, 2, and 7, corresponding to the largest values of $\widehat g_i$, which contribute heavily to the corresponding $U$–statistics and illustrate how the empirical eccentricities identify the observations responsible for within-class heterogeneity.
...
Two issues merit further study. The first is computation, since a full analysis requires all pairwise Wasserstein distances within a sample. The second is degeneracy: highly symmetric populations can have positive heterogeneity but vanishing first-order projection variance, so that the normal approximation in Theorem 2.2 no longer applies. Natural next steps are scalable approximations based on incomplete pairwise sampling or faster Wasserstein computation, together with a more systematic study of how the choice of transform $\psi$ trades sensitivity against robustness.
...
$$
\theta = D_{\psi}(\Pi),\quad h_{ij}=h(\mu_i,\mu_j)=\psi\{W_2(\mu_i,\mu_j)\}.
$$
...
**Proof of Lemma 2.1**\\ By the triangle inequality for $W_2$,

$$
W_2(\mu,\nu)\leq W_2(\mu,\mu_0)+W_2(\nu,\mu_0).
$$
...
$$
W_2(\mu,\nu)^p
\leq 2^{p-1}\bigl\{W_2(\mu,\mu_0)^p+W_2(\nu,\mu_0)^p\bigr\}.
$$
...
a+b\,W_2(\mu,\nu)^p
\leq
a+b\,2^{p-1}\bigl\{W_2(\mu,\mu_0)^p+W_2(\nu,\mu_0)^p\bigr\}.
$$
...
|h(\mu,\nu)|
\leq
C\bigl\{1+W_2(\mu,\mu_0)^p+W_2(\nu,\mu_0)^p\bigr\},
$$

which proves the stated domination bound.

Now suppose that $E\{W_2(\mu,\mu_0)^p\}<\infty$. Since $\mu$ and $\nu$ are independent copies,

$$
E|h(\mu,\nu)|
\leq
C\Bigl[1+E\{W_2(\mu,\mu_0)^p\}+E\{W_2(\nu,\mu_0)^p\}\Bigr]
=
C\Bigl[1+2E\{W_2(\mu,\mu_0)^p\}\Bigr]
<\infty.
$$

Next suppose that $E\{W_2(\mu,\mu_0)^{2p}\}<\infty$. Squaring the domination bound and using
...
h(\mu,\nu)^2
\leq
3C^2\bigl\{1+W_2(\mu,\mu_0)^{2p}+W_2(\nu,\mu_0)^{2p}\bigr\}.
$$
...
E\{h(\mu,\nu)^2\}
\leq
3C^2\Bigl[1+2E\{W_2(\mu,\mu_0)^{2p}\}\Bigr]
<\infty.
$$

It remains to show that the finiteness of the Wasserstein moments does not depend on the choice of $\mu_0$. Let $\mu_1\in\mathcal{P}_2(\mathbb{R}^d)$ be fixed, and let $q>0$. Again by the triangle inequality,

$$
W_2(\mu,\mu_1)\leq W_2(\mu,\mu_0)+W_2(\mu_0,\mu_1).
$$
...
W_2(\mu,\mu_1)^q
\leq
2^{q-1}\bigl\{W_2(\mu,\mu_0)^q+W_2(\mu_0,\mu_1)^q\bigr\}.
$$

Since $\mu_0$ and $\mu_1$ are fixed elements of $\mathcal{P}_2(\mathbb{R}^d)$, the quantity $W_2(\mu_0,\mu_1)$ is finite. Therefore

$$
E\{W_2(\mu,\mu_1)^q\}
\leq
2^{q-1}E\{W_2(\mu,\mu_0)^q\}
+
2^{q-1}W_2(\mu_0,\mu_1)^q.
$$

Thus finiteness of $E\{W_2(\mu,\mu_0)^q\}$ implies finiteness of $E\{W_2(\mu,\mu_1)^q\}$. Interchanging the roles of $\mu_0$ and $\mu_1$ gives the converse implication. Applying this with $q=p$ and $q=2p$ proves the claim.\qed

**Proof of Theorem 2.2**\\ For part (i), Lemma 2.1 implies that

$$
E|h(\mu_1,\mu_2)|<\infty.
$$

The kernel $h$ is measurable and symmetric, because $W_2(\mu,\nu)=W_2(\nu,\mu)$. Therefore the classical strong law for $U$--statistics of order two applies, and yields

$$
U_n\to \theta=D_{\psi}(\Pi)
$$

almost surely. For more details, see,  for example, [?] or [?].
...
The studentized central limit theorem now follows from Theorem 2.2(ii), the positivity assumption $\operatorname{var}\{g(\mu_1)\}>0$, and Slutsky's theorem:
...
**Proof of Proposition 2.4**\\ Write
...
in distribution. Because the two samples are independent, $Z_{A,n}$ and $Z_{B,n}$ are independent for each $n$. Hence, for any $s,t\in\mathbb{R}$,
...
**Proof of Proposition 2.5**\\ Write

$$
\widehat h_{ij}
=
\psi\bigl\{W_2(\widehat\mu_i,\widehat\mu_j)\bigr\},
\qquad
h_{ij}
=
\psi\bigl\{W_2(\mu_i,\mu_j)\bigr\}.
$$
...
\leq
L\left|
W_2(\widehat\mu_i,\widehat\mu_j)-W_2(\mu_i,\mu_j)
\right|.
$$

Now use the reverse triangle inequality for the metric $W_2$:
...
\right|
\leq
 W_2(\widehat\mu_i,\mu_i)+W_2(\widehat\mu_j,\mu_j).
$$
...
\Bigl\{W_2(\widehat\mu_i,\mu_i)+W_2(\widehat\mu_j,\mu_j)\Bigr\}.
$$

Each term $W_2(\widehat\mu_i,\mu_i)$ appears exactly $n-1$ times in the double sum, so

$$
\sum_{1\leq i<j\leq n}
\Bigl\{W_2(\widehat\mu_i,\mu_i)+W_2(\widehat\mu_j,\mu_j)\Bigr\}
=
(n-1)\sum_{i=1}^n W_2(\widehat\mu_i,\mu_i).
$$
...
|\widehat U_n-U_n|
\leq
\frac{2L}{n}\sum_{i=1}^n W_2(\widehat\mu_i,\mu_i),
$$
