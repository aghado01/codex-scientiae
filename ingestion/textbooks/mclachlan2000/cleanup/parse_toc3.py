...
roman = {'vii','viii','xiii','xiv','xv','xvi','xvii','xviii','contents','content'}
...
def is_separator(line):
    s=line.strip()
    return not s or s.lower() in roman or re.match(r'^\|[-\s\|]+$', s)
...
for line in lines[1:]:
    ...
    if not sec and a=='' and b=='' and c:
        if current is not None:
            level = current['level'] + 1
    items.append({'sec':sec,'text':title.strip(),'level':level,'page':page})
    current=items[-1]

out=['# Contents']
for item in items:
    if item['text'].lower() in roman:
        continue
    prefix='  ' * item['level'] + '- '
    ...
    if item['page']:
        line += ' — ' + item['page']
    out.append(line)
Path('formatted_toc4.md').write_text('\n'.join(out) + '\n', encoding='utf-8')
print('wrote formatted_toc4.md')
