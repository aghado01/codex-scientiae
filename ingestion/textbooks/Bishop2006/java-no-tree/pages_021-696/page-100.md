...
First of all, we note that the matrix Σ can be taken to be symmetric, without loss of generality, because any antisymmetric component would disappear from the

- Exercise 2.17 exponent. Now consider the eigenvector equation for the covariance matrix Σui = λiui (2.45)

where i = 1,...,D. Because Σ is a real, symmetric matrix its eigenvalues will be

- Exercise 2.18 real, and its eigenvectors can be chosen to form an orthonormal set, so that uTi uj = Iij (2.46)
