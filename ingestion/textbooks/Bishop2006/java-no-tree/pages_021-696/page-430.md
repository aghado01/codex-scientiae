...
So far, we have assumed that all of the variables in the graph are hidden. In most practical applications, a subset of the variables will be observed, and we wish to calculate posterior distributions conditioned on these observations. Observed nodes are easily handled within the sum-product algorithm as follows. Suppose we partition x into hidden variables h and observed variables v, and that the observed value of v is denoted v. Then we simply multiply the joint distribution p(x) by i I(vi, vi), where I(v, v) = 1 if v = v and I(v, v) = 0 otherwise. This product corresponds to p(h,v = v) and hence is an unnormalized version of p(h|v = v). By running the sum-product algorithm, we can efﬁciently calculate the posterior marginals p(hi|v = v) up to a normalization coefﬁcient whose value can be found efﬁciently using a local computation. Any summations over variables in v then collapse into a single term.
