...
- 1. Initialize the data weighting coefﬁcients {wn} by setting wn(1) = 1/N for n = 1,...,N.
- 2. For m = 1,...,M:


- (a) Fit a classiﬁer ym(x) to the training data by minimizing the weighted error function
...
- (c) Update the data weighting coefﬁcients wn(m+1) = wn(m) exp{αmI(ym(xn) = tn)} (14.18)
