...
divergence, and the minimum occurs when qj(Zj) = p(X,Zj). Thus we obtain a general expression for the optimal solution q j(Zj) given by

###### lnq j(Zj) = Ei =j[lnp(X,Z)] + const. (10.9)
...
The additive constant in (10.9) is set by normalizing the distribution q j(Zj). Thus if we take the exponential of both sides and normalize, we have
