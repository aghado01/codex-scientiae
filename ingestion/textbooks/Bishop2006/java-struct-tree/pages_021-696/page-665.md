...
where the same samples are used in the numerator as in the denominator. Thus the posterior distribution p(zn|xn) is represented by the set of samples {z(nl)} together with the corresponding weights {wn(l)}. Note that these weights satisfy 0 � wn(l)1 and
