...
Bern(x|µ) = µx(1 − µ)1−x (2.2) Exercise 2.1 which is known as the Bernoulli distribution. It is easily veriﬁed that this distribution
...
At this point, it is worth noting that the log likelihood function depends on the N observations xn only through their sum

�

n xn. This sum provides an example of a

sufﬁcient statistic for the data under this distribution, and we shall study the imporSection 2.4 tant role of sufﬁcient statistics in some detail. If we set the derivative of lnp(D|µ)
