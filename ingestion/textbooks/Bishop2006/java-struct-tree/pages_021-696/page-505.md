...
We now ask whether this resulting solution will factorize between A and B, in other words whether q�(A,B) = q�(A)q�(B). This will happen if, and only if, lnp(A,B|X,C) = lnp(A|X,C) + lnp(B|X,C), that is, if the conditional independence relation
