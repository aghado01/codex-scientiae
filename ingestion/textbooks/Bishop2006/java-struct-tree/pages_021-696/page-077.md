...
where we have used the fact that −lnx is a convex function, together with the normalization condition

�

q(x)dx = 1. In fact, −lnx is a strictly convex function, so the equality will hold if, and only if, q(x) = p(x) for all x. Thus we can interpret the Kullback-Leibler divergence as a measure of the dissimilarity of the two distributions p(x) and q(x).
