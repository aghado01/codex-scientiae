...
The gating functions πk(x) must satisfy the usual constraints for mixing coefﬁcients, namely 0 � πk(x) � 1 and

�

k πk(x) = 1. They can therefore be represented, for example, by linear softmax models of the form (4.104) and (4.105). If the experts are also linear (regression or classiﬁcation) models, then the whole model can be ﬁtted efﬁciently using the EM algorithm, with iterative reweighted least squares being employed in the M step (Jordan and Jacobs, 1994).
...
given by Bishop and Svensen (2003) based on variational inference.´
