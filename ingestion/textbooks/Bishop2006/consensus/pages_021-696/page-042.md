...
$$
\propto \text{likelihood} \times \text{prior}. \tag{1.44}
$$
...
$$
p(\mathcal{D}) = \int p(\mathcal{D} \mid \mathbf{w})p(\mathbf{w})\,d\mathbf{w}. \tag{1.45}
$$

In both the Bayesian and frequentist paradigms, the likelihood function $p(\mathcal{D} \mid \mathbf{w})$ plays a central role. However, the manner in which it is used is fundamentally different in the two approaches. In a frequentist setting, $\mathbf{w}$ is considered to be a ﬁxed parameter, whose value is determined by some form of ‘estimator’, and error bars
