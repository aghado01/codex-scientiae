...
We can gain some insight into the problem by examining the values of the coefﬁcients $\mathbf{w}$ obtained from polynomials of various order, as shown in Table 1.1. We see that, as $M$ increases, the magnitude of the coefﬁcients typically gets larger. In particular for the $M = 9$ polynomial, the coefﬁcients have become ﬁnely tuned to the data by developing large positive and negative values so that the correspond-

Table 1.1 Table of the coefﬁcients $\mathbf{w}$ for polynomials of various order. Observe how the typical magnitude of the coefﬁcients increases dramatically as the order of the polynomial increases.
