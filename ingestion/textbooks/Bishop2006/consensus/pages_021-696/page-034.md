...
$$
p(Y = y_j \mid X = x_i) = \frac{n_{ij}}{c_i}. \tag{1.8}
$$

From (1.5), (1.6), and (1.8), we can then derive the following relationship

$$
p(X = x_i, Y = y_j) = \frac{n_{ij}}{N} = \frac{n_{ij}}{c_i} \cdot \frac{c_i}{N} = p(Y = y_j \mid X = x_i) p(X = x_i). \tag{1.9}
$$
...
Here $p(X,Y)$ is a joint probability and is verbalized as “the probability of $X$ and $Y$”. Similarly, the quantity $p(Y \mid X)$ is a conditional probability and is verbalized as “the probability of $Y$ given $X$”, whereas the quantity $p(X)$ is a marginal probability
