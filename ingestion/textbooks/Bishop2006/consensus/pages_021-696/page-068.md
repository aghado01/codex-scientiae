...
- (b) First solve the inference problem of determining the conditional density $p(t \mid x)$, and then subsequently marginalize to ﬁnd the conditional mean given by (1.89).
...
The squared loss is not the only possible choice of loss function for regression. Indeed, there are situations in which squared loss can lead to very poor results and where we need to develop more sophisticated approaches. An important example concerns situations in which the conditional distribution $p(t \mid x)$ is multimodal, as often arises in the solution of inverse problems. Here we consider brieﬂy one simple generalization of the squared loss, called the Minkowski loss, whose expectation is given by

$$
\mathbb{E}[L_q] = \iint |y(x) - t|^q p(x, t)\, dx\, dt. \tag{1.91}
$$
