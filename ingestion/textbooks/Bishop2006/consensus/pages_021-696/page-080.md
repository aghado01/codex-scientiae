...
$$
\sum_{i=1}^{D} \sum_{j=1}^{D} w_{ij} x_i x_j = \sum_{i=1}^{D} \sum_{j=1}^{D} w_{ij}^{S} x_i x_j. \tag{1.132}
$$
...
The coefﬁcients $w_{i_1 i_2 \cdots i_M}$ comprise $D^M$ elements, but the number of independent parameters is signiﬁcantly fewer due to the many interchange symmetries of the factor $x_{i_1} x_{i_2} \cdots x_{i_M}$. Begin by showing that the redundancy in the coefﬁcients can be removed by rewriting this $M$th-order term in the form
