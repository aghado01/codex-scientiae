...
8.14 ($\star$) Consider a particular case of the energy function given by (8.42) in which the coefﬁcients $\beta = h = 0$. Show that the most probable conﬁguration of the latent variables is given by $x_i = y_i$ for all $i$.

8.15 ($\star$) www Show that the joint distribution $p(x_{n-1}, x_n)$ for two neighbouring nodes in the graph shown in Figure 8.38 is given by an expression of the form (8.58).
