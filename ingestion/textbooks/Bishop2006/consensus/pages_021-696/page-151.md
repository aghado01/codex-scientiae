...
2.16 ( $\star$ ) www Consider two random variables $x_1$ and $x_2$ having Gaussian distributions with means $\mu_1, \mu_2$ and precisions $\tau_1, \tau_2$ respectively. Derive an expression for the differential entropy of the variable $x = x_1 + x_2$. To do this, ﬁrst ﬁnd the distribution of $x$ by using the relation
...
2.20 ( $\star$ ) www A positive deﬁnite matrix $\boldsymbol{\Sigma}$ can be deﬁned as one for which the quadratic form

$$
\mathbf{a}^{\text{T}}\boldsymbol{\Sigma}\mathbf{a} \tag{2.285}
$$

is positive for any real value of the vector $\mathbf{a}$. Show that a necessary and sufﬁcient condition for $\boldsymbol{\Sigma}$ to be positive deﬁnite is that all of the eigenvalues $\lambda_i$ of $\boldsymbol{\Sigma}$, deﬁned by (2.45), are positive.
