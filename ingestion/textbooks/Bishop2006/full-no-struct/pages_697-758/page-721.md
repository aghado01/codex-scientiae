...
A matrix A is said to be positive deﬁnite , denoted by A 0 , if w Aw > 0 for all values of the vector w . Equivalently, a positive deﬁnite matrix has λ i > 0 for all of its eigenvalues (as can be seen by setting w to each of the eigenvectors in turn, and by noting that an arbitrary vector can be expanded as a linear combination of the eigenvectors). Note that positive deﬁnite is not the same as all the elements being positive. For example, the matrix
