...
which is the conditional average of t conditioned on x and is known as the regression function . This result is illustrated in Figure 1.28. It can readily be extended to multiple target variables represented by the vector t , in which case the optimal solution is the conditional average y ( x ) = E t [ t | x ] . We can also derive this result in a slightly different way, which will also shed
...
$$
\{ y ( x ) - t \} ^ { 2 } & = \{ y ( x ) - \mathbb { E } [ t | x ] + \mathbb { E } [ t | x ] - t \} ^ { 2 } \\ & = \ \{ y ( x ) - \mathbb { E } [ t | x ] \} ^ { 2 } + 2 \{ y ( x ) - \mathbb { E } [ t | x ] \} \{ \mathbb { E } [ t | x ] - t \} + \{ \mathbb { E } [ t | x ] - t \} ^ { 2 } \\ \text {where to keep the notation uncluttered, we use $\mathbb{E} [ t | x] \to \mathbf d e n o t e \mathbb{E} , [ t | x] \text { substiting} }
$$

where, to keep the notation uncluttered, we use E [ t | x ] to denote E t [ t | x ] . Substituting into the loss function and performing the integral over t , we see that the cross-term vanishes and we obtain an expression for the loss function in the form

$$
\text {vanishes and we obtain an expression for the loss function in the form } \\ \mathbb { E } [ L ] & = \int \{ y ( x ) - \mathbb { E } [ t | x ] \} ^ { 2 } \, p ( x ) \, d x + \int \{ \mathbb { E } [ t | x ] - t \} ^ { 2 } p ( x ) \, d x . \\ \intertext { The function y ( x ) \, we \, seek to determine enters only in the first term, which will be }
$$
