...
Figure 1.29 Plots of the quantity L q = | y − t | q for various values of q .
...
$$
H [ x ] = - \sum _ { x } p ( x ) \log _ { 2 } p ( x ) . \\ \text {quantity is called the entropy of the random variable } x . \ \text {Note that}
$$

This important quantity is called the entropy of the random variable x . Note that lim p → 0 p ln p = 0 and so we shall take p ( x )ln p ( x ) = 0 whenever we encounter a value for x such that p ( x ) = 0 .
