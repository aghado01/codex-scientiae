...
![The image is a graph titled p(x), which represents the probability of a particular event occurring. The graph shows a series of points, each representing a different value of x. The x-axis is labeled as p(x), and the y-axis is labeled as p(z). The graph shows a pattern of points that are connected by a series of lines. The points on the graph are connected by a series of lines, which are colored in blue and red. The lines are connected in a way that they form a pattern. The points are connected by a series of lines that are colored in red. The lines are connected in a way that they form a pattern. The graph is labeled as follows: - The x-axis is labeled as p(x). - The y-axis is labeled as p(z). The graph shows a pattern of points that are connected by a series of lines. The points are](../images/imageFile222.png)
...
points so that µ j = x n for some value of n . This data point will then contribute a term in the likelihood function of the form

$$
\mathcal { N } ( x _ { n } | x _ { n } , \sigma _ { j } ^ { 2 } I ) = \frac { 1 } { ( 2 \pi ) ^ { 1 / 2 } } \frac { 1 } { \sigma _ { j } } .
$$
