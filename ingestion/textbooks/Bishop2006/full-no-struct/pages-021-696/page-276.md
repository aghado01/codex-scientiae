...
and acting on these with the R{·} operator, we obtain expressions for the elements of the vector v T H

$$
\mathcal { R } \left \{ \frac { \partial E } { \partial w _ { k j } } \right \} & \ = \ \mathcal { R } \{ \delta _ { k } \} z _ { j } + \delta _ { k } \mathcal { R } \{ z _ { j } \} \\ \intertext { R } \mathcal { R } \left \{ \frac { \partial E } { \partial w _ { k j } } \right \} & \ = \ \mathcal { R } \{ \delta _ { k } \} z _ { j } + \delta _ { k } \mathcal { R } \{ z _ { j } \}
$$

$$
\Gamma \left ( \partial w _ { k j } \right ) & = x _ { i } \mathcal { R } \{ \delta _ { j } \} . \\ \mathcal { R } \left \{ \frac { \partial E } { \partial w _ { j i } } \right \} & = \ x _ { i } \mathcal { R } \{ \delta _ { j } \} . \\
$$

The implementation of this algorithm involves the introduction of additional variables R{ a j } , R{ z j } and R{ δ j } for the hidden units and R{ δ k } and R{ y k } for the output units. For each input pattern, the values of these quantities can be found using the above results, and the elements of v T H are then given by (5.110) and (5.111). An elegant aspect of this technique is that the equations for evaluating v T H mirror closely those for standard forward and backward propagation, and so the extension of existing software to compute this product is typically straightforward.
