...
As discussed in Section 3.1, the bias parameters in (5.2) can be absorbed into the set of weight parameters by deﬁning an additional input variable x 0 whose value is clamped at x 0 = 1 , so that (5.2) takes the form

$$
a _ { j } = \sum _ { i = 0 } ^ { D } w _ { j i } ^ { ( 1 ) } x _ { i } . \\ \intertext { o t h e s e d o n-l a r $ b i e s $ i n t a $ } \intertext { o t h e s e d o n-l a r $ b i e s $ i n t a $ }
$$
...
$$
\text {overall network function becomes} \\ y _ { k } ( x , w ) = \sigma \left ( \sum _ { j = 0 } ^ { M } w _ { k j } ^ { ( 2 ) } h \left ( \sum _ { i = 0 } ^ { D } w _ { j i } ^ { ( 1 ) } x _ { i } \right ) \right ) . \\ \intertext { c a n b e s e n from Figure 5.1, the neural network model comprises two stages }
$$
