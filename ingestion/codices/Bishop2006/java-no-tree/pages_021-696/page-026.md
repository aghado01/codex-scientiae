...
We can solve the curve ﬁtting problem by choosing the value of w for which E(w) is as small as possible. Because the error function is a quadratic function of the coefﬁcients w, its derivatives with respect to the coefﬁcients will be linear in the elements of w, and so the minimization of the error function has a unique solution,

- Exercise 1.1 denoted by w , which can be found in closed form. The resulting polynomial is given by the function y(x,w ).
