...
A distinct thread has been proposed by  Patel in [36] through the reinterpretation of the persistence diagram of a $\mathbf{Z}$-module as the  M\"obius inversion of its rank function. Patel's work was then extended by Kim and M\'emoli [25] to the setting of modules defined over any suitable locally finite poset. They generalized  the rank invariant via the *limit-to-colimit map* over subposets and then conveniently expressed its M\"obius inversion. In fact the limit-to-colimit map was suggested by Amit Patel to the authors of [25] who in [24] used it to define a notion of rank invariant for zigzag modules.  Chambers and Letscher [15] also  considered a notion of persistent homology over directed acyclic graphs using the limit-to-colimit map. Asashiba et al. [2] study  the case of modules defined on an $m\times n$ grid and propose a high-level algorithm for computing both their generalized rank function and their M\"obius inversions with the goal of providing an approximation of a given module by interval decomposables. Asashiba et al.  [1] tackle the interval decomposability of a given $\mathbf{Z}^d$-module via quiver representation theory.
...
In \S2.1, we review the notion of interval decomposability of persistence modules. In \S2.2, we review the notions of generalized rank invariant and generalized persistence diagram. In \S2.3, we discuss how to compute the limit and the colimit of a given functor $P\rightarrow \vect$.

### Persistence Modules and their decompositions

We fix a certain field $\mathbb{F}$ and every vector space in this paper is over $\mathbb{F}$. Let $\vect$ denote the category of *finite dimensional* vector spaces  and linear maps over $\mathbb{F}$.
...
For an interval $I$ of $P$, the **interval module** $\mathbb{I}_I:P\rightarrow \vect$ is defined as
...
**Definition 2.8.** The **$(\mathbf{Int})$-generalized persistence diagram of $M$** is the unique (The existence and uniqueness is guaranteed by properties of the M\"obius inversion formula [37, 38].) function $\mathrm{dgm}_{\mathbb{I}}(M):\mathbf{Int}(P)\rightarrow \mathbf{Z}$ that satisfies, for any $I\in \mathbf{Int}(P)$,
...
**Theorem 2.9.**  If a given $M:P\rightarrow\vect$ is interval decomposable, then for all $I\in \mathbf{Int}(P)$, $\mathrm{dgm}_{\mathbb{I}}(M)(I)$ is equal to the multiplicity of $I$ in $\mathrm{barc}(M)$.
...
[figure (pdf): intervals_new.pdf](2111.15058v3/intervals_new.pdf)
...
**Example 3.9.** Consider $M:\{(1,1),(1,2),(2,2),(2,1)\}(\subset \mathbf{Z}^2)\rightarrow \vect$ given as follows.


$$
![](2111.15058v3/diagram-2.svg)

\hspace{3mm}=\hspace{3mm}

![](2111.15058v3/diagram-3.svg)
$$
...
\begin{setup} In the rest of \S3.3, we fix both $I\in\mathbf{Int}(\mathbf{Z}^2)$ and  a functor $M:I\rightarrow \vect$. Each element in $\varprojlim M_{\partial I}$  is identified with the corresponding section of $M$ along $\partial I$. Also, we identify points in  (6) and (7) via $\iota_I$. \end{setup}

**Definition 3.11 (Zigzag module along $\partial I$).** Define the zigzag module $M_{\partial I}:\mathbf{ZZ}_{\partial I}\rightarrow \vect$ by $(M_{\partial I})_x:=M_{\iota_I(x)}$ for $x\in \mathbf{ZZ}_{\partial I}$ and $\varphi_{M_{\partial I}}(x, y):=\varphi_{M}(\iota_I(x), \iota_I(y))$ for $x\leq y$ in $\mathbf{ZZ}_{\partial I}$.
...
**Proposition 4.3.** Assume that a given $M:P\rightarrow \vect$ has the indecomposable decomposition $M\cong\bigoplus_{i=1}^m M_i$. Then, every summand $M_i$ is isomorphic to the interval module $\mathbb{I}_{P}$  if and only if $\mathrm{rk}_{\mathbb{I}}(M)(P)=\dim M_p=m$ for all $p\in P$.
...
**Problem 4.5.** Assume that $M_{\mathcal{F}}:P\rightarrow \vect$ is interval decomposable. Find $\mathrm{barc}(M_{\mathcal{F}})$.
...
[figure (pdf): example_modules3.pdf](2111.15058v3/example_modules3.pdf)

*Modules $M,N:\{1,2,3\}\times\{1,2\}\rightarrow \vect$. $M$ is interval decomposable, but $N$ is not.*

[figure (pdf): example_dgm2.pdf](2111.15058v3/example_dgm2.pdf)
...
It is possible that a functor does not have a colimit at all. However, if a functor does have a colimit then the initial property of the colimit guarantees its uniqueness up to isomorphism. For this reason, we sometimes refer to a colimit as *the* colimit of a functor. It is well-known that if $P$ is finite, then any functor $F:P\rightarrow \vect$ admits both limit and colimit in $\vect$. Assume that $P$ is also connected. Then, by the commutativity in (9) and (10), once a cone and a cocone of $F$ are specified, there exists the canonical map from the cone to the cocone, leading to Definition 2.5.

**Remark 6.5.** Let $P$ be a finite and connected poset. Let $Q$ be a finite and connected subposet of $P$. Let us fix any $F:P \rightarrow \vect$.

[label=(\roman*)]

- For any cone $\left(L', (\pi_p')_{p\in P}\right)$ over $F$, its restriction $\left(L', (\pi_p')_{p\in Q}\right)$ is a cone over the restriction $F|_Q:Q\rightarrow \vect$. Therefore, by the terminal property of the limit  $\left(\varprojlim F|_{Q}, (\pi_q)_{q\in Q}\right)$, there exists the unique morphism $u:L' \rightarrow \varprojlim F|_{Q}$ such that  $\pi_q'=\pi_q\circ u$ for all $q\in Q$.

- For any cocone $\left(C', (i_p')_{p\in P}\right)$ over $F$, its restriction $\left(C', (i_p')_{p\in Q}\right)$ is a cocone over the restriction $F|_Q:Q\rightarrow \vect$. Therefore, by the initial property of $\varinjlim F|_{Q}$, there exists the unique morphism $u:\varinjlim F|_{Q} \rightarrow C'$ such that $i'_q=u\circ i_q$ for all $q\in Q$.
...
1. Hideto Asashiba, Micka{ë}l Buchet, Emerson G Escolar, Ken Nakashima, and Michio Yoshiwaki. On interval decomposability of 2d persistence modules. *arXiv preprint arXiv:1812.05261v2*, 2018.
...
3. Gor{\^o} Azumaya. Corrections and supplementaries to my paper concerning {K}rull-{R}emak-{S}chmidt’s theorem. *Nagoya Mathematical Journal*, 1:117–124, 1950.
...
11. Chen Cai, Woojin Kim, Facundo M{é}moli, and Yusu Wang. Elder-rule-staircodes for augmented metric spaces. *SIAM Journal on Applied Algebra and Geometry*, 5(3):417–454, 2021.
12. Gunnar Carlsson and Vin de Silva. Zigzag persistence. *Foundations of computational mathematics*, 10(4):367–405, 2010.
13. Gunnar Carlsson and Facundo M{é}moli. Multiparameter hierarchical clustering methods. In *Classification as a Tool for Research*, pages 63–70. Springer, 2010.
...
16. J{é}r{é}my Cochoy and Steve Oudot. Decomposition of exact pfd persistence bimodules. *Discrete & Computational Geometry*, 63(2):255–293, 2020.
...
24. Woojin Kim and Facundo M{é}moli. Rank invariant for zigzag modules. *arXiv preprint arXiv:1810.11517v1*, 2018.
25. Woojin Kim and Facundo M{é}moli. Generalized persistence diagrams for persistence modules over posets. *Journal of Applied and Computational Topology*, 5(4):533–581, 2021.
26. Woojin Kim and Facundo M{é}moli. Spatiotemporal persistent homology for dynamic metric spaces. *Discrete & Computational Geometry*, 66(3):831–875, 2021.
27. Woojin Kim and Samantha Moore. The generalized persistence diagram encodes the bigraded {B}etti numbers. *arXiv preprint arXiv:2111.02551*, 2021.
...
34. Nikola Milosavljevi{ç}, Dmitriy Morozov, and Primoz Skraba. Zigzag persistent homology in matrix multiplication time. In *Proceedings of the twenty-seventh Annual Symposium on Computational Geometry*, pages 216–225, 2011.
...
37. Gian-Carlo Rota. On the foundations of combinatorial theory i. theory of {M}{ö}bius functions. { Zeitschrift f{ü}r Wahrscheinlichkeitstheorie und verwandte Gebiete}, 2(4):340–368, 1964.
