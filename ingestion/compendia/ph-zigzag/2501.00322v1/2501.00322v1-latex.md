...
[figure (pdf): posets.pdf](2501.00322v1/posets.pdf)
...
[figure (pdf): zigzag_cover_full.pdf](2501.00322v1/zigzag_cover_full.pdf)
...
We take a functorial perspective. A *persistence module* over a poset $P$ is a functor $F\colon P\to\ensuremath{\mathsf{Vec}}$ from $P$, viewed as a category, to the category $\ensuremath{\mathsf{Vec}}$ of finite-dimensional vector spaces over a fixed field $\Bbbk$. We denote by $\ensuremath{\mathsf{Vec}}^P$ the category of persistence modules over $P$ where the morphisms are natural transformations. We denote the internal maps of a persistence module $F$ by $F_{p\to q}$ for all $p\leq q \in P$. A persistence module $F$ is *indecomposable* if $F\cong A\oplus B$ then either $A = 0$ or $B = 0$.
...
$$
![](2501.00322v1/diagram-1.svg)
$$


\newcommand{\vartriangleleft}{\vartriangleleft} \newcommand{\vartriangleright}{\vartriangleright} \newcommand{\mathcal{B}}{\mathcal{B}} \newcommand{\mathrm{full}}{\mathrm{full}} By the result of Aoki, Escolar and Tada [3] mentioned in the introduction, every persistence module over $B$ decomposes into interval modules. It is straightforward to verify that an interval of $B$ belongs to one of the following types (see 3).
...
$$
[i,j]_{\vartriangleleft} \coloneqq \Set{z \in B %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} 0 \leq z \leq i} \cup \Set{z\in B %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
...
$$
[i,j]_{\vartriangleright} \coloneqq \Set{z \in B %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} i \leq z \leq n} \cup \Set{z\in B %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
...
[figure (pdf): intervals_2.pdf](2501.00322v1/intervals_2.pdf)
...
[figure (pdf): bipath_arccode.pdf](2501.00322v1/bipath_arccode.pdf)
...
The *zigzag poset* $\mathbb{ZZ}$ is the subposet of $\mathbb{Z}^{\mathrm{op}}\times\mathbb{Z}$ given by $\Set{(a, b) \in \mathbb{Z}^{\mathrm{op}}\times\mathbb{Z} % \nonscript\:\vert{} \allowbreak{} \nonscript\: \mathopen{} a = b \text{ or } a = b + 1}$. A zigzag module is a persistence module over the zigzag poset.
...
[figure (pdf): covering.pdf](2501.00322v1/covering.pdf)

*Illustration of the covering map $\zeta\colon\mathbb{ZZ}\to B$. In this case, $n = m = 4$*

The covering map $\zeta\colon\mathbb{ZZ}\to B$ induces a functor $R\colon \ensuremath{\mathsf{Vec}}^{B}\to\ensuremath{\mathsf{Vec}}^{\mathbb{ZZ}}$, called the *restriction functor*, by precomposition $M\mapsto M\circ\zeta$. It is easy to check, since our persistence modules are pointwise finite-dimensional, the restriction functor $R$ preserves direct sums.

Shortly, we describe the arc code of the bipath module $M$ in terms of the barcode of $R(M)$. Indeed, persistence modules over $\mathbb{ZZ}$ (when they are pointwise finite-dimensional, like here) have a barcode, that is, for every module $Z\in\ensuremath{\mathsf{Vec}}^{\mathbb{ZZ}}$ there is a unique multiset $\mathcal{B}(Z)$ of intervals such that $Z\cong \bigoplus_{I\in \mathcal{B}(Z)} \Bbbk I$, by a result of Botnan [12].
...
$$
\begin{aligned}
&[a,b]_{\mathbb{ZZ}} \coloneqq \Set{(c,d)\in\mathbb{ZZ} %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} c\leq b, d\geq a},&[a, b)_{\mathbb{ZZ}} &\coloneqq \Set{(c,d)\in\mathbb{ZZ} %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} a\leq d < b},\\
  ...
  \allowbreak{}
  \nonscript\:
  \mathopen{} a < c \leq b},&(a, b)_{\mathbb{ZZ}} &\coloneqq \Set{(c, d)\in\mathbb{ZZ}%
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} c > a, d < b}.
...
We describe the image under the restriction functor $R\colon \ensuremath{\mathsf{Vec}}^{B}\to\ensuremath{\mathsf{Vec}}^{\mathbb{ZZ}}$ of the different types of intervals of a bipath $B$ in the following lemma, illustrated in 6.
...
[figure (pdf): zz_decomp.pdf](2501.00322v1/zz_decomp.pdf)
...
We review interleavings over $\RR$ and the bottleneck distance. Recall that for an $\epsilon\in [0, \infty]$, we define the *shift functor* $(\cdot)(\epsilon)\colon \ensuremath{\mathsf{Vec}}^{\RR} \to \ensuremath{\mathsf{Vec}}^{\RR}$ by sending $M$ to the persistence module given by $M(\epsilon)_{(x, y)} \coloneqq M_{(x-\epsilon, y+\epsilon)}$ and $M(\epsilon)_{(x, y)\to (w, z)} \coloneqq M_{(x-\epsilon, y+\epsilon)\to (w-\epsilon, z+\epsilon)}$. On morphisms $f\colon M\to N$, we set $f(\epsilon)_{(x, y)}\coloneqq f_{(x-\epsilon, y+\epsilon)}$. For every module $M$, we define a morphism $\phi^M_\epsilon\colon M\to M(\epsilon)$ by the internal maps $M_{(x, y)\to (x-\epsilon, y+\epsilon)}$. We say that two persistence modules $M, N\in\ensuremath{\mathsf{Vec}}^{\RR}$ are *$\epsilon$-interleaved* if there exist morphisms $f\colon M\to N(\epsilon)$ and $g\colon N\to M(\epsilon)$ such that $g(\epsilon)\circ f = \phi^M_{2\epsilon}$ and $f(\epsilon)\circ g = \phi^N_{2\epsilon}$.

As in [10], we describe matchings in terms of interleavings between interval modules. Let $A$ and $B$ be two multisets of intervals of $\RR$. An *$\epsilon$-matching* between $A$ and $B$ is given by a bijection $\sigma\colon A'\to B'$ for some subsets $A'\subset A$ and $B'\subset B$ such that
...
Then, the *interleaving distance* $d_{I}(M, N)$ between two persistence modules $M, N\colon$ $\RR\to\ensuremath{\mathsf{Vec}}$ and the *bottleneck distance* $d_{B}(C, D)$ between two multisets of intervals $C$ and $D$ are given by


$$
\begin{aligned}
d_{I}(M, N) &\coloneqq \inf\Set{\epsilon %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} \text{$M$ and $N$ are $\epsilon$-interleaved}},\\
  d_{B}(C, D) &\coloneqq \inf\Set{\epsilon %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
...
Let ${\mathbb U}$ be the subposet of $\RR$ given by $\Set{(a, b)\in \RR % \nonscript\:\vert{} \allowbreak{} \nonscript\: \mathopen{} a\leq b}$. As in [11, 10], we are interested in the following four types of intervals of ${\mathbb U}$.
...
$$
\begin{aligned}
&[a,b]_{\operatorname{BL}} \coloneqq \Set{(c, d)\in {\mathbb U} %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} c\le b, d\ge a}, &[a, b)_{\operatorname{BL}} &\coloneqq \Set{(c, d)\in {\mathbb U} %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} a\le d<b},\\
&(a, b]_{\operatorname{BL}} \coloneqq \Set{(c, d)\in {\mathbb U} %
  ...
  \allowbreak{}
  \nonscript\:
  \mathopen{} a<c\le b}, &(a, b)_{\operatorname{BL}} &\coloneqq \Set{(c, d)\in {\mathbb U} %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
  \mathopen{} c>a, d<b}.
...
Consider the inclusion $\iota\colon\mathbb{ZZ}\hookrightarrow\RR$. By composing first the left Kan extension along $\iota$ and then the restriction functor of the inclusion ${\mathbb U}\to\RR$, one obtains the *block extension functor* $E\colon\ensuremath{\mathsf{Vec}}^{\mathbb{ZZ}}\to\ensuremath{\mathsf{Vec}}^{{\mathbb U}}$; we refer to [11, 10] for more context on this functor and the definitions involved. The block extension functor preserves direct sums, and sends barcodes of zigzag modules to barcodes of block-decomposable modules, by which we mean the following. See 7 for an illustration.
...
[figure (pdf): blocks_zigzag.pdf](2501.00322v1/blocks_zigzag.pdf)
...
Recall the restriction functor $R\colon \ensuremath{\mathsf{Vec}}^{B}\to\ensuremath{\mathsf{Vec}}^{\mathbb{ZZ}}$ induced by the covering map defined in 2.1. Also recall that by 2.3, $R$ preserves arc codes up to the action of $\mathbb{Z}$. In turn, and in parallel to the definition of interleaving and bottleneck distances in zigzag modules, for two bipath modules $M$ and $N$, we define
...
*Proof.* Take an $\epsilon$-matching between $\mathcal{B}(R(M))$ and $\mathcal{B}(R(N))$ given by the bijection $\tau\colon E\to F$. Suppose that there exist some $z\in \mathbb{Z}$ and $I\in E$ such that $\tau(z\uparrow I) \neq z\uparrow\tau(I)$. For such the interval $I$, we can construct a new $\epsilon$-matching given by the bijection $\sigma\colon E\cup\Set{z'\uparrow I% \nonscript\:\vert{} \allowbreak{} \nonscript\: \mathopen{} z'\in\mathbb{Z}}\to F\cup\Set{z'\uparrow \tau(I)% \nonscript\:\vert{} \allowbreak{} \nonscript\: \mathopen{} z'\in\mathbb{Z}}$, where $\sigma(z'\uparrow I)\coloneqq z'\uparrow \tau(I)$ for every $z'\in\mathbb{Z}$ and $\sigma(J)\coloneqq \tau(J)$ for any $J\in E$ that is not in the orbit $\Set{z'\uparrow I% \nonscript\:\vert{} \allowbreak{} \nonscript\: \mathopen{} z'\in\mathbb{Z}}$ of $I$. We note the reader that $\sigma(I) = \sigma(0\uparrow I) = 0\uparrow \tau(I) = \tau(I)$ by the definition, it then follows that $\sigma(z'\uparrow I) = z'\uparrow \sigma(I)$ for every $z'\in \mathbb{Z}$. Finally, since there is a finite number of orbits in $\mathcal{B}(R(M))$, we can repeat the procedure just described to obtain the desired $\epsilon$-matching.
...
$$
d_{B}(M, N) \coloneqq \inf\Set{\epsilon %
  \nonscript\:\vert{}
  \allowbreak{}
  \nonscript\:
...
$$
\begin{aligned}
M \cong & (\Bbbk B)^{\mu_M(B)}\ds \bigoplus\limits\Bbbk [i, j]_{\vartriangleleft}^{\mu_M([i, j]_{\vartriangleleft})} \ds \bigoplus\limits\Bbbk [i, j]_{\vartriangleright}^{\mu_M([i, j]_{\vartriangleright})}\\
    &\quad\quad \ds \bigoplus\limits\Bbbk [i, j]_{\top}^{\mu_M([i, j]_{\top})}\ds \bigoplus\limits\Bbbk [i, j]_{\bot}^{\mu_M([i, j]_{\bot})}.
\end{aligned}
$$
...
$$
\begin{aligned}
{R}'(M) \cong & {R}'(\Bbbk B)^{\mu_{{R}'(M)}(\mathbb{ZZ})}\ds \bigoplus\limits {R}'(\Bbbk [i, j]_{\vartriangleleft})^{\mu_M([i, j]_{\vartriangleleft})} \ds \bigoplus\limits {R}'(\Bbbk [i, j]_{\vartriangleright})^{\mu_M([i, j]_{\vartriangleright})}\nonumber\\
    &\quad\quad \ds \bigoplus\limits {R}'(\Bbbk [i, j]_{\top})^{\mu_M([i, j]_{\top})}\ds \bigoplus\limits {R}'(\Bbbk [i, j]_{\bot})^{\mu_M([i, j]_{\bot})}.
\end{aligned}
$$
...
\Bbbk (-n-m+j, i+1)_{\mathbb{ZZ}}\ds \Bbbk (j-1, n+m)_{\mathbb{ZZ}}, & j \neq 0.
\end{cases}
$$
...
\Bbbk [i, j]_{\mathbb{ZZ}}\ds \Bbbk (-m+1, -n-m+j+1]_{\mathbb{ZZ}}, & j \neq n.
\end{cases}
$$
...
- For the bottom interval $[i,j]_{\bot}$, ${R}'(\Bbbk [i,j]_{\bot}) = \Bbbk (i-1,j]_{\mathbb{ZZ}}\ds \Bbbk (-n-m+i, -n-m+j+1]_{\mathbb{ZZ}}$.
...
Lesnick and Wright [22] introduced the *fibered barcode*, an invariant of persistence modules over $\mathbb{R}^{2}$ given by the collection of barcodes of all 1-D affine slices. In detail, denoting by $L$ the collection of all (unparametrized) lines in $\mathbb{R}^{2}$ with non-negative slope and for a fixed persistence module $M\colon\mathbb{R}^{2}\to\ensuremath{\mathsf{Vec}}$, each line $l\in L$ induces a single-parameter persistence module $M|_{l} = M\circ i_{L}$ along the inclusion $i_{L}\colon l\hookrightarrow\mathbb{R}^{2}$. The *fibered barcode* of $M$ is then the assignment of each line $l\in L$ to the barcode of $M|_{l}$.

Closely following the construction of the fibered barcode, and due to bipath modules having arc codes, given a bipath poset $B$ and a persistence module $M\colon\mathbb{R}^{2}\to\ensuremath{\mathsf{Vec}}$, an order-preserving map $f\colon B\to\mathbb{R}^{2}$ induces a bipath module $M|_{f} = M\circ f$ and we can define the *fibered arc code*:
...
$$
![](2501.00322v1/diagram-5.svg)


\text{and}


![](2501.00322v1/diagram-6.svg)
...
$$
![](2501.00322v1/diagram-7.svg)


\text{and}


![](2501.00322v1/diagram-8.svg)
...
[figure (pdf): fibered_barcodes.pdf](2501.00322v1/fibered_barcodes.pdf)
...
$$
![](2501.00322v1/diagram-9.svg)


\text{and}


![](2501.00322v1/diagram-10.svg)
...
$$
![](2501.00322v1/diagram-11.svg)


\text{and}


![](2501.00322v1/diagram-12.svg)
...
[figure (pdf): Box_Example_M.pdf](2501.00322v1/Box_Example_M.pdf)

\quad\quad

[figure (pdf): Box_Example_N.pdf](2501.00322v1/Box_Example_N.pdf)
...
[figure (pdf): fibered_bipath.pdf](2501.00322v1/fibered_bipath.pdf)

yields that arc codes $\mathcal{B}(M|_{f})$ and $\mathcal{B}(N|_{f})$ share the same intervals as illustrated below:

[figure (pdf): same_fibered_arccodes.pdf](2501.00322v1/same_fibered_arccodes.pdf)
...
[figure (pdf): interval_in_CL4.pdf](2501.00322v1/interval_in_CL4.pdf)

*An interval of the grid*

## References

1. {\'A}ngel Javier Alonso and Michael Kerber. Decomposition of {{Zero-Dimensional Persistence Modules}} via {{Rooted Subsets}}. *Discrete & Computational Geometry*, November 2024. \href {https://doi.org/10.1007/s00454-024-00700-7} {\path{doi:10.1007/s00454-024-00700-7}}.
2. {\'A}ngel Javier Alonso, Michael Kerber, and Primoz Skraba. Probabilistic {{Analysis}} of {{Multiparameter Persistence Decompositions}} into {{Intervals}}. *LIPIcs, Volume 293, SoCG 2024*, 293:6:1–6:19, 2024. \href {https://doi.org/10.4230/LIPICS.SOCG.2024.6} {\path{doi:10.4230/LIPICS.SOCG.2024.6}}.
3. Toshitaka Aoki, Emerson G. Escolar, and Shunsuke Tada. Summand-{I}njectivity of {I}nterval {C}overs and {M}onotonicity of {I}nterval {R}esolution {G}lobal {D}imensions, November 2023. \href {https://arxiv.org/abs/2308.14979} {\path{arXiv:2308.14979}}.
4. Toshitaka Aoki, Emerson G. Escolar, and Shunsuke Tada. Bipath {P}ersistence. *Japan Journal of Industrial and Applied Mathematics*, December 2024. \href {https://doi.org/10.1007/s13160-024-00681-3} {\path{doi:10.1007/s13160-024-00681-3}}.
5. Hideto Asashiba, Micka{ë}l Buchet, Emerson G. Escolar, Ken Nakashima, and Michio Yoshiwaki. On {I}nterval {D}ecomposability of {{2D}} {P}ersistence {M}odules. *Computational Geometry*, 105–106:101879, August 2022. \href {https://doi.org/10.1016/j.comgeo.2022.101879} {\path{doi:10.1016/j.comgeo.2022.101879}}.
6. Hideto Asashiba, Emerson G. Escolar, Ken Nakashima, and Michio Yoshiwaki. Approximation by {I}nterval-{D}ecomposables and {I}nterval {R}esolutions of {P}ersistence {M}odules. *Journal of Pure and Applied Algebra*, 227(10):107397, October 2023. \href {https://arxiv.org/abs/2207.03663} {\path{arXiv:2207.03663}}, \href {https://doi.org/10.1016/j.jpaa.2023.107397} {\path{doi:10.1016/j.jpaa.2023.107397}}.
7. Hideto Asashiba, Etienne Gauthier, and Enhao Liu. Interval {{Replacements}} of {{Persistence Modules}}, June 2024. \href {https://arxiv.org/abs/2403.08308} {\path{arXiv:2403.08308}}.
8. Hideto Asashiba and Enhao Liu. Interval {{Multiplicities}} of {{Persistence Modules}}, November 2024. \href {https://arxiv.org/abs/2411.11594} {\path{arXiv:2411.11594}}.
9. Ulrich Bauer and Luis Scoccola. Generic {{Multi-Parameter Persistence Modules}} are {{Nearly Indecomposable}}, October 2023. \href {https://arxiv.org/abs/2211.15306} {\path{arXiv:2211.15306}}.
10. H{\aa}vard Bakke Bjerkevik. On the {{Stability}} of {{Interval Decomposable Persistence Modules}}. *Discrete & Computational Geometry*, 66(1):92–121, July 2021. \href {https://doi.org/10.1007/s00454-021-00298-0} {\path{doi:10.1007/s00454-021-00298-0}}.
11. Magnus Botnan and Michael Lesnick. Algebraic {S}tability of {Z}igzag {P}ersistence {M}odules. *Algebraic & Geometric Topology*, 18(6):3133–3204, October 2018. \href {https://doi.org/10.2140/agt.2018.18.3133} {\path{doi:10.2140/agt.2018.18.3133}}.
12. Magnus Bakke Botnan. Interval {D}ecomposition of {I}nfinite {Z}igzag {P}ersistence {M}odules. *Proceedings of the American Mathematical Society*, 145(8):3571–3577, January 2017. \href {https://doi.org/10.1090/proc/13465} {\path{doi:10.1090/proc/13465}}.
13. Magnus Bakke Botnan, Steffen Oppermann, and Steve Oudot. Signed {{Barcodes}} for {{Multi-Parameter Persistence}} via {{Rank Decompositions}}. In { {{LIPIcs}}, {{Volume}} 224, {{SoCG}} 2022}, volume 224, pages 19:1–19:18. Schloss Dagstuhl – Leibniz-Zentrum f{ü}r Informatik, 2022. \href {https://doi.org/10.4230/LIPICS.SOCG.2022.19} {\path{doi:10.4230/LIPICS.SOCG.2022.19}}.
14. Magnus Bakke Botnan, Steffen Oppermann, and Steve Oudot. Signed {{Barcodes}} for {{Multi-parameter Persistence}} via {{Rank Decompositions}} and {{Rank-Exact Resolutions}}. *Foundations of Computational Mathematics*, September 2024. \href {https://doi.org/10.1007/s10208-024-09672-9} {\path{doi:10.1007/s10208-024-09672-9}}.
15. Dan Burghelea and Tamal K. Dey. Topological {{Persistence}} for {{Circle-Valued Maps}}. *Discrete & Computational Geometry*, 50(1):69–98, July 2013. \href {https://doi.org/10.1007/s00454-013-9497-x} {\path{doi:10.1007/s00454-013-9497-x}}.
16. Chen Cai, Woojin Kim, Facundo Memoli, and Yusu Wang. Elder-{{Rule-Staircodes}} for {{Augmented Metric Spaces}}. *SIAM Journal on Applied Algebra and Geometry*, 5(3):417–454, January 2021. \href {https://doi.org/10.1137/20M1353605} {\path{doi:10.1137/20M1353605}}.
17. Tamal K. Dey and Tao Hou. Fast {{Computation}} of {{Zigzag Persistence}}. *LIPIcs, Volume 244, ESA 2022*, 244:43:1–43:15, 2022. \href {https://doi.org/10.4230/LIPICS.ESA.2022.43} {\path{doi:10.4230/LIPICS.ESA.2022.43}}.
18. Tamal K. Dey, Woojin Kim, and Facundo M{é}moli. Computing {{Generalized Rank Invariant}} for 2-{{Parameter Persistence Modules}} via {{Zigzag Persistence}} and {{Its Applications}}. *Discrete & Computational Geometry*, 71(1):67–94, January 2024. \href {https://doi.org/10.1007/s00454-023-00584-z} {\path{doi:10.1007/s00454-023-00584-z}}.
19. Xiaowen Gao and Minghui Zhao. Isometry {Theorem} for {Continuous} {Quiver} of {Type} $\tilde{A}$, December 2024. \href {https://arxiv.org/abs/2412.12462} {\path{arXiv:2412.12462}}, \href {https://doi.org/10.48550/arXiv.2412.12462} {\path{doi:10.48550/arXiv.2412.12462}}.
20. Eric J. Hanson and Job Daisie Rock. Decomposition of {P}ointwise {F}inite-{D}imensional {$\mathbb{S}^1$} {P}ersistence {M}odules. *Journal of Algebra and Its Applications*, 23(03):2450054, March 2024. \href {https://doi.org/10.1142/S0219498824500543} {\path{doi:10.1142/S0219498824500543}}.
21. Woojin Kim and Facundo M{é}moli. Generalized {P}ersistence {D}iagrams for {P}ersistence {M}odules over {P}osets. *Journal of Applied and Computational Topology*, 5(4):533–581, December 2021. \href {https://doi.org/10.1007/s41468-021-00075-1} {\path{doi:10.1007/s41468-021-00075-1}}.
22. Michael Lesnick and Matthew Wright. Interactive {{Visualization}} of 2-{{D Persistence Modules}}, December 2015. \href {https://arxiv.org/abs/1512.00180} {\path{arXiv:1512.00180}}.
23. Francesco Sala and Olivier Schiffmann. Fock {{Space Representation}} of the {{Circle Quantum Group}}. *International Mathematics Research Notices*, 2021(22):17025–17070, November 2021. \href {https://doi.org/10.1093/imrn/rnz268} {\path{doi:10.1093/imrn/rnz268}}.
24. Shunsuke Tada. A {C}omputation of {B}ipath {P}ersistent {H}omology and {B}ipath {P}ersistence {D}iagrams. Seminar presented at *Asia Pacific Seminar on Applied Topology and Geometry*, September 2024. Slides available at \url{https://shunsuketada1357.github.io/documents/slide2024-09-20.pdf}. Recording movie available at \url{https://drive.google.com/drive/u/0/folders/1MB4wFxrQe6ph9hBde8CLGWiBSYZya6Pf}.
