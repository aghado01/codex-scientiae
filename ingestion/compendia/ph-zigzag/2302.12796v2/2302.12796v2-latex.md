...
$$
\mathcal{F}:\emptyset= G_0 \xhookrightarrow{\;\sG_0\;}
G_1 \xhookrightarrow{\;\sG_1\;}
\cdots
\cdots \xhookrightarrow{\;\sG_{m-1}\;} G_m,
$$
...
- [$\sG_{i}$ is a vertex:] $\mathsf{MF}^{i+1}(\mathcal{F})$ equals $\mathsf{MF}^{i}(\mathcal{F})$ union an isolated leaf at level $i$ corresponding to $\sG_i$.

- [$\sG_{i}$ is a positive edge:] Set $\mathsf{MF}^{i+1}(\mathcal{F})=\mathsf{MF}^{i}(\mathcal{F})$.

- [$\sG_{i}$ is a negative edge:] Let $\sG_i=(u,v)$. Since $u$ and $v$ are in different connected components $C_1$ and $C_2$ in $G_i$, let $T_1,T_2$ be the trees in $\mathsf{MF}^{i}(\mathcal{F})$ corresponding to $C_1,C_2$ respectively. To form $\mathsf{MF}^{i+1}(\mathcal{F})$, we add an internal node at level $i$ (corresponding to $\sG_i$) to $\mathsf{MF}^{i}(\mathcal{F})$ whose children are the roots of $T_1$ and $T_2$.
...
In the above operation, the addition of two simplices $\sG$ and $\tau$ are switched from $\mathcal{F}$ to $\mathcal{F}'$. We also require that $\sG\not\subseteq\tau$ [5] ($\sG$ is not a vertex of the edge $\tau$) because otherwise $G'_i$ is not a valid graph.
...
We also notice the following fact about the change on pairing caused by the switch in Equation (2) when $\sG,\tau$ are both positive or both negative. Let $\sG$ be paired with $\sG'$ and $\tau$ be paired with $\tau'$ in $\mathcal{F}$. (If $\sG$ or $\tau$ are *unpaired*, then let $\sG'$ or $\tau'$ be $null$.) By the update algorithm for general complexes [5], either (i) the pairing for $\mathcal{F}$ and $\mathcal{F}'$ stays the same, or (ii) the only difference on the pairing is that $\sG$ is paired with $\tau'$ and $\tau$ is paired with $\sG'$ in $\mathcal{F}'$.
...
[figure (pdf): e1-e2-same-conns.pdf](2302.12796v2/e1-e2-same-conns.pdf)
...
[figure (pdf): e-e-sw-a.pdf](2302.12796v2/e-e-sw-a.pdf)

} \subfloat[]{

[figure (pdf): e-e-sw-b.pdf](2302.12796v2/e-e-sw-b.pdf)

} \subfloat[]{

[figure (pdf): e-e-sw-c.pdf](2302.12796v2/e-e-sw-c.pdf)
...
[figure (pdf): C1-C2-C3-1.pdf](2302.12796v2/C1-C2-C3-1.pdf)

}

\subfloat[]{

[figure (pdf): C1-C2-C3-2.pdf](2302.12796v2/C1-C2-C3-2.pdf)
...
In all cases, the algorithm also updates the levels of the leaves in $\mathbb{T}$ corresponding to $\sG$ and $\tau$ (if such leaves exist) due to the change of indices for the vertices. Notice that the positivity/negativity of simplices can be easily read off from the simplex pairing $\Pi$.
...
If the switch is a vertex-vertex switch, let $v_1:=\sG$, $v_2:=\tau$. If $v_1,v_2$ are in the same tree in $\mathbb{T}$, then do the following:
...
If the switch is an edge-edge switch, let $e_1:=\sG$, $e_2:=\tau$. We have the following sub-cases:
...
**Proposition 2.** For the switch operation in Equation (2) where $v_1:=\sG$ and $v_2:=\tau$ are vertices ($v_1,v_2$ are thus both positive), the pairings for $\mathcal{F}$ and $\mathcal{F}'$  change if and only if the following two conditions hold:
...
**Proposition 3.** For the switch operation in Equation (2) where $v_1:=\sG$ and $v_2:=\tau$ are both vertices, $\mathsf{MF}(\mathcal{F})$ and $\mathsf{MF}(\mathcal{F}')$ have the same structure, with the only difference being on the levels of $v_1$, $v_2$ due to the index change of simplices.
...
[figure (pdf): v-e-sw.pdf](2302.12796v2/v-e-sw.pdf)
...
[figure (pdf): convert.pdf](2302.12796v2/convert.pdf)
...
- \textit{{Forward}} switch is the counterpart of the switch on standard filtrations, which swaps two forward inclusions (i.e., additions) and also requires $\sG\nsubseteq\tau$:
...
The switch from $\mathcal{F}$ to $\mathcal{F}'$ is called an \textit{{outward}} switch and the switch from $\mathcal{F}'$ to $\mathcal{F}$ is called an \textit{{inward}} switch. We also require $\sG\neq\tau$ because if $\sG=\tau$, e.g., for outward switch, we cannot delete $\tau$ from $G_{i-1}$ in $\mathcal{F}'$ because $\tau\not\in G_{i-1}$.
...
**Algorithm 4.** We describe the algorithm for the forward switch in Equation (9). The procedure for a backward switch on an up-down filtration is symmetric. The algorithm maintains a set of edge-edge pairs $\Pi$ initially for ${\mathcal U}$. It also maintains a representative cycle for each edge-edge pair in $\Pi$. After the processing, edge-edge pairs in $\Pi$ and their representatives correspond to ${\mathcal U}'$. As mentioned, a switch containing a vertex makes no changes to the edge-edge pairs (see Algorithm 2). So, suppose that the switch is an edge-edge switch and let $e_1:=\hat{\sG}$, $e_2:=\hat{\tau}$. Moreover, let ${\mathcal U}_u$ be the ascending part of ${\mathcal U}$. We have the following cases:
...
We now describe how to address the connectivity query as in Observation 17 for $\mathcal{U}$ (the query for $\mathcal{U}'$ can be done similarly by first changing the data structures we maintain for $\mathcal{U}$ into ones for $\mathcal{U}'$). One way to solve such a connectivity problem for $\mathcal{U}$ is to maintain an MSF for each $\hat{G}_\lG$ in the descending part $\mathcal{U}_d$ of $\mathcal{U}$ ($\lG\geqk$), where edges in  $\hat{G}_\lG$ are weighted by their indices in the ascending part $\mathcal{U}_u$. Then for a $\hat{G}_\gamma$ in $\mathcal{U}_u$ ($\gamma<k$), determining whether two vertices $u,v$ are connected in $\hat{G}_\gamma\intersect\hat{G}_\lG$ can be done by querying the bottleneck weight of the path connecting $u,v$ in the MSF of $\hat{G}_\lG$. Representing the MSF's as Link-Cut trees [20], such a query can be done in $O(\log m)$ time.
...
Now, to obtain the MSF for an arbitrary $\hat{G}_\lG$ in $\mathcal{U}_d$, we first find the  graph $\hat{G}_{m-q\sqrt{m}}$ with the smallest index which is contained in $\hat{G}_\lG$. Then, from the MSF of $\hat{G}_{m-q\sqrt{m}}$, we go over each edge $e$ between $\hat{G}_{m-q\sqrt{m}}$ and $\hat{G}_\lG$ in $\mathcal{U}_d$ and see whether $e$ should be inserted into the MSF of $\hat{G}_{m-q\sqrt{m}}$. We can get the MSF for $\hat{G}_\lG$ in $O(\sqrt{m}\,\log m)$ time using the above process where no more than $\sqrt{m}$ edges need to be processed. Notice that we can also understand the above process as building the MSF for $\hat{G}_\lG$ using Kruskal's algorithm starting from the `already built'  MSF for $\hat{G}_{m-q\sqrt{m}}$, where Link-Cut trees [20] are used for checking the connectivity.

Notice that after making queries on the MSF of $\hat{G}_\lG$, we need to recover the MSF of $\hat{G}_{m-q\sqrt{m}}$ so that exactly the MSF's of graphs in Sequence (10) are maintained. The MSF of $\hat{G}_{m-q\sqrt{m}}$ can be recovered by recording what edges are added to the original MSF of $\hat{G}_{m-q\sqrt{m}}$ to form the MSF of $\hat{G}_\lG$, and then deleting those edges from the MSF of $\hat{G}_\lG$ using the **cut** operation in Link-Cut trees [20]. Since no more than $\sqrt{m}$ edges need to be deleted, recovering the MSF of $\hat{G}_{m-q\sqrt{m}}$ takes $O(\sqrt{m}\,\log m)$ time.
...
1. Nicolas Berkouk and Luca Nyckees. One diamond to rule them all: Old and new topics about zigzag, levelsets and extended persistence, 2022. URL: \url{https://arxiv.org/abs/2210.00916}, \href {http://dx.doi.org/10.48550/ARXIV.2210.00916} {\path{doi:10.48550/ARXIV.2210.00916}}.
...
4. David Cohen-Steiner, Herbert Edelsbrunner, and John Harer. Extending persistence using {P}oincar{é} and {L}efschetz duality. *Foundations of Computational Mathematics*, 9(1):79–103, 2009.
...
8. Tamal K. Dey and Tao Hou. Fast computation of zigzag persistence. In { 30th Annual European Symposium on Algorithms, {ESA} 2022, September 5-9, 2022, Berlin/Potsdam, Germany}, volume 244 of *LIPIcs*, pages 43:1–43:15. Schloss Dagstuhl - Leibniz-Zentrum f{ü}r Informatik, 2022.
9. Tamal K. Dey and Yusu Wang. *Computational Topology for Data Analysis*. Cambridge University Press, 2022.
10. Herbert Edelsbrunner and John Harer. { Computational topology: {A}n introduction}. American Mathematical Soc., 2010.
...
17. Cl{é}ment Maria and Steve Y. Oudot. Zigzag persistence via reflections and transpositions. In *Proceedings of the Twenty-Sixth Annual ACM-SIAM Symposium on Discrete Algorithms*, pages 181–199. SIAM, 2014.
18. Nikola Milosavljevi{ç}, Dmitriy Morozov, and Primoz Skraba. Zigzag persistent homology in matrix multiplication time. In *Proceedings of the Twenty-Seventh Annual Symposium on Computational Geometry*, pages 216–225, 2011.
...
22. Simon Zhang, Soham Mukherjee, and Tamal K. Dey. {GEFL:} {E}xtended filtration learning for graph classification. In Bastian Rieck and Razvan Pascanu, editors, *Learning on Graphs Conference, LoG 2022*, volume 198 of *Proceedings of Machine Learning Research*, page 16. {PMLR}, 2022. URL: \url{https://proceedings.mlr.press/v198/zhang22b.html}.
