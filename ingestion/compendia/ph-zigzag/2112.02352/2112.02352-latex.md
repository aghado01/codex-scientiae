...
\stepcounter{Item}\refstepcounter{desccounter}
- [Case \Alph{desccounter} ($b<i,d=i$)]: The corresponding interval in $\mathsf{Pers}_{*}(\mathcal{F}')$ is $[b,i-1]$. We have that $z_{i-1}+z_{i}=\partial(c_{i-1})$ and $z_{i}=\partial(c_{i})$ for $c_{i-1}\subseteq K_{i-1}\subseteq K'_i$ and $c_{i}\subseteq K_{i+1}\subseteq K'_i$. So $z_{i-1}=\partial(c_{i-1}+c_{i})$. Since $\tau\in c_{i}$ and $\tau\not\in c_{i-1}$ (because $\tau\not\in K_{i-1}$), it is true that $\tau\in c_{i-1}+c_{i}$. Then, the representative for $[b,i-1]\in\mathsf{Pers}_{*}(\mathcal{F}')$ is:
