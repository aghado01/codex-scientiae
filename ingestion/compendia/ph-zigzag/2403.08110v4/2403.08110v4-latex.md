...
[figure (pdf): Genrank.pdf](2403.08110v4/Genrank.pdf)
...
[figure (pdf): UnfoldingEx.pdf](2403.08110v4/UnfoldingEx.pdf)
...
**Definition 3.5.** For a folding $s: Q \rightarrow \mathrm{Fld}_s(Q)$ and a $Q$-module $\mathbb{N}$, a summand $\mathbb{N}'$ in a decomposition $\mathbb{N}=\mathbb{N}'\oplus \mathbb{N}''$ is called *invertible* if there exists a foldable summand $\Q$ of $\mathbb{N}$ so that $\mathbb{N}=\Q\oplus \mathbb{N}''$.
...
[figure (pdf): UnfoldingEx3.pdf](2403.08110v4/UnfoldingEx3.pdf)
...
(3) First, assume that $\mathrm{Fld}_s(\mathbb{N}_1)$ exists and is a summand of $\mathbb{M}$. Then, we have $\mathbb{M}=\mathrm{Fld}_s \mathbb{N}_1\oplus \overline{\mathrm{Fld}_s \mathbb{N}_1}$. Applying (2) above, we have $\mathbb{N}=\mathrm{Fld}_s^{-1}(\mathrm{Fld}_s\mathbb{N}_1)\oplus \mathrm{Fld}_s^{-1}(\overline{\mathrm{Fld}_s \mathbb{N}_1}) = \mathbb{N}_1\oplus \mathrm{Fld}_s^{-1}(\overline{\mathrm{Fld}_s \mathbb{N}_1})$. Let $\Q=\mathrm{Fld}_s^{-1}(\overline{\mathrm{Fld}_s \mathbb{N}_1})$ which is foldable. Then,


$$
\begin{aligned}
\mathbb{N}_1\oplus \overline{\mathbb{N}_1}=\mathbb{N}=\mathbb{N}_1\oplus \Q
\end{aligned}
$$
...
Now, assume that $\overline{\mathbb{N}_1}$ is invertible. By definition, $\mathbb{N}=\mathbb{N}_1\oplus\Q$ for some foldable summand $\Q$. Apply (1) above to conclude that $\mathrm{Fld}_s(\mathbb{N}_1)$ exists and is a summand of $\mathbb{M}$.
...
[figure (pdf): UnfoldingEx2.pdf](2403.08110v4/UnfoldingEx2.pdf)
...
[figure (pdf): unfoldingPath2.pdf](2403.08110v4/unfoldingPath2.pdf)
...
[figure (pdf): MatrixReduct.pdf](2403.08110v4/MatrixReduct.pdf)
...
To check invertibility of $\overline{\mathbb{I}}$, we do the following: first, we observe that $\mathrm{Fld}_s(\mathbb{I})$ is well defined because $\mathbb{I}$ is already converted to be foldable. Consider the quotient module $\Q=\mathbb{M}/(\mathrm{Fld}_s(\mathbb{I}))$. If $\Q$ is a submodule of $\mathbb{M}$, we know that $\mathrm{Fld}_s(\mathbb{I})$ is a summand. In other words, $\overline{\mathbb{I}}$ is then invertible due to Theorem 3.1(3). We construct a $P$-filtration $\hat{F}$ that induces $\Q$ and check algorithmically if it is a submodule of $\mathbb{M}$ as follows.

After converting $\mathbb{I}$ to a foldable module, suppose that we obtain the representative cycles $\{[z_p]\}_{p\in P_{ZZ}}$ for $\mathbb{I}$. We *cone* these cycles in the $P$-filtration $F$ of the simplicial complex $K$ that induces $\mathbb{M}$. It means that, we use a dummy vertex $\omega$ and for every simplicial complex $K_{\mathrm{Fld}_s(p)}$, $p\in P_{ZZ}$, we introduce the simplex $\sigma\cup {\omega}$ for every $\sigma\in z_p$ if it is not already present. Let the new $P$-filtration be $\hat{F}$ which filters the coned complex denoted $\hat{K}$. The $P$-module induced by $\hat{F}$ is the quotient module $\Q$. We unfold $\Q$ implicitly by unfolding $\hat{F}$ to $\hat{F}_{ZZ}$ onto $P_{ZZ}$. Let $\Q_{ZZ}$ denote the zigzag module obtained by this unfolding process. We compute a direct decomposition $\mathcal{D}$ of $\Q_{ZZ}$ from $\hat{F}_{ZZ}$ by the zigzag persistence algorithm in [15]. We say $\mathcal{D}$ is *extendable* if there is a direct decomposition $\mathcal{D}'$ of $\mathbb{M}_{ZZ}$ so that $\mathcal{D}'=\mathcal{D}\cup\{\mathbb{I}\}$.

**Proposition 6.3.** $\overline{\mathbb{I}}$ is invertible if and only if $\mathcal{D}$ is extendable.

*Proof.* If $\mathcal{D}$ is extendable, we have $\mathbb{M}_{ZZ}=\mathbb{I}\oplus \Q_{ZZ}$ by definition of extendability and the fact that $\mathcal{D}$ is a direct decomposition of $\Q_{ZZ}$. It follows that $\overline{\mathbb{I}}$ is invertible becasue $\Q_{ZZ}$ is foldable by construction.

For the opposite direction, assume that $\overline{\mathbb{I}}$ is invertible. Since $\mathbb{I}$ is foldable, $\mathrm{Fld}_s(\mathbb{I})$ exists and is a summand of $\mathbb{M}$ according to Theorem 3.1(3). The quotient $\Q=\mathbb{M}/(\mathrm{Fld}_s(\mathbb{I}))$ is a submodule of $\mathbb{M}$. This means the morphisms $\Q(p\leq_P q)$ is a restriction of $\mathbb{M}(p\leq_P q)$ for every pairs $p\leq_P q$. Therefore, by the definition of unfolding, every morphism $\Q_{ZZ}(p\leq_{P_{ZZ}} q)$ is a restriction of $\mathbb{M}_{ZZ}(p\leq_{P_{ZZ}}q)$. We claim that $\mathcal{D}'=\mathcal{D}\cup{\mathbb{I}}$ is a direct decomposition of $\mathbb{M}_{ZZ}$ establishing that $\mathcal{D}$ is extendable.

To prove the claim, observe that, due to the quotienting, the vector generating $\mathbb{I}(p)$ cannot lie in the vector space $\Q_{ZZ}(p)$ for every $p\in P_{ZZ}$. Then, $\mathbb{M}_{ZZ}(p)=\mathbb{I}(p)\oplus \Q_{ZZ}(p)$ for every $p\in P_{ZZ}$. Also, for every summand $\mathbb{I}'$ of $\Q_{ZZ}$ in $\mathcal{D}$, the morphism $\mathbb{I}'(p\leq_{P_{ZZ}} q)$ is a restriction of $\mathbb{M}_{ZZ}(p\leq_{P_{ZZ}} q)$ because we already established that $\Q_{ZZ}(p\leq_{P_{ZZ}} q)$ is a restriction of $\mathbb{M}_{ZZ}(p\leq_{P_{ZZ}}q)$. Therefore, for every interval module $\mathbb{I}_i\in \mathcal{D}\cup \{\mathbb{I}\}$, every morphism $\mathbb{I}_i(p\leq_{P_{ZZ}} q)$ is a restriction of $\mathbb{M}_{ZZ}(p\leq_{P_{ZZ}} q)$ over all pairs $p\leq_{P_ZZ} q$ while $\mathbb{M}_{ZZ}(p)=\oplus_i \mathbb{I}_i(p)$. It follows that $\mathcal{D}'$ is a direct decomposition of $\mathbb{M}_{ZZ}$.
...
*Proof.* First, assume that $\bar\mathbb{I}$ is invertible. By definition, this means that there is a summand, say $\mathbb{U}$, of $\mathbb{M}_{ZZ}$ so that $\mathbb{U}\cong \bar{\mathbb{I}}$ and $\mathbb{U}$ is foldable. Since $\mathbb{I}$ is foldable, By Theorem 3.1(1), $\mathrm{Fld}_s(\mathbb{U})$ exists and $\mathbb{M}=\mathrm{Fld}_s(\mathbb{U})\oplus\mathrm{Fld}_s(\int)$. Then $\Q\cong \mathrm{Fld}_s(\mathbb{U})$, which implies that $\Q_{ZZ}\cong \mathrm{Fld}_s^{-1}(\mathrm{Fld}_s(\mathbb{U}))=\mathbb{U}$. Therefore, the barcode of $\Q_{ZZ}$ coincides with that of $\mathbb{U}$. The barcode of $\Q_{ZZ}$ is the barcode of $\hat{F}_{ZZ}$ and the barcode of $\mathbb{U}$ is same as that of $\bar{\mathbb{I}}$.

Now, assume that the barcode of $\hat{F}_{ZZ}$ coincides with that of $\bar{\mathbb{I}}$. For contradiction, assume that $\bar{\mathbb{I}}$ is not invertible. Since $\mathrm{Fld}_s(\mathbb{I})$ exists, we still have $\Q=\mathbb{M}/(\mathrm{Fld}_s\mathbb{I})$ well defined. Then, $\mathbb{M}_{ZZ}=\mathbb{I}\oplus \Q_{ZZ}$. It means that $\Q_{ZZ}\cong \bar{\mathbb{I}}$ in which case $\bar\mathbb{I}$ is invertible reaching a contradiction.
...
Step 4 for testing convertibility and converting of $\mathbb{I}$ takes a total of $O(t^{\omega}n^2)$ time as discussed before. Testing invertibility of $\bar\mathbb{I}$ for every $\mathbb{I}$ involves coning out the filtration $F$ and unfolding $\hat{F}$ which cannot take more than $O(t)$ time. Computing the zigzag barcode of $\hat F_{ZZ}$ with representative cycles takes $O(t^2n)$ time by the algorithm in [15] giving a total time of $O(t^2 n^2)$ for $O(n)$ full interval modules like $\mathbb{I}$. The verification of the extendability of the interval decomposition $\mathcal{D}$ of the quotient module $\Q_{ZZ}$ takes $O(tn^{\omega})$ time, giving a total time of $O(tn^{\omega+1})$ over all. Therefore, step 4 takes a total time $O(t^2n^2+tn^{\omega+1})$. Accounting for all terms, we get:
...
1. Gor{\^o} Azumaya. Corrections and supplementaries to my paper concerning {K}rull-{R}emak-{S}chmidt’s theorem. *Nagoya Mathematical Journal*, 1:117–124, 1950.
2. H{\aa}vard Bakke Bjerkevik and Michael Lesnick. {$\ell$}${}^{\mbox{p}}$-distances on multiparameter persistence modules. *CoRR*, abs/2106.13589, 2021. URL: \url{https://arxiv.org/abs/2106.13589}, \href {http://arxiv.org/abs/2106.13589} {\path{arXiv:2106.13589}}.
...
5. Magnus Bakke Botnan and Michael Lesnick. An introduction to multiparameter persistence, 2023. \href {http://arxiv.org/abs/2203.14289} {\path{arXiv:2203.14289}}.
...
13. William Crawley-Boevey. Locally finitely presented additive categories. *Communications in Algebra*, 22(5):1641–1674, 1994. \href {http://arxiv.org/abs/https://doi.org/10.1080/00927879408824927} {\path{arXiv:https://doi.org/10.1080/00927879408824927}}, \href {https://doi.org/10.1080/00927879408824927} {\path{doi:10.1080/00927879408824927}}.
...
16. Tamal K. Dey, Woojin Kim, and Facundo M{é}moli. Computing generalized rank invariant for 2-parameter persistence modules via zigzag persistence and its applications. In *38th International Symposium on Computational Geometry, SoCG 2022*, volume 224 of *LIPIcs*, pages 34:1–34:17, 2022.
...
21. Peter Gabriel. {Unzerlegbare Darstellungen I}. *Manuscripta Mathematica*, 6(1):71–103, 1972. \href {https://doi.org/10.1007/BF01298413} {\path{doi:10.1007/BF01298413}}.
22. Claude{-}Pierre Jeannerod, Cl{é}ment Pernet, and Arne Storjohann. Rank-profile revealing gaussian elimination and the {CUP} matrix decomposition. *J. Symb. Comput.*, 56:46–68, 2013. URL: \url{https://doi.org/10.1016/j.jsc.2013.04.004}.
...
30. Cl{é}ment Maria and Steve Y Oudot. Zigzag persistence via reflections and transpositions. In *Proceedings of the Twenty-Sixth Annual ACM-SIAM Symposium on Discrete Algorithms*, pages 181–199. SIAM, 2014.
31. Ezra Miller. Modules over posets: commutative and homological algebra. *arXiv preprint arXiv:1908.09750*, 2019.
32. Nikola Milosavljevi{ç}, Dmitriy Morozov, and Primoz Skraba. Zigzag persistent homology in matrix multiplication time. In *Proceedings of the twenty-seventh Annual Symposium on Computational Geometry*, pages 216–225, 2011.
