...
![figure: Genrank](2403.08110v4/Genrank.png)
...
$$
![diagram 1 (tikzcd)](2403.08110v4/diagram-1.png)
$$
...
$$
![diagram 2 (tikzcd)](2403.08110v4/diagram-2.png)
$$
...
![figure: UnfoldingEx](2403.08110v4/UnfoldingEx.png)
...
![figure: UnfoldingEx3](2403.08110v4/UnfoldingEx3.png)
...
![figure: UnfoldingEx2](2403.08110v4/UnfoldingEx2.png)
...
![figure: unfoldingPath2](2403.08110v4/unfoldingPath2.png)
...
![figure: MatrixReduct](2403.08110v4/MatrixReduct.png)
...
Our goal is to determine coefficients $\alpha_i$s if Eq. (8) holds. To do this, for every $i=1,\ldots, \ell$ and every $p\in L$, consider the matrices $A_i^p$ whose columns are annotations of the representative cycles $\hat{z}_{p_j}^i$ of $v^i_j$ for every $p_j\in \mathrm{prt}(p)$; see Figure 6 for an illustration. We can compute the $\mathbb{F}$-sum of two chains $\hat{z}_{p_j}^i=z_{p_j}^i+z_p^i$ and add their annotation vectors which is well defined because their annotation vectors have the same dimension $g_p=\dim H_k(K_p)$ since all complexes at $p$ and $p_j$s were made equal during unfolding. Notice that, if $t_p=|\mathrm{prt}(p)|$, the matrix $A_i^p$ has dimensions $g_p\times t_p$. Then, checking Eq. (8) boils down to determining $\alpha_i$s so that $A_1^p=\sum_{i=2}^\ell\alpha_iA_i^p$. This can be done by using the linearization trick presented in [18]. Each of the $g_p\times t_p$ matrices $A_i^p$, $i\in \{2,\ldots,\ell\}$, is linearized into a vector $\sf v_i$ of length $t_pg_p$ and concatenated into a matrix of dimensions $t_pg_p \times \ell$. Let $A^p$ denote this matrix (Figure 6,bottom-right).
...
2. Håvard Bakke Bjerkevik and Michael Lesnick. $\ell$ ${}^{\text{p}}$-distances on multiparameter persistence modules. *CoRR*, abs/2106.13589, 2021. URL: \url<https://arxiv.org/abs/2106.13589>, \href <http://arxiv.org/abs/2106.13589> {\patharXiv:2106.13589}.
...
22. Claude-Pierre Jeannerod, Clément Pernet, and Arne Storjohann. Rank-profile revealing gaussian elimination and the CUP matrix decomposition. *J. Symb. Comput.*, 56:46–68, 2013. URL: \url<https://doi.org/10.1016/j.jsc.2013.04.004>.
