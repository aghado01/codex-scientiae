...
either, because the stamp under a module already IS the run. Inventory-tree engine batches are not
a test lane: `src/batch-runner.ps1` mints `artifacts/{engine}/{stamp}/` and must not be hung off
`tests/parallel.ps1`. The repository `.codex/` tree and
