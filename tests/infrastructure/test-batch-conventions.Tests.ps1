...
Describe 'Set-TempEnvironment' -Tag 'Infrastructure' {
    ...
    BeforeEach {
        $script:Saved = @{}
        foreach ($name in @('CDXSCI_TEMP', 'TEMP', 'TMP', 'TMPDIR')) {
    ...
    It 'sets CDXSCI_TEMP under the run directory and does not mutate ambient TEMP' {
        $outside = $script:RepositoryRoot
        foreach ($name in @('TEMP', 'TMP', 'TMPDIR')) {
            Set-Item -LiteralPath "env:$name" -Value $outside
        }
        $result = Set-TempEnvironment -RunDirectory $script:RunDir `
        ...
        $env:CDXSCI_TEMP | Should -BeExactly $result
        foreach ($name in @('TEMP', 'TMP', 'TMPDIR')) {
    ...
    It 'takes over when CDXSCI_TEMP is outside artifacts' {
        Set-Item -LiteralPath 'env:CDXSCI_TEMP' -Value $script:RepositoryRoot
        $result = Set-TempEnvironment -RunDirectory $script:RunDir `
            -RepositoryRoot $script:RepositoryRoot
        $result | Should -BeExactly ([System.IO.Path]::GetFullPath((Join-Path $script:RunDir 'temp')))
        $env:CDXSCI_TEMP | Should -BeExactly $result
    ...
    It 'refuses a RunDirectory outside artifacts and does not create it' {
        ...
        { Set-TempEnvironment -RunDirectory $outside `
            -RepositoryRoot $script:RepositoryRoot } |
    ...
    It 'does not resolve a relative RunDirectory against the process working directory' {
        ...
        try {
            ...
            { Set-TempEnvironment -RunDirectory $leaf `
                -RepositoryRoot $script:RepositoryRoot } |
                Should -Throw '*RunDirectory must be a descendant of RepositoryRoot/artifacts*'
...
Describe 'Assert-TempEnvironment' -Tag 'Infrastructure' {
    ...
    It 'rejects a missing CDXSCI_TEMP even when ambient TEMP is under artifacts' {
        Remove-Item -LiteralPath 'env:CDXSCI_TEMP' -ErrorAction SilentlyContinue
        ...
        { Assert-TempEnvironment -RepositoryRoot $script:RepositoryRoot } |
            Should -Throw '*CDXSCI_TEMP must be an absolute path under RepositoryRoot/artifacts*'
    }

    It 'projects CDXSCI_TEMP onto TEMP TMP and TMPDIR for this process' {
        ...
        $result = Assert-TempEnvironment -RepositoryRoot $script:RepositoryRoot
