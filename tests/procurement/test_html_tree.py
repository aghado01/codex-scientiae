...
class TestHtmlConfine(unittest.TestCase):
    ...
    def test_arxiv_paper_marker_accepts_latexml_generator_comment(self) -> None:
        prefix = (
            "<!DOCTYPE html><html lang=\"en\"><head>"
            "<!--Generated by LaTeXML oxide (version 0.7.6) http://dlmf.nist.gov/LaTeXML/.-->"
            "<title>Paper</title></head><body><p>site chrome</p></body></html>"
        )
        self.assertTrue(is_arxiv_html_paper(prefix))

    def test_arxiv_paper_marker_rejects_html_without_paper_or_latexml(self) -> None:
        prefix = (
            "<!DOCTYPE html><html lang=\"en\"><head><title>x</title></head><body>"
            + ("n" * 9000)
            + "</body></html>"
        )
        self.assertFalse(is_arxiv_html_paper(prefix))
...
class TestHtmlAcquisition(unittest.TestCase):
    ...
    def test_landing_with_ltx_document_after_site_chrome_is_acquired(self) -> None:
        landing = (
            b"<!DOCTYPE html><html lang=\"en\"><head><title>Paper</title></head><body>"
            + b"<nav>"
            + (b"M" * 9000)
            + b"</nav>"
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            calls.append(request.url.path)
            if request.url.path in {f"/html/{SLUG}", f"/html/{SLUG}/"}:
