...
- [Abstract](#abstract)
- [1 Introduction: Not All Angles Are Created Equal](#1-introduction-not-all-angles-are-created-equal)
- [2 A Diachronic View: Why Cosine Similarity Became Ubiquitous](#2-a-diachronic-view-why-cosine-similarity-became-ubiquitous)
  - [2.1 Roots in Information Retrieval](#21-roots-in-information-retrieval)
  - [2.2 Cosine in the Era of Word Embeddings](#22-cosine-in-the-era-of-word-embeddings)
  - [2.3 Cosine in Neural and Multimodal Models](#23-cosine-in-neural-and-multimodal-models)
  - [2.4 Why It Was the Right Tool Then](#24-why-it-was-the-right-tool-then)
- [3 Why Cosine Similarity Has Worked So Far](#3-why-cosine-similarity-has-worked-so-far)
  - [3.1 Invariance to Magnitude](#31-invariance-to-magnitude)
  - [3.2 Semantic Expressivity Through Directionality](#32-semantic-expressivity-through-directionality)
  - [3.3 Compatibility with Contrastive Losses](#33-compatibility-with-contrastive-losses)
  - [3.4 Mitigating the Curse of Dimensionality](#34-mitigating-the-curse-of-dimensionality)
- [4 Mathematical Anatomy of Cosine Similarity](#4-mathematical-anatomy-of-cosine-similarity)
  - [4.1 Vector Decomposition: Norm and Direction](#41-vector-decomposition-norm-and-direction)
  - [4.2 Cosine vs. Dot Product vs. Euclidean Distance](#42-cosine-vs-dot-product-vs-euclidean-distance)
- [Dot Product](#dot-product)
- [Euclidean Distance](#euclidean-distance)
- [Cosine Similarity](#cosine-similarity)
  - [4.3 What Cosine Ignores: The Semantics of Norm](#43-what-cosine-ignores-the-semantics-of-norm)
  - [4.4 Manifold Interpretation: Cosine Lives on the Sphere](#44-manifold-interpretation-cosine-lives-on-the-sphere)
- [5 When Cosine Similarity Breaks Down](#5-when-cosine-similarity-breaks-down)
  - [5.1 Anisotropy in Embedding Space](#51-anisotropy-in-embedding-space)
  - [5.2 Frequency Bias in Token-Level Embeddings](#52-frequency-bias-in-token-level-embeddings)
  - [5.3 Hubness in High Dimensions](#53-hubness-in-high-dimensions)
  - [5.4 Loss of Calibration and Semantic Granularity](#54-loss-of-calibration-and-semantic-granularity)
  - [5.5 Empirical Failure Scenarios](#55-empirical-failure-scenarios)
- [6 Toward Better Embedding Similarity](#6-toward-better-embedding-similarity)
  - [6.1 Norm-Aware Similarity Functions](#61-norm-aware-similarity-functions)
  - [6.2 Post-Hoc Isotropization and Whitening](#62-post-hoc-isotropization-and-whitening)
  - [6.3 Query-Normalized Adjustments for Retrieval](#63-query-normalized-adjustments-for-retrieval)
  - [6.4 Hybrid Measures and Angular–Radial Decoupling](#64-hybrid-measures-and-angularradial-decoupling)
...
As of April 2025, for instance, OpenAI’s embedding models 1 and Google Vertex AI’s text-embedding-004/005 models 2 return $\ell_2$ -normalized vectors, illustrating the field’s implicit endorsement of cosine-based comparisons, although many other providers and open-source models still ship unnormalized embeddings.

![Figure 1: Historical trends of relevant terminologies—cosine similarity, text embedding, and retrieval-augmented generation (RAG)—from 2004 to 2024, using Google Trends and PubMed publication data.](2504.16318v2_images/imageFile1.png)
...
As representation learning matured, cosine similarity was integrated into deep neural architectures, particularly within contrastive learning frameworks. In S IM CLR [2], M O C O [7], and BYOL [6], embeddings are often $\ell_2$ -normalized before loss computation, effectively reducing the training objective to maximizing cosine similarity between positive pairs while minimizing it for negatives.
...
One of the primary advantages of cosine similarity is its normalization of vector scale. Given two vectors $u, v \in R^d$ , cosine similarity is defined as:
...
$$
\mathcal{L} = - \log \sigma ( v_{w}^{\top} v_{c} ) - \sum_{i = 1}^{k} \log \sigma ( - v_{n_{i}}^{\top} v_{c} ) ,
$$

where $\sigma$ is the sigmoid function, and $v_w$, $v_c$ , and $v_{n_i}$ are the embeddings of the center word w , the context word c , and the $k$ negative samples n i , respectively.
...
$$
\mathcal{L}_{\text{INONCE}} = - \log \frac{\exp ( \sin ( u , v ) / \tau )}{\sum_{i = 1}^{N} \exp ( \sin ( u , v_{i} ) / \tau )} ,
$$

where sim ( · , · ) typically denotes cosine similarity, and $\tau$ is a temperature parameter controlling distribution sharpness.
...
Cosine similarity typically outperforms unnormalized dot products and raw $\ell_2$ distances, yet it still suffers from pronounced hubness in high-dimensional settings, a limitation addressed by debiasing methods such as QB-N ORM [1]. Nevertheless, it remains a pragmatic choice for a myriad of applications in high-dimensional regimes.
...
Let $x$, $y$ $\in$ $R^d$ be two nonzero vectors, such as word or sentence embeddings. Each vector can be

$$
x = \| x \| \cdot \hat{x} \ \Longrightarrow \ \hat{x} = \frac{x}{\| x \|} \in S^{d - 1} ,
$$

where $\|x\|$ is the Euclidean norm, and $\hat{x}$ lies on the unit hypersphere $S^{d-1}$ $\subset$ $R^d$ . The cosine similarity between x and y is:

$$
\cos_{-} \sin ( x , y ) = \frac{x^{\top} y}{\| x \| \| y \|} = \hat{x}^{\top} \hat{y} = \cos ( \theta ) ,
$$
...
$\|x\|$ $\to$ certainty, salience, or informativeness , $\hat{x}$ $\to$ semantic direction .
...
$$
d_{\text{angle}} ( x , y ) = \arccos ( \hat{x}^{\top} \hat{y} ) .
$$
...
$$
\cos_{0} \quad \cos_{-} \sin ( w_{h i g h \text{-freq}} , w^{\prime} ) < \cos_{-} \sin ( w_{l o w \text{-freq}} , w^{\prime} ) ,
$$
...
Cosine similarity treats all vectors on the unit sphere equally, discarding variation in norm that may correlate with prediction confidence, specificity, or informativeness. For example, consider two vectors $c_{1}$ and $c_{2}$ such that ˆ $c_{1}$ = ˆ $c_{2}$ but $\|$ $c_{1}$ $\|$ $\gg$ $\|$ $c_{2}$ $\| .$ Cosine similarity considers them equivalent:

$$
\cos \sin ( c_{1} , v ) = \cos \sin ( c_{2} , v ) .
$$
...
where $\alpha \in [0,1]$ governs the trade-off between angular and radial components. Such formulations have been explored in contexts involving confidence estimation and retrieval calibration [13, 15].
...
$$
\tilde{s} ( q , d ) = \frac{\cos_{\sim} \sin ( q , d ) - \mu_{q}}{\sigma_{q}} ,
$$

where $\mu$ q and $\sigma_q$ are the mean and standard deviation of cosine similarities between query q and a background set. This contextualizes similarity scores and improves ranking quality without requiring retraining.
