...
Let σ ∈ S(ω). By (11.33), $\sigma_y$ = ηx,yσx where ηx,y is the product of the signs of the Je for e ∈ ρ. Thus, the relative signs of the spins on C are determined by knowledge of ω. Since there are two possible choices for the spin at any given x, there are two choices for the spin-conﬁguration on C, and we choose between these according to the ﬂip of a fair coin. In summary, we assign spins randomly to V in such a way that: the spins within a cluster satisfy $\sigma_y$ = ηx,yσx as above, and the spins of different clusters are independent.
