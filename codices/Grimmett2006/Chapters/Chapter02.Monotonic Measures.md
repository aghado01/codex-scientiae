...
- (2.7) µ2($\omega_e$)µ1($\omega_e$) ≥ µ1($\omega_e$)µ2($\omega_e$), ω ∈ , e ∈ E. If either µ1 or µ2 satisﬁes (2.8) µ(ωef )µ(ωef ) ≥ µ(ωef )µ(ωef ), ω ∈ , e, f ∈ E, then (2.2) holds.
...
Let E be a finite set as in the last section, and let \Omega = \{0,1\}^E. A probability measure µ on is said to have the FKG lattice property if it satisﬁes the so-called FKG lattice condition:
...
Let N ≥ 1, and let E be an arbitary finite set with |E| = N. We write \Omega = \{0,1\}^E as usual, and F for the set of all subsets of . Let µ be a probability measure on ($\Omega, \mathcal{F}$), and A an increasing event. The (conditional) inﬂuence on A of the edge e ∈ E is defined by
