...
Unfortunately, this heuristic approach suffers from the problem that the different classiﬁers were trained on different tasks, and there is no guarantee that the realvalued quantities $y_k(\mathbf{x})$ for different classiﬁers will have appropriate scales.
