...
$$
It is known that any zigzag module decomposes into a direct sum of interval modules [5]. Following [3], we distinguish four different types of interval modules. Here, we denote by < and \leq the partial order in Z^2.
$$

\begin{aligned} ( a, b ) _ { ZZ } &\colon= \{ i \in Z \mid ( a, a ) < \iota ( i ) < ( b, b ) \} \quad \text {for } a < b \in Z \cup \{ - \infty, \infty \}, \\ [ a, b ) _ { ZZ } &\colon= \{ i \in Z \mid ( a, a ) \leq \iota ( i ) < ( b, b ) \} \quad \text {for } a < b \in Z \cup \{ \infty \}, \\ ( a, b ] _ { ZZ } &\colon= \{ i \in Z \mid ( a, a ) < \iota ( i ) \leq ( b, b ) \} \quad \text {for } a < b \in Z \cup \{ - \infty \}, \\ [ a, b ] _ { ZZ } &\colon= \{ i \in Z \mid ( a, a ) \leq \iota ( i ) \leq ( b, b ) \} \quad \text {for } a \leq b \in Z.
\end{aligned}
...
\begin{aligned} ( a, b ) _ { B L } & \colon = \{ ( x, y ) \in Z ^ { \mathrm{op} } \times Z \mid a < x, y < b \} \quad \text { for } a < b \in Z \cup \{ - \infty, \infty \}, \\ [ a, b ) _ { B L } & \colon = \{ ( x, y ) \in Z ^ { \mathrm{op} } \times Z \mid a \leq y < b \} \quad \text { for } a < b \in Z \cup \{ \infty \}, \\ ( a, b ] _ { B L } & \colon = \{ ( x, y ) \in Z ^ { \mathrm{op} } \times Z \mid a < x \leq b \} \quad \text { for } a < b \in Z \cup \{ - \infty \}, \\ [ a, b ] _ { B L } & \colon = \{ ( x, y ) \in Z ^ { \mathrm{op} } \times Z \mid x \leq b, y \geq a \} \quad \text { for } a \leq b \in Z \,.
\end{aligned}
...
$$
\in R^{\mathrm{op}} \times R \times R be such that v_1 \leq 0 and v_2,v_3 \geq 0 .
...
## 2. Let F,G \in \mathbf{vec}^{R^{\mathrm{op}} \times R \times R} For any morphism \eta : F \to G , the corresponding morphism \eta^v : F^v \to G^v is defined as \eta^v ( x ) = \eta ( x + v ) : F^v ( x ) \to G^v ( x ) for all x \in R^{\mathrm{op}} \times R \times R .
...
where $\psi^v \_F : F \to F^v$ denotes the natural transformation whose restriction to $F ( x )$ is the linear map $F ( x \leq x + v )$.
